...
export function RolePermissionsDialog({ role, isOpen, onClose }: RolePermissionsDialogProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            ...
            <Select value={departmentId ||"none"} options={[ { id:"none", title:"Без отдела" }, ...departments.map(dept => ({ id: dept.id, title: dept.name }))
              ]}
              ...
              <div key={section.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {ACTIONS.map(action => {
                    ...
                    return (
                      <Button key={action.id} variant="ghost" onClick={() => handleToggle(section.id, action.id)}
                    ...
                    {ACTIONS.map(action => {
                      ...
                      return (
                        <td key={action.id} className="crm-td text-center">
                          <Button variant="ghost" size="icon" onClick={() => handleToggle(section.id, action.id)}
  ...
  const ActionsContent = (
    <div className="flex items-center justify-between gap-3 w-full">
      {role && role.name !=="Администратор" && (
        <Button variant="ghost" onClick={handleDelete} disabled={loading} className="flex-1 inline-flex items-center justify-center gap-2 h-11 text-rose-600 bg-rose-50 hover:bg-rose-100/50 text-sm font-bold rounded-[var(--radius-inner)] transition-all active:scale-95 disabled:opacity-50 border-none">
      ...
      <div className="flex items-center gap-3 flex-1 justify-end">
        <Button variant="ghost" onClick={onClose} className="hidden sm:flex h-11 px-6 font-bold text-slate-500 hover:bg-slate-50 rounded-[var(--radius-inner)] border border-slate-200" disabled={loading}>
          Отмена
        </Button>
        <Button onClick={handleSave} disabled={loading} className="flex-1 sm:flex-none h-11 sm:px-10 min-w-[120px] font-bold text-white shadow-lg shadow-primary/20">
          {loading ? (
  ...
  return (
    <>
      <ResponsiveModal isOpen={isOpen} onClose={onClose} title="Редактирование роли" description="Настройте уровень доступа и параметры для роли" className="max-w-4xl">
        <div>
          <div className="mb-4">
            {FormContent}
          </div>
      ...
      <DeleteRoleDialog role={role ? { id: role.id, name: role.name } : null} isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)}
