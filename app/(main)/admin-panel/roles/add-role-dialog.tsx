...
export function AddRoleDialog({ onSuccess }: AddRoleDialogProps) {
  ...
  const PermissionButton = ({ sectionId, actionId, label, isDesktop = false }: { sectionId: string, actionId: string, label?: string, isDesktop?: boolean }) => {
    ...
    if (isDesktop) {
      return (
        <Button type="button" variant="ghost" size="icon" onClick={() => handleToggle(sectionId, actionId)}
    ...
    return (
      <Button type="button" variant="ghost" onClick={() => handleToggle(sectionId, actionId)}
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <IconInput startIcon={Shield} type="text" name="name" required placeholder="Например: Оператор цеха" className="crm-dialog-field" />
              </div>
                  ...
                  {SECTIONS.map(section => (
                    <div key={section.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {ACTIONS.map(action => (
                          <PermissionButton key={action.id} sectionId={section.id} actionId={action.id} label={action.label} />
                        ))}
                      </div>
                    </div>
                  ))}
                        ...
                        <tr key={section.id} className="crm-tr">
                          ...
                          {ACTIONS.map(action => (
                            <td key={action.id} className="crm-td text-center">
                              <PermissionButton sectionId={section.id} actionId={action.id} isDesktop={true} />
                            </td>
                          ))}
                        </tr>
                      ))}
            ...
            <div className="mt-5 pt-5 border-t border-slate-200 flex flex-col md:flex-row gap-3 sticky bottom-0 bg-white">
              ...
              <Button type="submit" disabled={loading} className="flex-[2] inline-flex justify-center items-center gap-2 rounded-[var(--radius-inner)] font-bold text-white shadow-lg shadow-primary/20 transition-all active:scale-[0.98] h-11">
