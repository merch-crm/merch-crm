  ...
  return (
    <div className="space-y-3">
      <div className="relative max-w-md">
        ...
        <Input type="text" placeholder="Поиск сотрудника..." value={state.searchValue} onChange={(e) => actions.handleSearch(e.target.value)}
        ...
        />
        {state.searchValue && (
          <Button variant="ghost" size="icon" onClick={() => { actions.setSearchValue(""); router.push(pathname); }}
                      ...
                      >
                        ...
                        <td className="crm-td">
                          <div className="flex items-center gap-3">
                            <Avatar className={cn("h-10 w-10 border shadow-sm", isSelected ? "border-primary/20" : "border-slate-100" )}>
                              <AvatarImage src={user.image || ""} alt={user.name} />
                              <AvatarFallback className="text-white font-bold text-xs" style={{ backgroundColor: (typeof user.role === 'object' ? user.role?.color : null) || user.department_color || "#94a3b8" }}>
                        ...
                        <td className="crm-td crm-td-actions" onClick={(e) => e.stopPropagation()}>
                          <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="icon" onClick={() => actions.setImpersonateUserConfirm(user)}
      ...
      />
      {
        initialUsers.length === 0 && (
          <EmptyState icon={Users} title="Сотрудники не найдены" description="Попробуйте изменить запрос или добавьте нового сотрудника" />
        )
      }

      <DeleteUserDialog isOpen={!!state.deletingUser} user={state.deletingUser} onClose={() => actions.setDeletingUser(null)}
        onSuccess={() => {
      ...
      <EditUserDialog key={state.editingUser?.id} isOpen={!!state.editingUser} user={state.editingUser} onClose={() => actions.setEditingUser(null)}
        onSuccess={() => {
      ...
      <UserStatsDrawer isOpen={!!state.viewingStatsUser} userId={state.viewingStatsUser?.id || null} onClose={() => actions.setViewingStatsUser(null)}
      />

      <ConfirmDialog isOpen={!!state.impersonateUserConfirm} onClose={() => actions.setImpersonateUserConfirm(null)}
        onConfirm={async () => {
