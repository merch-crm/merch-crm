...
export function AddUserDialog({ onSuccess }: { onSuccess?: () => void }) {
  ...
  return (
    <>
      ...
      <ResponsiveModal isOpen={state.isOpen} onClose={() => updateState({ isOpen: false })}
        ...
        <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pb-5">
            <div className="space-y-1">
              ...
              <IconInput startIcon={User} type="text" name="name" required placeholder="Иван Иванов" className="crm-dialog-field" />
            </div>

            <div className="space-y-1">
              <label className="text-sm font-bold text-slate-700 pl-1">Email (Логин)</label>
              <IconInput startIcon={Mail} type="email" name="email" required placeholder="ivan@crm.local" className="crm-dialog-field" />
            </div>

            <div className="space-y-1">
              <label className="text-sm font-bold text-slate-700 pl-1">Пароль</label>
              <IconInput startIcon={Lock} type="password" name="password" required placeholder="••••••••" className="crm-dialog-field" />
            </div>
