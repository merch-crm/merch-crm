...
export function EditUserDialog({ user, isOpen, onClose, onSuccess }: EditUserDialogProps) {
  ...
  return (
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title="Редактирование" description={`Измените данные сотрудника ${user.name}`} className="items-start">
      <div>
        {state.error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-3 pb-5">
          <div className="space-y-1">
            ...
            <IconInput startIcon={User} type="text" name="name" required defaultValue={user.name} placeholder="Иван Иванов" className="block w-full rounded-[18px] border-slate-200 bg-slate-50 text-slate-900 shadow-sm focus:border-primary focus:ring-0 px-3 py-2.5 transition-all placeholder:text-slate-300 h-11" />
          </div>

          <div className="space-y-1">
            <label className="text-sm font-bold text-slate-700 pl-1">Email (Логин)</label>
            <IconInput startIcon={Mail} type="email" name="email" required defaultValue={user.email} placeholder="ivan@crm.local" className="block w-full rounded-[18px] border-slate-200 bg-slate-50 text-slate-900 shadow-sm focus:border-primary focus:ring-0 px-3 py-2.5 transition-all placeholder:text-slate-300 h-11" />
          </div>

          <div className="space-y-1">
            <label className="text-sm font-bold text-slate-700 pl-1">Новый пароль (оставьте пустым, если не хотите менять)</label>
            <div className="relative">
              <IconInput startIcon={Lock} type={state.showPassword ?"text" :"password"} name="password" placeholder="••••••••" className="block w-full pr-10 rounded-[18px] border-slate-200 bg-slate-50 text-slate-900 shadow-sm focus:border-primary focus:ring-0 px-3 py-2.5 transition-all placeholder:text-slate-300 h-11" />
              <Button type="button" variant="ghost" size="icon" onClick={() => updateState({ showPassword: !state.showPassword })}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <IconSelect startIcon={Shield} name="roleId" value={state.selectedRoleId} options={state.roles.map(role => ({ id: role.id, title: role.name }))}
            ...
            <div className="space-y-1">
              ...
              <IconSelect startIcon={Building} name="departmentId" value={state.selectedDeptId} options={[ { id:"", title:"Все отделы" }, ...state.departments.map(dept => ({ id: dept.id, title: dept.name }))
                ]}
