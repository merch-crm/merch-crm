...
export default function RolesDepartmentsPage() {
  ...
  return (
    <div className="space-y-3 pb-20">
      <AdminPageHeader title="Структура и роли" subtitle="Управление отделами компании и правами доступа сотрудников" icon={Shield} actions={ <div className="flex gap-2">
            {activeTab === "roles" ? (
      ...
      <Tabs defaultValue="roles" onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-slate-100/50 p-1 rounded-2xl border border-slate-200/60 mb-4 h-auto w-full sm:w-auto overflow-x-auto justify-start">
          <TabsTrigger value="roles" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-primary font-bold text-slate-500 transition-all gap-2">
            <Shield className="w-4 h-4" />
            Роли и права
          </TabsTrigger>
          <TabsTrigger value="departments" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-primary font-bold text-slate-500 transition-all gap-2">
