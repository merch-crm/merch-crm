...
export function DepartmentSettingsDialog({ department, isOpen, onClose, onSuccess }: DepartmentSettingsDialogProps) {
    ...
    <div className="space-y-3">
      <div className="space-y-3">
        <div className="space-y-1">
          ...
          <IconInput startIcon={Building} type="text" name="name" defaultValue={department.name} required className="block w-full rounded-xl border-slate-200 bg-slate-50 text-slate-900 shadow-sm focus:border-primary focus:ring-0 px-3 py-3 transition-all font-bold h-12" />
        </div>

        <div className="space-y-1">
          <label className="text-sm font-bold text-slate-700 ml-1">Описание деятельности</label>
          <Textarea name="description" defaultValue={department.description ||""} rows={3} placeholder="Опишите, чем занимается этот отдел..." className="block w-full rounded-xl border-slate-200 bg-slate-50 text-slate-900 shadow-sm focus:border-primary focus:ring-0 px-3 py-3 transition-all resize-none leading-relaxed" />
        </div>
      ...
      <div className="pt-6 border-t border-slate-200">
        <Button type="submit" disabled={state.loading} className="w-full h-12 rounded-xl font-bold text-white shadow-xl transition-all active:scale-[0.98]">
          ...
          {state.rolesLoading ? (
            ...
            state.departmentRoles.map(role => (
              <div key={role.id} className="flex items-center justify-between p-4 bg-white border border-slate-200/60 rounded-xl group hover:border-primary/40 hover:shadow-md transition-all">
                ...
                <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveRole(role.id)}
          ...
          {state.rolesLoading ? (
            ...
            availableRoles.map(role => (
              <div key={role.id} className="flex items-center justify-between p-4 bg-slate-50/50 border border-slate-200 rounded-xl group hover:border-primary/40 hover:bg-white transition-all">
                ...
                <Button type="button" variant="outline" size="icon" onClick={() => handleAddRole(role.id)}
                  disabled={!!state.actionLoading}
  ...
  return (
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title={department.name} description="Параметры и управление отделом" className="max-w-3xl">
      <Tabs value={state.activeTab} onValueChange={(val) => updateState({ activeTab: val })} className="w-full">
        <div className="px-0 pt-2 pb-1">
          <TabsList className="bg-[#f1f5f9] border-none rounded-full p-1 w-full sm:w-auto inline-flex h-auto shadow-none">
            <TabsTrigger value="general" className="flex-1 sm:flex-none px-6 py-2.5 text-xs font-bold rounded-full transition-all data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-[0_2px_10px_-3px_rgba(0,0,0,0.07)]">
              Основное
            </TabsTrigger>
            <TabsTrigger value="roles" className="flex-1 sm:flex-none px-6 py-2.5 text-xs font-bold rounded-full transition-all data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-[0_2px_10px_-3px_rgba(0,0,0,0.07)]">
              Роли отдела
            </TabsTrigger>
          </TabsList>
        </div>
