...
export function AddDepartmentDialog({ onSuccess }: AddDepartmentDialogProps) {
  ...
  return (
    <>
      <Button type="button" onClick={() => updateState({ isOpen: true })}
        size={isMobile ? "sm" : "lg"}
        ...
        <div className="pb-2">
          ...
          >
            <div className="space-y-1">
              ...
              <IconInput startIcon={Building} type="text" name="name" required placeholder="Например: Цех печати" className="crm-dialog-field" />
            </div>

            <div className="space-y-1">
              <label className="text-sm font-bold text-slate-700 ml-1">Описание</label>
              <Textarea name="description" rows={isMobile ? 3 : 2} placeholder="Чем занимается этот отдел..." className="crm-dialog-textarea h-auto min-h-[80px]" />
            </div>
                ...
                {state.fetchingRoles ? (
                  ...
                  <div className="grid grid-cols-1 gap-1">
                    {state.roles.map(role => {
                      ...
                      return (
                        <Button key={role.id} type="button" variant="ghost" onClick={() => toggleRole(role.id)}
            ...
            <div className="mt-5 pt-4 border-t border-slate-200">
              <Button type="submit" disabled={state.loading} className="w-full h-12 rounded-[var(--radius-inner)] font-bold text-white shadow-lg shadow-primary/20 transition-all active:scale-[0.98]">
