...
export function AuditClient({ initialLogs, pagination, users }: AuditClientProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
        <div className="lg:col-span-2 crm-card p-4">
          <form onSubmit={handleSearchSubmit} className="relative group">
            ...
            <Input value={search} onChange={handleSearchChange} placeholder="Поиск по действию..." className="pl-10 h-11 bg-slate-50 border-slate-200 rounded-2xl font-medium focus:ring-primary/10 transition-all" />
          </form>
        </div>
        
        <div className="lg:col-span-1 crm-card p-4 flex gap-2">
          <Select value={userId} options={[ { id: "all", title: "Все пользователи" }, ...(users || []).map(u => ({ id: u.id, title: u.name }))
            ]}
      ...
      {/* Details Modal */}
      <ResponsiveModal isOpen={isDetailsOpen} onClose={() => setIsDetailsOpen(false)}
