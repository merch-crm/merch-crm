  ...
  return (
    <nav className="flex-1 space-y-2 mb-8 px-2">
      {navItems.map((item) => {
        ...
        return (
          <Button key={item.name} type="button" variant="ghost" onClick={() => router.push(item.href)}
          ...
          >
            ...
            <ChevronRight className={cn("ml-auto w-5 h-5 relative z-10 transition-all duration-500", isActive ?"opacity-100 text-primary translate-x-0" :"opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 text-slate-600" )} />
          </Button>
        );
      })}
    </nav>
