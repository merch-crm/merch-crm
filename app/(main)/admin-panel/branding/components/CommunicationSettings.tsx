...
export function CommunicationSettings({ formData, setFormData, handleFileUpload, uploadStates }: CommunicationSettingsProps) {
        ...
        <div className="crm-card p-6 space-y-3">
          ...
          <div className="space-y-3">
              ...
              <div className="p-4 bg-slate-50/50 rounded-2xl border border-slate-200 border-dashed flex items-center gap-3">
                <div className="w-20 h-12 bg-white rounded-xl border border-slate-200 p-2 flex items-center justify-center">
                  {formData.emailLogoUrl ? (
                    <Image src={formData.emailLogoUrl} className="max-w-full max-h-full object-contain" alt="Email Logo" width={80} height={48} />
                  ) : (
                ...
                <div className="flex-1">
                  ...
                  <Button type="button" variant="outline" className="h-9 px-4 bg-white font-bold" onClick={() => document.getElementById("email-l-u")?.click()} disabled={currentUpload?.uploading}>
                    {currentUpload?.uploading ? (
                      ...
                      <>
                        <Upload className="w-3.5 h-3.5 mr-2" /> Загрузить
                      </>
                    )}
                  </Button>
            ...
            <div>
              ...
              <Input value={formData.emailSignature || ""} onChange={(e) => setFormData(prev => ({ ...prev, emailSignature: e.target.value }))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="space-y-3">
                <div>
                  ...
                  <Input value={formData.socialTelegram || ""} onChange={(e) => setFormData(prev => ({ ...prev, socialTelegram: e.target.value }))}
                ...
                <div>
                  ...
                  <Input value={formData.socialWhatsapp || ""} onChange={(e) => setFormData(prev => ({ ...prev, socialWhatsapp: e.target.value }))}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input value={formData.socialWebsite || ""} onChange={(e) => setFormData(prev => ({ ...prev, socialWebsite: e.target.value }))}
            ...
            <div className="pt-4 border-t border-slate-50">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden scale-90 origin-top">
                <div className="p-6 border-b border-slate-100 flex justify-center">
                  <Image src={formData.emailLogoUrl || formData.logoUrl || "/logo.png"} className="h-8 w-auto object-contain" alt="Brand" width={120} height={32} />
                </div>
            ...
            <div className="p-6 bg-slate-50/50 rounded-2xl border border-slate-200 border-dashed flex items-center gap-3">
              <div className="w-20 h-20 bg-white rounded-xl border border-slate-200 p-3 flex items-center justify-center shadow-inner">
                {formData.printLogoUrl ? (
                  <Image src={formData.printLogoUrl} className="max-w-full max-h-full object-contain" alt="Print" width={80} height={80} />
                ) : (
              ...
              <div className="flex-1 space-y-2">
                ...
                <Button type="button" variant="outline" className="h-10 px-6 bg-white font-bold shadow-sm" onClick={() => document.getElementById("print-u-unified")?.click()} disabled={currentUpload?.uploading}>
                  {currentUpload?.uploading ? (
                    <div className="flex items-center gap-2">
          ...
          <div className="bg-slate-50/10 rounded-2xl p-6 border border-slate-100 flex flex-col items-center justify-center">
            ...
            <div className="bg-white p-6 border border-slate-200 shadow-sm rounded-sm w-full max-w-[280px] aspect-[1/1.41] space-y-3">
              <div className="flex justify-between items-start">
                <Image src={formData.printLogoUrl || formData.logoUrl || "/logo.png"} className="h-10 w-auto object-contain grayscale opacity-80" alt="Print Logo" width={120} height={40} />
