            ...
            {[
                ...
                return (
                    <TabsTrigger
                        key={tab.id}
                        value={tab.id}
                        className={cn("flex-1 h-full relative flex items-center justify-center gap-2.5 px-6 !rounded-[16px] text-sm font-bold transition-all border-none data-[state=active]:bg-transparent data-[state=active]:!text-white data-[state=active]:shadow-none text-slate-500 hover:text-slate-900"
                        )}
                    >
                        <AnimatePresence>
                            {isActive && (
