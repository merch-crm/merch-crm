  ...
  return (
    <TabsList className="flex w-full h-[58px] items-center gap-2 !p-[6px] !rounded-2xl crm-card bg-white/50 border border-slate-200/60 transition-all overflow-x-auto no-scrollbar">
      {[
        ...
        return (
          <TabsTrigger key={tab.id} value={tab.id} className={cn("flex-1 h-full relative flex items-center justify-center gap-2.5 px-6 !rounded-[16px] text-sm font-bold transition-all border-none data-[state=active]:bg-transparent data-[state=active]:!text-white data-[state=active]:shadow-none text-slate-500 hover:text-slate-900" )}>
            <AnimatePresence>
              {isActive && (
                <motion.div
