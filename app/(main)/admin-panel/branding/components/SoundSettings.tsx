...
import { Volume2, VolumeX, Upload, RefreshCw, ShoppingBag, Users, Warehouse, Banknote, MessageCircle, ListTodo, Cpu, Printer, ScanLine, MousePointer2, Stars, Loader2 } from "lucide-react";
...
export function SoundSettings({ formData, setFormData, ui, setUi, handleFileUpload, uploadStates }: SoundSettingsProps) {
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5 leading-tight">{info.desc}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button type="button" variant="ghost" size="sm" className={cn("h-10 w-10 hover:text-slate-900 overflow-hidden", (isCustom || currentUpload?.uploading) ? "text-primary font-bold" : "text-slate-400")} title="Заменить звук" onClick={() => document.getElementById(`upload-${soundType}`)?.click()}
                        disabled={currentUpload?.uploading}
                      >
                        {currentUpload?.uploading ? (
                          <div className="relative w-full h-full flex items-center justify-center">
                            <Loader2 className="w-5 h-5 animate-spin absolute" />
                            <span className="text-xs font-bold z-10">{Math.round(currentUpload.progress)}</span>
                          </div>
                        ) : (
                      ...
                      {/* Reset Button */}
                      <Button type="button" variant="ghost" size="sm" className={cn("h-10 w-10 transition-colors", isModified ? "text-slate-400 hover:text-rose-500" : "text-slate-200 cursor-not-allowed" )} disabled={!isModified} title="Восстановить по умолчанию" onClick={resetSound}>
                        <RefreshCw className="w-5 h-5" />
                      </Button>

                      <div className="w-px h-5 bg-slate-200 mx-1" />

                      <Button type="button" variant="ghost" size="icon" onClick={() => playSound(soundType as SoundType)}
