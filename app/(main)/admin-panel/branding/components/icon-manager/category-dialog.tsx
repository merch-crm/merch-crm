        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-8">
              ...
              <Button type="button" variant="ghost" size="icon" onClick={onClose} className="w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200">
            ...
            <div className="space-y-3">
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <Input value={category.label} onChange={e => onChange({ ...category, label: e.target.value })}
                    ...
                    <PopoverContent className="w-[320px] p-0 z-[100] rounded-2xl shadow-2xl border-slate-100 overflow-hidden" align="end">
                      <div className="p-3 border-b border-slate-50 bg-slate-50/50">
                        <div className="relative">
                          ...
                          <Input type="text" placeholder="Поиск..." className="w-full h-9 pl-9 pr-4 border-slate-200 rounded-lg text-xs font-bold" value={iconPickerSearch} onChange={(e) => setIconPickerSearch(e.target.value)}
                          />
                        </div>
                      </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2 px-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Button type="button" variant="outline" size="sm" className="h-7 px-2 text-xs font-bold rounded-lg border-slate-200" onClick={() => fileInputRef.current?.click()}
                    >
                    ...
                    <Popover open={isLibraryPickerOpen} onOpenChange={setIsLibraryPickerOpen}>
                      <PopoverTrigger asChild>
                        <Button type="button" variant="outline" size="sm" className="h-7 px-2 text-xs font-bold rounded-lg border-slate-200">
                      ...
                      <PopoverContent className="w-[320px] p-0 z-[100] rounded-2xl shadow-2xl border-slate-100 overflow-hidden" align="end">
                        <div className="p-3 border-b border-slate-50 bg-slate-50/50">
                          <div className="relative">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                            <Input type="text" placeholder="Поиск в библиотеке..." className="w-full h-9 pl-9 pr-4 border-slate-200 rounded-lg text-xs font-bold" value={librarySearch} onChange={(e) => setLibrarySearch(e.target.value)}
                              autoFocus
                            />
                          </div>
                        </div>
            ...
            <div className="flex items-center justify-between mt-8 pt-2">
              {isExisting ? (
                <Button type="button" variant="ghost" className="btn-destructive-ghost font-bold px-4 h-11 rounded-[var(--radius-inner)]" onClick={() => onDelete(category.id)}
                >
              ...
              <div className="flex items-center gap-3">
                <Button type="button" variant="outline" className="rounded-[14px] font-bold h-11 px-6 border-slate-200 text-slate-500" onClick={onClose}>
                  Отмена
                </Button>
                <Button type="button" variant="ghost" className="btn-dark !bg-[#1b1b1b] hover:!bg-black hover:!text-white rounded-[var(--radius-inner)] font-bold h-11 px-8 shadow-xl shadow-slate-900/10 border-none text-white" onClick={onSave} disabled={isSaving}>
                  {isSaving ? (
