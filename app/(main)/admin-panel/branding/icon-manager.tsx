...
export function IconManager({ initialData }: { initialData?: SerializedIconGroup[] }) {
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="px-6 pt-6 pb-2">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
            ...
            <div className="flex items-center gap-2">
              <Button type="button" variant="outline" size="sm" className="rounded-md font-bold h-9 border-slate-200" onClick={() => handleAddNewCategory()}
              >
                <Plus className="w-4 h-4 mr-2 text-primary" />
                Создать категорию
              </Button>
              <Button type="button" variant="outline" size="sm" className="rounded-md font-bold h-9 border-slate-200" onClick={() => setUiState(prev => ({ ...prev, showUploadModal: true }))}
              >
                  ...
                  <div key={`group-${group.id}-${groupIndex}`} className="bg-white rounded-xl border border-slate-200/60 shadow-sm flex flex-col overflow-hidden group/card shadow-hover transition-all">
                    ...
                    <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <Button type="button" variant="ghost" size="icon" onClick={() => setUiState(prev => ({ ...prev, editingCategory: group }))}
                          className="h-7 w-7 rounded-lg text-slate-400 hover:text-primary"
                          aria-label={`Редактировать категорию ${group.label}`}
                        >
                          <Pencil className="w-3.5 h-3.5" />
                        </Button>
                        <Button type="button" variant="ghost" size="icon" onClick={() => handleDeleteCategory(group.id)}
      ...
      <CategoryDialog isOpen={!!uiState.editingCategory} onClose={() => setUiState(prev => ({ ...prev, editingCategory: null }))}
      ...
      <UploadIconModal isOpen={uiState.showUploadModal} onClose={() => setUiState(prev => ({ ...prev, showUploadModal: false }))}
        onUpload={handleUploadFromModal}
      />

      <DeleteConfirmation isOpen={!!uiState.categoryToDelete} onClose={() => setUiState(prev => ({ ...prev, categoryToDelete: null }))}
            ...
            >
              <div className="crm-card flex items-center gap-3 !shadow-2xl !border-2 !border-primary/20 !bg-white/95 backdrop-blur-xl">
                ...
                <div className="flex items-center gap-2 shrink-0 ml-auto">
                  <Button type="button" size="sm" variant="ghost" className="rounded-[10px] h-9 w-9 p-0" onClick={() => setUiState(prev => ({ ...prev, selectedIcon: null }))}
