...
export async function getBrandingAction(): Promise<ActionResult<BrandingSettings>> {
    return withAuth(async () => {
        ...
        if (!setting) return ok(defaultBranding);
    ...
    }, { roles: ROLE_GROUPS.ADMINS, errorPath: "getBrandingAction" });
}
...
export async function updateBrandingAction(data: BrandingSettings): Promise<ActionResult> {
    ...
    return withAuth(async () => {
        ...
        return okVoid();
    }, { roles: ROLE_GROUPS.ADMINS, errorPath: "updateBrandingAction" });
}
...
export async function uploadBrandingFile(formData: FormData): Promise<ActionResult<{ url: string }>> {
    return withAuth(async () => {
        ...
        if (!file || !type) return ERRORS.VALIDATION("Отсутствуют необходимые данные");
        ...
        if (result.success) {
            return ok({ url: `/api/storage/local/${fileName}` });
        }
        return ERRORS.INTERNAL(result.error || "Failed to save file");
...
export async function getIconGroups(): Promise<ActionResult<SerializedIconGroup[]>> {
    return withAuth(async () => {
        ...
        if (!settings) return ok(serializeIconGroups(ICON_GROUPS));
        return ok(settings.value as SerializedIconGroup[]);
    }, { roles: ROLE_GROUPS.ADMINS, errorPath: "getIconGroups" });
}
