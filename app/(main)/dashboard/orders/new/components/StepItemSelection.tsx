      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-2 max-h-[400px] overflow-y-auto pr-2">
            {inventory.map(item => (
              <Button key={item.id} type="button" variant="ghost" disabled={selectedItems.some(i => i.id === item.id)}
                ...
                <div key={item.id} className="bg-card p-4 rounded-2xl shadow-sm border border-border space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-bold">
                    <div className="space-y-1.5">
                      ...
                      <Input type="number" value={item.orderQuantity || 0} onChange={(e) => onUpdateItem(item.id, { orderQuantity: Number(e.target.value) })}
                        className="h-10"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-muted-foreground ml-1">Цена ({currencySymbol})</label>
                      <Input type="number" value={item.price || 0} onChange={(e) => onUpdateItem(item.id, { price: Number(e.target.value) })}
                        className="h-10"
                      />
                    </div>
                  </div>
