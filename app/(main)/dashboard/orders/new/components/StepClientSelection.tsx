  ...
  return (
    <div className="max-w-2xl space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="relative">
        ...
        <Input type="search" placeholder="Поиск по имени, email или телефону..." value={searchQuery} onChange={(e) => onSearchChange(e.target.value)}
          className="pl-12"
          ...
          <div className="absolute top-full left-0 w-full mt-2 bg-card border border-border rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
            ...
            <div className="p-2">
              {searchHistory.map((h, i) => (
                <Button key={i} type="button" variant="ghost" onClick={() => onSearchChange(h)}
      ...
      {(searchResults || []).length > 0 && !selectedClient && (
        <div className="border border-border rounded-2xl shadow-xl bg-card overflow-hidden">
          {(searchResults || []).map((client) => (
            <Button key={client.id} type="button" variant="ghost" onClick={() => onSelectClient(client)}
