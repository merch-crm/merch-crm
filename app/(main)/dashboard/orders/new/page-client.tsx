  ...
  return (
    <div className="flex-1 flex flex-col md:h-[calc(100vh-130px)] overflow-hidden bg-muted/50">
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0 gap-3 px-4 sm:px-8 pb-8 pt-4 overflow-y-auto lg:overflow-hidden">
        {/* Sidebar */}
        <OrderSidebar steps={steps} currentStep={uiState.step} onStepClick={(sid) => {
        ...
        <div className="flex-1 overflow-visible lg:overflow-hidden h-full flex flex-col gap-3">
          <div className="crm-card flex-1 min-w-0 flex flex-col h-full min-h-[400px]" data-testid="order-form">
            <div className="flex-1 overflow-y-auto p-6 md:p-6">
              {uiState.step === 0 && (
                <StepClientSelection searchQuery={searchState.query} onSearchChange={(q) => setSearchState(prev => ({ ...prev, query: q }))}
              ...
              {uiState.step === 1 && (
                <StepItemSelection inventory={inventory} selectedItems={orderData.selectedItems} onAddItem={addItem} onRemoveItem={removeItem} onUpdateItem={updateItem} currencySymbol={currencySymbol} />
              )}

              {uiState.step === 2 && (
                <StepOrderDetails details={orderData.details} onUpdateDetails={(updates) => setOrderData(prev => ({
                    ...prev,
              ...
              {uiState.step === 3 && (
                <StepConfirmation selectedClient={orderData.selectedClient} selectedItems={orderData.selectedItems} details={orderData.details} currencySymbol={currencySymbol} />
              )}
            </div>
