...
export function OrdersDashboardClient({
    ...
    const salesChartData = salesData.map((d) => d.sales);
        ...
        <div className="space-y-3">
            ...
            {stats && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <StatCard
                        ...
                        chart={<MiniChart data={salesChartData} type="bar" color="rgb(59, 130, 246)" height={32} />}
                    />
                    <StatCard
                        title="В работе"
                        value={stats.inWork}
                        ...
                        variant={stats.inWork > 10 ? "warning" : "default"}
                    />
                    <StatCard
                        title="Отправлено"
                        value={stats.shipped}
            ...
            {stats && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <StatCard
                        ...
                        icon={TrendingUp}
                        iconColor="text-blue-600"
                        chart={<MiniChart data={salesChartData} type="line" color="rgb(34, 197, 94)" height={32} />}
                    />
                    <StatCard
                        title="Средний чек"
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <QuickActionCard
                        title="Создать заказ"
                        description="Новый заказ клиента"
                        icon={Plus}
                    ...
                    <QuickActionCard
                        title="Все заказы"
                        description={`${stats?.total || 0} заказов`}
                        icon={ShoppingCart}
                    ...
                    <QuickActionCard
                        title="В работе"
                        description={`${stats?.inWork || 0} заказов`}
                        icon={Clock}
                        href="/dashboard/orders/list?status=in_work"
                        iconColor="text-purple-600"
                        iconBgColor="bg-purple-100"
                    />
                    <QuickActionCard
                        title="Отправлено"
                        description={`${stats?.shipped || 0} заказов`}
                        icon={Package}
                        href="/dashboard/orders/list?status=shipped"
                        iconColor="text-blue-600"
                        iconBgColor="bg-blue-100"
                    />
                </div>
                ...
                <Card>
                    <CardHeader className="pb-3">
                        <SectionHeader
                            ...
                            href="/dashboard/orders/list?filter=action_required"
                            className="mb-0"
                        />
                    </CardHeader>
                    <CardContent>
                        {ordersRequiringAction.length === 0 ? (
                            <EmptyWidget
                                icon={AlertTriangle}
                                title="Всё в порядке"
                                description="Нет заказов требующих внимания"
                            />
                        ) : (
                            <div className="space-y-3">
                                {ordersRequiringAction.slice(0, 5).map((order) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            >
                                                <AlertTriangle
                                                    className={`h-4 w-4 ${order.reason === "Просрочен"
                                                        ? "text-red-600"
                                                        : "text-yellow-600"
                                                        }`}
                                                />
