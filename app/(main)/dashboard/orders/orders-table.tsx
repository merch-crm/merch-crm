...
export function OrdersTable({ orders, error, isAdmin, showFinancials, showArchived }: OrdersTableProps) {
  ...
  return (
    <div className="space-y-3" data-testid="orders-list">
      <ResponsiveDataView data={orders} mobileGridClassName="flex flex-col divide-y divide-slate-100 md:hidden" desktopClassName="hidden md:block" emptyState={ <GlassEmptyState icon={PackageOpen} title="Заказов пока нет" description="Как только появятся новые заказы, они отобразятся здесь" />
        }
        renderTable={() => (
          <div className="crm-card !p-0 overflow-hidden">
            <table className="crm-table min-w-full table-fixed">
              <thead className="crm-thead">
                <tr>
                  <th className="crm-th crm-td-selection">
                    <Checkbox id="select-all-orders" aria-label="Выбрать все заказы" checked={isAllSelected} onChange={(val) => toggleAll(!!val)}
                    />
                  </th>
              ...
              <tbody className="crm-tbody">
                {orders?.map((order) => (
                  <OrderTableRow key={order.id} order={order} config={{ isSelected: selectedIds.includes(order.id), isAdmin: isAdmin, showFinancials: !!showFinancials, showArchived: !!showArchived, currencySymbol: currencySymbol || "₽", }} actions={{ onSelect: handleSelectRow, onUpdateField: handleUpdateField, onToggleArchived: onToggleArchivedAction, onNavigate: onNavigate, }} />
                ))}
              </tbody>
            </table>
          </div>
        ...
        renderCard={(order) => (
          <OrderCard key={order.id} order={order} isSelected={selectedIds.includes(order.id)} showFinancials={!!showFinancials} currencySymbol={currencySymbol || "₽"} onSelect={handleSelectRow} onNavigate={onNavigate} />
        )}
      />

      <BulkActionsPanel selectedIds={selectedIds} onClear={() => setSelectedIds([])}
        isAdmin={isAdmin}
