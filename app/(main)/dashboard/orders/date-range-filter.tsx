    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 sm:flex sm:items-center gap-1.5 p-1 bg-slate-100/50 rounded-[22px] sm:rounded-2xl border border-slate-200/50 shadow-sm">
        {ranges.slice(0, 4).map((range) => (
          <Button key={range.value} variant="ghost" asChild onClick={() => handleRangeChange(range.value)}
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
          <PopoverTrigger asChild>
            <Button variant="ghost" asChild className={cn("flex items-center justify-center gap-2 px-4 py-2 text-[12px] sm:text-sm font-bold rounded-[14px] transition-all whitespace-nowrap w-full sm:w-auto col-span-2 sm:col-span-1 h-auto hover:bg-white/50", isCustom ?"bg-white text-primary shadow-sm hover:bg-white" :"text-slate-500 hover:text-slate-700 hover:bg-white/50" )}>
              <button type="button">
          ...
          <AnimatePresence>
            {isPopoverOpen && (
              <PopoverContent className="p-0 bg-transparent border-none shadow-none w-auto overflow-visible duration-0 data-[state=open]:animate-none data-[state=closed]:animate-none" align="start" sideOffset={8} forceMount asChild>
                <motion.div
                ...
                >
                  <div className="p-4 bg-white rounded-[22px] ring-1 ring-slate-900/10 border border-slate-200 overflow-hidden">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200 flex items-center justify-between gap-3">
                      ...
                      <Button size="sm" onClick={handleApplyCustomRange} disabled={!dateRange?.from} className="bg-primary hover:bg-primary/90 text-white rounded-2xl font-bold shadow-lg shadow-primary/20 px-6">
                        Выбрать
                      </Button>
                    </div>
                  </div>
        ...
        {isCustom && fromParam && toParam && (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-[14px] shadow-sm border border-slate-200/50 animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <Button variant="ghost" size="icon" onClick={() => handleRangeChange("all")}
