    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between crm-card sm: gap-3">
        ...
        <div className="flex items-center gap-3 sm:gap-3 shrink-0 ml-auto sm:ml-0">
          <OrderActions orderId={order.id} isArchived={order.isArchived} canDelete={canDelete} canArchive={canArchive} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="lg:col-span-2 space-y-3">
          <OrderItemsTable items={order.items || []} currencySymbol={currencySymbol} showFinancials={showFinancials} totalAmount={order.totalAmount ?? 0} discountAmount={order.discountAmount} />
          ...
          {showFinancials && (
            <div className="crm-card">
              ...
              <div className="space-y-3">
                ...
                <div className="pt-2">
                  <AddPaymentDialog orderId={order.id} remainingAmount={Number(order.totalAmount) - ((order.payments)?.reduce((acc: number, p) => acc + Number(p.amount), 0) || 0)}
                  />
                </div>
                ...
                <div className="pt-6">
                  <RefundDialog orderId={order.id} maxAmount={(order.payments)?.reduce((acc: number, p) => acc + Number(p.amount), 0) || 0}
                  />
                </div>
              </div>
            </div>
              ...
              <div className="flex justify-between items-center pt-5 border-t border-slate-800">
                ...
                <div className="flex items-center font-bold">
                  <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center text-xs mr-2 overflow-hidden">
                    {order.creator?.image ? (
                      <NextImage src={order.creator.image} alt={order.creator.name || 'Avatar'} width={24} height={24} className="w-full h-full object-cover" />
                    ) : (
