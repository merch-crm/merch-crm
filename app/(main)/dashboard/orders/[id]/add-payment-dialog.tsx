      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col h-full space-y-3 p-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <Button key={m.id} type="button" variant="ghost" onClick={() => setMethod(m.id)}
          ...
          <div role="button" tabIndex={0} className="flex items-center gap-3 bg-slate-50 p-3 rounded-[14px] border border-slate-100 cursor-pointer" onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.currentTarget.click(); } }} onClick={() => setIsAdvance(!isAdvance)}>
            <Checkbox checked={isAdvance} onChange={setIsAdvance} />
          ...
          <div className="pt-4 mt-auto sticky bottom-0 bg-white border-t border-slate-100">
            <Button type="submit" disabled={loading} className="w-full py-6 rounded-[14px] bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm shadow-xl shadow-emerald-200 transition-all active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100 flex items-center justify-center gap-2">
