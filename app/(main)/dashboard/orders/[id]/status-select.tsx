...
export default function StatusSelect({ orderId, currentStatus }: { orderId: string, currentStatus: string }) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      {/* Trigger Button */}
      <Button variant="ghost" onClick={() => !loading && setIsOpen(!isOpen)}
        disabled={loading}
        className={`
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-200 rounded-2xl shadow-crm-xl z-50 py-2 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
          ...
          {statuses.map((s) => (
            <Button key={s.id} variant="ghost" onClick={() => handleStatusChange(s.id)}
              className={`
      ...
      <ResponsiveModal isOpen={showCancelDialog} onClose={() => setShowCancelDialog(false)}
        title="Отмена заказа"
