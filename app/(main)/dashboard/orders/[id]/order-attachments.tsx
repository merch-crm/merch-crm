...
export default function OrderAttachments({ orderId, attachments = [] }: OrderAttachmentsProps) {
    ...
    const getFileIcon = (contentType: string) => {
        if (contentType?.startsWith("image/")) return <ImageIcon className="w-5 h-5 text-blue-500" aria-label="Image file" />;
        ...
        <div className="crm-card ">
            <div className="flex items-center justify-between mb-6">
                ...
                <Button
                    ...
                    disabled={isPending}
                    className="flex items-center gap-2 px-4 py-2 bg-primary/5 text-primary rounded-2xl text-xs font-bold hover:bg-primary/10 transition-all disabled:opacity-50 h-auto"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {attachments.length === 0 && (
                    ...
                    >
                        ...
                        <p className="text-xs font-bold  mb-1 text-slate-500">Нет прикрепленных файлов</p>
                        <p className="text-xs font-medium text-slate-400">Нажмите, чтобы добавить макет или ТЗ</p>
                    </div>
                )}
            </div>
        </div>
