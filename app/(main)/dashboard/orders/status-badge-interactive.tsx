    ...
    <div role="button" tabIndex={0} className="relative inline-block" ref={containerRef} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.currentTarget.click(); } }} onClick={(e) => e.stopPropagation()}>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {statuses.map((s) => (
              <Button key={s.id} variant="ghost" onClick={() => handleChange(s.id)}
                className={cn("w-full flex items-center gap-2.5 px-3 py-2.5 rounded-[16px] transition-all text-left group h-auto justify-start",
