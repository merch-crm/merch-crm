              ...
              <Tabs defaultValue="preview">
                ...
                {["preview", "source", "mockup", "client_file"].map((type) => (
                  <TabsContent key={type} value={type} className="mt-0">
                    <FileUploadZone taskId={task.id} type={type as "preview" | "source" | "mockup" | "client_file"} files={filesByType[type] || []} onUpdate={(file: DesignFile) => {
                        setTask((prev) => ({
                          ...prev,
        ...
        <div className="space-y-3">
            ...
            <CardContent className="space-y-3">
              {task.order && (
                <div>
                  ...
                  <Link href={`/dashboard/orders/${task.order.id}`} className="font-medium hover:underline flex items-center gap-2 text-sm">
          ...
          {task.sourceDesign && (
            <Card>
              ...
              <CardContent>
                <Link href={`/dashboard/design/prints/${task.sourceDesign.collectionId}/${task.sourceDesign.id}`} className="block hover:bg-muted/50 rounded-lg p-3 -m-1 transition-colors border">
                  {task.sourceDesign.preview && (
                    <div className="relative w-full h-32 mb-3">
                      <Image src={task.sourceDesign.preview} alt={task.sourceDesign.name} fill className="object-contain bg-white rounded border" unoptimized />
                    </div>
                  )}
