        ...
        <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                    <Button variant="outline" className="rounded-xl h-10 px-5 border-gray-200 hover:bg-gray-50 font-medium text-gray-700 transition-colors" asChild>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <QuickActionCard
                        ...
                        description="Создать новый макет"
                        icon={PenTool}
                        href="/dashboard/design/editor/new"
                        iconColor="text-purple-600"
                        iconBgColor="bg-purple-100"
                    />
                    <QuickActionCard
                        title="Очередь задач"
                        description={`${stats?.inQueue || 0} в очереди`}
                        icon={ListTodo}
                    ...
                    <QuickActionCard
                        title="Коллекции"
                        description={`${stats?.totalCollections || 0} коллекций`}
                        icon={FolderOpen}
                    ...
                    <QuickActionCard
                        ...
                        description="Новая дизайн-задача"
                        icon={Plus}
                        href="/dashboard/design/queue/new"
                            ...
                            <div className="space-y-2 mt-4">
                                {myTasks.slice(0, 5).map((task) => {
                                    ...
                                    return (
                                        <Link
                                            key={task.id}
                                            href={`/dashboard/design/queue/${task.id}`}
                                            className="flex items-center justify-between p-3.5 rounded-2xl border border-transparent hover:bg-gray-50 hover:border-gray-100 transition-all duration-300 group"
                                        >
                ...
                <Card>
                    <CardHeader className="p-6 pb-2">
                        <SectionHeader
                            ...
                            href="/dashboard/design/queue?filter=urgent"
                            className="mb-0"
                        />
                    </CardHeader>
                    <CardContent className="p-6 pt-2">
                            ...
                            <div className="space-y-2 mt-4">
                                {urgentTasks.map((task) => (
                                    <Link
                                        key={task.id}
                                        href={`/dashboard/design/queue/${task.id}`}
                                        className="flex items-center justify-between p-3.5 rounded-2xl border border-red-50 bg-red-50/30 hover:bg-red-50/80 hover:border-red-100 transition-all duration-300 group"
                                    >
                    ...
                    <CardContent className="p-6 pt-4">
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                            {popularDesigns.map((design) => (
                                <Link
                                    key={design.id}
                                    href={`/dashboard/design/prints/${design.id}`}
            ...
            {applicationTypesStats.length > 0 && (
                <div className="mt-4">
                    <ApplicationTypesChart
                        data={applicationTypesStats}
                        title="Задачи по типам нанесения"
                    />
                </div>
            )}
        </div>
