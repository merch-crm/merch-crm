  ...
  return (
    <nav className="flex flex-wrap gap-2">
      {designNavItems.map((item) => {
        ...
        return (
          <Link key={item.href} href={item.href} className={cn( "flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors", isActive ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80 text-muted-foreground hover:text-foreground" )}>
      ...
      <Link href="/dashboard/design/prints?create=true" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-primary/10 text-primary hover:bg-primary/20 transition-colors ml-auto">
