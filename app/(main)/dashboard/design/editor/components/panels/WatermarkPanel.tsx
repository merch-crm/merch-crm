      ...
      <div className="p-4 space-y-3">
        ...
        {config.enabled && (
          <>
            ...
            {/* Type */}
            <Tabs value={config.type} onValueChange={(type) => updateConfig({ type: type as "text" | "image" })}
            >
              ...
              <TabsContent value="text" className="space-y-3 mt-4">
                <div className="space-y-2">
                  <Label>Текст</Label>
                  <Input value={config.text || ""} onChange={(e) => updateConfig({ text: e.target.value })}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Slider value={[config.fontSize || 48]} min={12} max={120} step={1} onValueChange={([value]) => updateConfig({ fontSize: value })}
                      className="flex-1"
                    />
                ...
                <div className="space-y-2">
                  <Label>Цвет</Label>
                  <div className="flex items-center gap-2">
                    <Input type="color" value={config.color || "#000000"} onChange={(e) => updateConfig({ color: e.target.value })}
                      className="w-12 h-10 p-1 cursor-pointer"
                    />
                    <Input type="text" value={config.color || "#000000"} onChange={(e) => updateConfig({ color: e.target.value })}
                      className="flex-1"
                    />
                  </div>
                </div>
              ...
              <TabsContent value="image" className="space-y-3 mt-4">
                <div className="space-y-2">
                  ...
                  {config.imagePath ? (
                    ...
                    <div className="border-2 border-dashed rounded-lg p-4 text-center">
                      <Input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" id="watermark-image" />
                      <label
                ...
                <div className="space-y-2">
                  <Label>Масштаб</Label>
                  <div className="flex items-center gap-2">
                    <Slider value={[(config.scale || 1) * 100]} min={10} max={200} step={5} onValueChange={([value]) => updateConfig({ scale: value / 100 })}
                      className="flex-1"
                    />
            ...
            <div className="space-y-2">
              ...
              <Select value={config.position} onChange={(position) => updateConfig({ position: position as WatermarkPosition })}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Slider value={[config.opacity * 100]} min={5} max={100} step={5} onValueChange={([value]) => updateConfig({ opacity: value / 100 })}
                  className="flex-1"
                />
            ...
            <div className="space-y-2">
              <Label>Поворот</Label>
              <div className="flex items-center gap-2">
                <Slider value={[config.rotation || -30]} min={-180} max={180} step={5} onValueChange={([value]) => updateConfig({ rotation: value })}
                  className="flex-1"
                />
