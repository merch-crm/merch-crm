...
export function BackgroundRemover({ isPending, processingType, onProcess }: BackgroundRemoverProps) {
  ...
  return (
    <Button variant="outline" onClick={onProcess} disabled={isPending} className={cn( "h-20 rounded-[var(--radius-outer)] gap-3 border-slate-100 transition-all group hover:border-primary/20 hover:bg-slate-50/50 bg-white shadow-sm", isActive && "bg-primary text-white border-primary shadow-xl shadow-primary/20" )}>
      <div className={cn(
