...
export function Upscaler({ isPending, processingType, onProcess }: UpscalerProps) {
    ...
    return (
        <Button 
            ...
            disabled={isPending}
            className={cn(
                "h-20 rounded-[var(--radius-outer)] gap-3 border-slate-100 transition-all group hover:border-violet-200 hover:bg-slate-50/50 bg-white shadow-sm",
                isActive && "bg-violet-600 text-white border-violet-600 shadow-xl shadow-violet-100"
            )}
        >
            <div className={cn(
