...
export function CollectionPageClient({
  ...
  return (
    <div className="space-y-3">
      {/* Back button */}
      <Button variant="ghost" size="sm" className="mb-4" onClick={() => router.push("/dashboard/design/prints")}
      >
      ...
      {/* Header */}
      <CollectionHeader collection={collection} onEdit={() => setIsEditDialogOpen(true)}
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mt-8 mb-6">
        <div className="relative w-full sm:w-80">
          ...
          <Input placeholder="Поиск принтов..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
            className="pl-10"
          />
        </div>
      ...
      {filteredDesigns.length === 0 ? (
        searchQuery ? (
          ...
          <DesignsEmptyState onCreateClick={() => setIsCreateDialogOpen(true)}
          />
        )
      ) : (
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={filteredDesigns.map((d) => d.id)}
            strategy={rectSortingStrategy}
            disabled={!isSortMode}
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {filteredDesigns.map((design) => (
                <DesignCard key={design.id} design={design} isSortMode={isSortMode} onClick={() => handleDesignClick(design.id)}
                />
              ))}
            </div>
          </SortableContext>
      ...
      <DesignFormDialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen} collectionId={collection.id} onSuccess={handleDesignCreated} />

      <CollectionFormDialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} collection={collection as never} onSuccess={handleCollectionUpdated} />
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            {canDelete && (
              <AlertDialogAction onClick={handleDeleteCollection} disabled={isDeleting} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
