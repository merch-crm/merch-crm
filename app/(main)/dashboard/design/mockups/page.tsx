      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 h-full md:h-[calc(100vh-160px)] min-h-[700px]">
        ...
        <div className="md:col-span-8 flex flex-col gap-3 h-full">
          ...
          <Card className="flex-1 p-0 overflow-hidden relative rounded-2xl border border-slate-200/60 shadow-sm bg-slate-50 flex flex-col min-h-[400px] h-full group">
            <ThreeViewer modelUrl={modelConfig.modelUrl} itemColor={modelConfig.color} logoUrl={modelConfig.logo} placementZone={modelConfig.zone} decalScale={modelConfig.scale} decalOffset={modelConfig.offset} decalRotation={modelConfig.rotation} backgroundColor={modelConfig.bgColor} />
          </Card>
          ...
          <Card className="p-4 border border-slate-200/60 shadow-sm rounded-2xl bg-white/80 backdrop-blur-xl shrink-0 w-full">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button variant="ghost" size="sm" className="h-6 text-xs font-semibold px-2 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg" onClick={() => fileInputRef.current?.click()}
              >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[160px] overflow-y-auto pr-1 minimal-scrollbar">
                    {PRESET_MODELS.filter(m => m.category === cat).map((m) => (
                      ...
                      >
                        ...
                        <Button type="button" variant="ghost" size="icon" className="h-6 w-6 shrink-0 rounded-md opacity-0 group-hover:opacity-100 hover:bg-white text-slate-400 hover:text-slate-600 transition-all shadow-sm" onClick={(e) => { e.stopPropagation(); downloadModel(m.url, m.id); }}
                        >
