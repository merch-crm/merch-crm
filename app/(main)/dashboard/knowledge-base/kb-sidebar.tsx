...
export const KBSidebar = memo(({
  ...
  function renderFolder(folder: KBFolder, depth = 0) {
    ...
    return (
      <FolderItem key={folder.id} folder={folder} depth={depth} isExpanded={isExpanded} onToggle={toggleFolder} onAddPage={onAddPage} canEdit={canEdit}>
        <div className="space-y-1">
          {childFolders.map(f => renderFolder(f, depth + 1))}
          {folderPages.map(page => (
            <PageItem key={page.id} page={page} isActive={activePageId === page.id} onSelect={onSelectPage} />
          ))}
        </div>
      </FolderItem>
    );
    ...
    >
      ...
      {canEdit && (
        <div className="mt-auto px-2">
          <Button type="button" className="w-full h-11 rounded-[12px] bg-primary hover:bg-primary/90 text-white gap-2 font-bold shadow-lg shadow-primary/20" onClick={() => onAddPage(null)}
          >
