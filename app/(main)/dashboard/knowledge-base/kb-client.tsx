...
import { getKBPageDetail, createKBFolder, createKBPage, updateKBPage, deleteKBPage } from"./actions";
...
export function KBClient({ initialFolders = [], initialPages = [], userRole }: KBClientProps) {
  ...
  const fetchPageDetail = useCallback(async (id: string) => {
    ...
    try {
      const res = await getKBPageDetail(id);
      if (res.success && res.data) {
        const data = res.data;
        setPageState(prev => ({
          ...prev,
          content: data as KBPage,
  ...
  const handleSave = useCallback(async () => {
    ...
    setPageState(prev => ({ ...prev, isLoading: true }));
    try {
      const res = await updateKBPage(uiState.selectedPageId, pageState.editData);
      if (res.success) {
  ...
  const confirmCreateFolder = async (name: string) => {
    ...
    const res = await createKBFolder(name, dialogs.folder.parentId);
    if (res.success) {
      router.refresh();
    }
  };
  ...
  const confirmCreatePage = async (title: string) => {
    ...
    const res = await createKBPage({ title, content:"", folderId: dialogs.page.folderId });
    if (res.success && res.data) {
  ...
  const confirmDeletePage = async () => {
    ...
    try {
      const res = await deleteKBPage(uiState.selectedPageId);
      if (res.success) {
  ...
  return (
    <div className="crm-card flex h-[calc(100vh-140px)] !p-0 !border-none !shadow-crm-xl !bg-white/40">
      ...
      <div className="hidden md:flex shrink-0">
        <KBSidebar {...sidebarProps} />
      </div>

      {/* Mobile Sidebar (ResponsiveModal) */}
      <ResponsiveModal isOpen={uiState.isSidebarOpen} onClose={() => setUiState(prev => ({ ...prev, isSidebarOpen: false }))}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white/20">
        ...
        <div className="h-16 border-b border-slate-200/60 px-4 md:px-6 flex items-center justify-between bg-white/40">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <Button variant="ghost" size="icon" type="button" className="md:hidden shrink-0" onClick={() => setUiState(prev => ({ ...prev, isSidebarOpen: true }))}
          ...
          <div className="flex items-center gap-2 ml-2">
            {uiState.selectedPageId && (
              <>
                {pageState.isEditing ? (
                  <>
                    <Button variant="ghost" size="icon" type="button" onClick={() => setPageState(prev => ({ ...prev, isEditing: false }))}
                  ...
                  <>
                    <Button variant="ghost" size="icon" type="button" onClick={() => setPageState(prev => ({ ...prev, isEditing: true }))}
                      aria-label="Редактировать статью"
                    >
                      <Edit2 className="w-4 h-4 text-slate-400" />
                    </Button>
                    <Button variant="ghost" size="icon" type="button" onClick={() => setPageState(prev => ({ ...prev, isPreview: !prev.isPreview }))}
                    ...
                    <Button variant="ghost" size="icon" type="button" onClick={handleDeletePage} className="hover:text-rose-600 hover:bg-rose-50" aria-label="Удалить статью">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          {uiState.selectedPageId ? (
              ...
              {pageState.isEditing ? (
                <div className="flex flex-col h-full gap-3">
                  <div className="flex items-center justify-between gap-2 mb-2 p-2 bg-slate-50/50 rounded-xl border border-slate-200/60">
                    ...
                    <Button variant="ghost" size="sm" type="button" onClick={() => setPageState(prev => ({ ...prev, isPreview: !prev.isPreview }))}
                ...
                <div className="prose prose-premium prose-slate max-w-none animate-in fade-in duration-500">
                  {pageState.content?.content ? (
                    ...
                    <div className="text-center py-20 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
                      ...
                      {canEdit && (
                        <Button variant="link" type="button" onClick={() => setPageState(prev => ({ ...prev, isEditing: true }))}
            ...
            <div className="flex-1 h-full flex flex-col items-center justify-center text-center p-6 md:p-[--padding-xl]">
              ...
              <Button variant="outline" type="button" className="md:hidden mt-6 font-bold h-11 px-6 rounded-2xl" onClick={() => setUiState(prev => ({ ...prev, isSidebarOpen: true }))}
              >
      ...
      <PromptDialog isOpen={dialogs.folder.isOpen} onClose={() => setDialogs(prev => ({ ...prev, folder: { ...prev.folder, isOpen: false } }))}
      ...
      <PromptDialog isOpen={dialogs.page.isOpen} onClose={() => setDialogs(prev => ({ ...prev, page: { ...prev.page, isOpen: false } }))}
      ...
      <ConfirmDialog isOpen={dialogs.delete.isOpen} onClose={() => setDialogs(prev => ({ ...prev, delete: { isOpen: false } }))}
