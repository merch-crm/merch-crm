      ...
      <div className="mb-1">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            {item.subItems && (
              <ChevronDown size={16} className={`text-slate-400 transition-transform duration-200 ${item.isOpen ? 'rotate-180' : ''}`} />
            )}
          </div>
        </div>
        ...
        */}
        ...
        <div className="w-[280px] h-[700px] bg-white rounded-3xl shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)] p-5 flex flex-col relative overflow-visible">
          ...
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar">
            {items.map(item => (
              <SidebarItem key={item.id} item={item} isCollapsed={false} isActive={item.id === 'income'} onToggle={toggleItem} />
            ))}
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-slate-200">
            <div className="flex items-center gap-3 px-2 py-1 cursor-pointer hover:bg-slate-50 rounded-xl transition-colors">
              <Image src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop" alt="Profile" className="w-9 h-9 rounded-full object-cover" width={36} height={36} />
        ...
        <div className="w-[84px] h-[700px] bg-white rounded-3xl shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)] p-4 flex flex-col relative items-center">
          ...
          <div className="flex-1 w-full space-y-2">
            {items.map(item => (
              <SidebarItem key={item.id} item={item} isCollapsed={true} isActive={item.id === 'income'} onToggle={toggleItem} />
            ))}
          </div>
