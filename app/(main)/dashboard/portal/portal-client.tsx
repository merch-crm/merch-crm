...
export function PortalClient({ data }: PortalClientProps) {
  ...
  useEffect(() => {
    if (isClient) {
      setPortalDate(format(Date.now(), "d MMMM, EEEE", { locale: ru })); // audit-ok
    }
  }, [isClient]);
  ...
  if (!isStaff || !data.staff) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-8 bg-white/50 backdrop-blur-xl rounded-[40px] border border-white/60 shadow-2xl">
        ...
        <Link href="/dashboard">
          <Button type="button" color="black" className="mt-8 px-8 font-bold">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button 
            ...
            color="black"
            className="px-6 font-bold shadow-lg shadow-slate-900/10"
          >
          ...
          <Button 
            variant="outline"
            size="icon"
            className="p-3 rounded-2xl text-slate-600 hover:text-slate-900"
          >
                  ...
                  <div key={task.id} className="p-4 hover:bg-slate-50/50 transition-colors flex items-center gap-3 group">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <Button 
                        type="button"
                        size="icon"
                        className="h-8 w-8 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity shadow-md shadow-primary/20"
                      >
