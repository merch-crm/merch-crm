...
export function ItemAnalyticsSection({ history, currentQuantity, unit, lowStockThreshold, criticalStockThreshold = 0 }: ItemAnalyticsSectionProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-stretch">
      ...
      <ItemAnalyticsChart analytics={analytics} chartLines={chartLines} unit={unit} />

      {/* Vertical Stock Timeline Section - 3 columns on desktop */}
      <StockTimeline currentQuantity={currentQuantity} unit={unit} lowStockThreshold={lowStockThreshold} criticalStockThreshold={criticalStockThreshold} analytics={{ daysToLow: analytics.daysToLow, daysToCritical: analytics.daysToCritical }} />
    </div>
  );
}
