...
export function ItemDialogs() {
  ...
  return (
    <>
      <AdjustStockDialog item={item} locations={storageLocations} itemStocks={stocks} initialType={(adjustType as "in" | "out") || "in"} user={user} isOpen={!!adjustType} onClose={() => {
          setAdjustType(null);
          fetchData();
          router.refresh();
        }}
      ...
      <TransferItemDialog item={item} locations={storageLocations} itemStocks={stocks} isOpen={dialogs.transfer} onClose={() => {
      ...
      <ArchiveReasonDialog isOpen={dialogs.archiveReason} onClose={() => setDialogs(prev => ({ ...prev, archiveReason: false }))}
        onConfirm={handleArchive}
        isLoading={isSaving}
      />

      <ConfirmDialog isOpen={dialogs.deleteConfirm} onClose={() => setDialogs(prev => ({ ...prev, deleteConfirm: false }))}
      ...
      <ConfirmDialog isOpen={dialogs.duplicateConfirm.open} onClose={() => setDialogs(prev => ({ ...prev, duplicateConfirm: { ...prev.duplicateConfirm, open: false } }))}
        onConfirm={() => {
      ...
      <ConfirmDialog isOpen={dialogs.draftConfirm} onClose={() => {
      ...
      <ConfirmDialog isOpen={dialogs.unsavedChanges} onClose={() => setDialogs(prev => ({ ...prev, unsavedChanges: false }))}
        onConfirm={() => {
          pendingExitAction?.();
      ...
      <QRScanner isOpen={dialogs.scanner} onClose={() => setDialogs(prev => ({ ...prev, scanner: false }))}
        onResult={handleScan}
      />

      <LabelPrinterDialog item={item} isOpen={dialogs.label} onClose={() => setDialogs(prev => ({ ...prev, label: false }))}
