    ...
    <div className="crm-card animate-in fade-in slide-in-from-top-4 duration-500 col-span-2 md:col-span-1">
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="ghost" size="icon" onClick={resetThumbSettings} className="w-6 h-6 rounded-full bg-muted flex items-center justify-center text-muted-foreground hover:bg-muted/80 hover:text-foreground transition-colors p-0 border-none shadow-none" title="Сбросить">
        ...
        <div className="flex items-start gap-3">
          <div className="flex-1 grid grid-cols-2 gap-3">
            <AxisSlider label="По горизонтали" value={thumbSettings.x ?? 0} bound={maxBounds.x} onChange={(v) => updateThumb({ x: v })}
            />
            <AxisSlider label="По вертикали" value={thumbSettings.y ?? 0} bound={maxBounds.y} onChange={(v) => updateThumb({ y: v })}
            />
          </div>
