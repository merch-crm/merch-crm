...
export function ItemTabsSwitcher({ tabletTab, setTabletTab }: ItemTabsSwitcherProps) {
  return (
    <div className="flex xl:hidden col-span-2 bg-card rounded-3xl p-1 shadow-sm border border-border items-center justify-between gap-1 overflow-x-auto relative z-0">
      {TABS.map((tab) => {
        const isActive = tabletTab === tab.id;
        return (
          <Button key={tab.id} onClick={() => setTabletTab(tab.id)}
            variant="ghost"
