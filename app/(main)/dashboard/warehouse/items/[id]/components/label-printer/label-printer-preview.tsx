  ...
  return (
    <div className="flex-1 bg-[#929292] relative overflow-hidden flex flex-col min-w-0">
      ...
      <div className="absolute top-8 left-8 right-8 flex justify-end items-start z-50 pointer-events-none">
        <Button type="button" variant="ghost" size="icon" onClick={onClose} className="h-12 w-12 rounded-2xl bg-white border border-slate-200 hover:bg-rose-50 hover:border-rose-100 hover:text-rose-500 transition-all shadow-xl active:scale-95 group pointer-events-auto cursor-pointer">
        ...
        >
          ...
          <div className={cn("pt-1.5 border-t border-slate-200 flex gap-2 shrink-0",
            ...
            {(contentSettings.showBarcode || uiState.layoutStyle === 'minimal') && (
              <QRCodeSVG value={item.sku || item.id} size={Math.round(( uiState.layoutStyle === 'minimal' ? ( (dimensions.isLandscape ? widthNum : heightNum)>= 110 ? 160 :
