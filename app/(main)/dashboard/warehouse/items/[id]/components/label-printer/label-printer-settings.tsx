...
export function LabelPrinterSettings({
  ...
  return (
    <div className="w-[420px] bg-white border-r border-slate-200 flex flex-col h-full z-20 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)] shrink-0 isolate">
      {/* 1. Header - FIXED */}
      <div className="flex-none px-6 py-5 border-b border-slate-200 bg-white z-10 shrink-0">
      ...
      {/* 2. Top Fixed Settings Area (Size, Style) - FIXED */}
        ...
        <div className="space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {['58x40', '58x60', '75x120', 'a4', 'custom'].map((size) => (
              <Button type="button" key={size} onClick={() => setDimensions(prev => ({ ...prev, paperSize: size as PaperSize }))}
                variant="ghost"
          ...
          {dimensions.paperSize === 'custom' && (
            <div className="grid grid-cols-2 gap-2 animate-in slide-in-from-top-2 fade-in duration-300 mt-2">
              <div className="relative">
                <Input type="number" value={dimensions.customWidth} onChange={(e) => setDimensions(prev => ({ ...prev, customWidth: Number(e.target.value) }))}
              ...
              <div className="relative">
                <Input type="number" value={dimensions.customHeight} onChange={(e) => setDimensions(prev => ({ ...prev, customHeight: Number(e.target.value) }))}
        ...
        <div className="space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <Button type="button" key={style.id} onClick={() => setUiState(prev => ({ ...prev, layoutStyle: style.id as LayoutStyle }))}
                variant="ghost"
      ...
      <div className={cn("flex-1 px-6 py-4 overflow-y-auto custom-scrollbar relative min-h-[100px]", uiState.layoutStyle === 'minimal' && "opacity-40 pointer-events-none")}>
        ...
        <div className="grid grid-cols-2 gap-x-1 gap-y-0.5">
          ...
          <ToggleItem label="Состав" checked={contentSettings.showComposition} onChange={(v) => setContentSettings(prev => ({ ...prev, showComposition: v }))} compact />
          ...
          {resolvedParams
            ...
            .map(p => (
              <ToggleItem key={p.slug} label={p.label} checked={extraAttributesToggles[p.slug] ?? true} onChange={(val) => setExtraAttributesToggles(prev => ({ ...prev, [p.slug]: val }))}
                compact
              />
            ))}
        ...
        {/* Inner faded edge for scrolling esthetics, optional */}
        <div className="sticky bottom-0 h-4 bg-gradient-to-t from-white to-transparent pointer-events-none -mx-6" />
      </div>

      {/* 4. Bottom Fixed Settings Area (Text, Alignment) - FIXED */}
      <div className="flex-none px-6 pt-4 pb-6 space-y-3 bg-white border-t border-slate-100 shrink-0 z-10 w-full">
        ...
        <div className={cn("space-y-2.5 transition-opacity", uiState.layoutStyle === 'minimal' && "opacity-40 pointer-events-none")}>
          ...
          <Input type="text" value={contentSettings.customText} onChange={(e) => setContentSettings(prev => ({ ...prev, customText: e.target.value }))}
        ...
        <div className="space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button type="button" onClick={() => setDimensions(prev => ({ ...prev, isLandscape: !prev.isLandscape }))}
              variant="ghost"
            ...
            <div className="flex bg-slate-50 p-1 rounded-2xl border-2 border-transparent">
              <Button type="button" onClick={() => setUiState(prev => ({ ...prev, alignment: 'left' }))}
                variant="ghost"
              ...
              <Button type="button" onClick={() => setUiState(prev => ({ ...prev, alignment: 'center' }))}
                variant="ghost"
                className={cn("flex-1 py-1.5 flex items-center justify-center rounded-2xl transition-all h-auto",
      ...
      <div className="flex-none px-6 pt-5 pb-6 bg-white border-t border-slate-200 z-50 space-y-3 shadow-[0_-12px_30px_-15px_rgba(0,0,0,0.08)] shrink-0 w-full relative">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-2xl">
            ...
            <Input type="number" value={uiState.quantity} onChange={(e) => setUiState(prev => ({ ...prev, quantity: Math.max(1, parseInt(e.target.value) || 1) }))}
        ...
        <div className="flex gap-3">
          <Button type="button" variant="outline" className="h-12 w-16 rounded-2xl bg-white border-2 border-slate-200 text-slate-900 flex items-center justify-center hover:bg-slate-50 hover:border-slate-300 transition-all active:scale-95 group shadow-sm px-0" title="Скачать PDF">
            <Download className="w-5 h-5 text-slate-400 group-hover:text-primary transition-colors" />
          </Button>
          <Button type="button" onClick={onPrint} className="flex-1 h-12 bg-slate-900 hover:bg-black text-white rounded-2xl font-black text-xs flex items-center justify-center gap-2.5 shadow-xl hover:shadow-2xl transition-all active:scale-[0.98] group">
