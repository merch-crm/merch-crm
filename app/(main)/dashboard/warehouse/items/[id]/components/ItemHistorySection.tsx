...
export function ItemHistorySection({ history }: ItemHistorySectionProps) {
  ...
  const renderTableView = () => (
    <>
      ...
      <div className="hidden md:flex flex-col gap-2">
        ...
        <div className="flex items-center px-5 py-3 mb-1 bg-slate-50/50 rounded-2xl border border-slate-100/50 text-xs font-bold text-slate-400 select-none">
        ...
        <div className="space-y-2">
          {
            paginatedHistory.map((tx, idx) => {
              ...
              return (
                <div key={tx.id || idx} className="bg-white px-5 py-4 rounded-2xl border border-slate-100/80 flex items-center transition-all duration-300 hover:shadow-sm hover:border-slate-200 cursor-default group">
                  {/* Icon + Type */}
                  ...
                  <div className="w-[100px] shrink-0 mr-16 flex justify-center">
                    <span className={cn("inline-flex items-center justify-center px-3 py-1 rounded-lg text-sm font-bold font-mono",
                      isPositive
                        ? "bg-emerald-50 text-emerald-600"
                        : "bg-rose-50 text-rose-600"
                    )}>
                  ...
                  {/* Warehouse */}
                  <div className="flex-1 flex items-center gap-2.5 text-sm text-muted-foreground font-semibold min-w-0 pr-4">
                  ...
                  {/* Reason */}
                  <div className="flex-1 text-sm text-muted-foreground/70 font-medium min-w-0 pr-4">
                  ...
                  {/* Author */}
                  <div className="w-[160px] shrink-0 text-sm text-muted-foreground font-semibold">
                  ...
                  <div className="w-[100px] shrink-0 flex flex-col items-end justify-center">
                    {(() => {
                      ...
                      return (
                        <>
                          <span className="text-xs font-bold text-foreground leading-tight">{format(d, "dd.MM.yy", { locale: ru })}</span>
                          <span className="text-xs font-bold text-muted-foreground leading-tight mt-0.5">{format(d, "HH:mm", { locale: ru })}</span>
                        </>
                      );
                    })()}
                  </div>
  ...
  return (
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      {/* Header / Controls */}
      <div className="bg-slate-50/50 rounded-2xl p-1.5 border border-slate-100 flex flex-col sm:flex-row md:items-center gap-2">
        {/* Search Input Box */}
        <div className="relative flex-1">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 z-10" />
          <Input type="text" placeholder="Поиск по истории..." value={searchQuery} onChange={(e) => {
              setSearchQuery(e.target.value);
              setCurrentPage(1);
            }}
            className="w-full pl-10 pr-10 bg-white border border-slate-200/60 rounded-xl focus:outline-none h-11 text-sm shadow-sm transition-colors hover:border-slate-300 focus:border-blue-400"
          />
          {searchQuery && (
      ...
      {filteredHistory.length > 0 && (
        <div className="pt-2">
          <Pagination currentPage={currentPage} totalItems={filteredHistory.length} pageSize={pageSize} onPageChange={setCurrentPage} itemNames={['операция', 'операции', 'операций']} variant="card" />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="md:hidden flex flex-col gap-2.5">
      {history.map((tx, idx) => {
        ...
        return (
          <div key={tx.id || idx} className="bg-white rounded-2xl border border-slate-100/80 shadow-sm overflow-hidden flex flex-col transition-all duration-300">
            {/* Main Row */}
            <div role="button" tabIndex={0}
              className="p-3.5 flex items-center gap-3.5 cursor-pointer hover:bg-slate-50 transition-colors"
                  ...
                  <div className="px-3 pb-3 pt-0">
                    <div className="bg-muted/30 rounded-lg p-3 border border-border flex flex-col gap-2">
                      ...
                      <div className="flex items-center justify-between border-b border-border/60 pb-2">
                        ...
                        <span className="text-xs font-bold text-foreground">{(tx.creator as { name?: string } | undefined)?.name || "Система"}</span>
                      </div>
