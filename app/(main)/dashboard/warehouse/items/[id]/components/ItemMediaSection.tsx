      ...
      <label className={cn(
        ...
        {isUploading ? (
          <div className="relative flex flex-col items-center">
            ...
            <Button variant="outline" size="sm" className="h-7 px-3 rounded-full text-xs font-bold border-slate-200 hover:bg-white active:scale-95" onClick={(e) => { e.preventDefault(); e.stopPropagation(); cancelUpload(globalIndex); }}
            >
          ...
          >
            {mainImg && (mainImg.src || isEditing) && (
              ...
              >
                {mainImg.src ? (
                  <>
                    <Image src={mainImg.src} alt={mainImg.label} fill priority={true} className="object-cover transition-transform duration-1000 ease-out group-hover/item:scale-105" unoptimized />
            ...
            {secondaryImgs.map((img, idx) => {
              ...
              return (
                ...
                >
                  {img.src ? (
                    <>
                      <Image src={img.src} alt={img.label} fill className="object-cover transition-transform duration-700 ease-out group-hover/item:scale-110" unoptimized />
                  ...
                  {isUploading && (
                    <div className="absolute inset-0 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center p-4">
                      ...
                      <Button variant="ghost" size="sm" className="h-7 text-xs font-bold text-slate-500 hover:text-red-600" onClick={(e) => { e.preventDefault(); e.stopPropagation(); cancelUpload(globalIndex); }}
                      >
                        Отмена
                      </Button>
                    </div>
