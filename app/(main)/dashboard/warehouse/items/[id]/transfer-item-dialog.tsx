    ...
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title="Перемещение товара" showVisualTitle={false} className="sm:max-w-[640px]">
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="sticky bottom-0 z-10 p-5 sm:p-6 pt-3 bg-white/95 backdrop-blur-md border-t border-slate-100 mt-auto flex items-center justify-end lg:justify-between gap-3 shrink-0">
          <Button type="button" variant="ghost" onClick={onClose} className="flex h-11 flex-1 lg:flex-none lg:px-8 text-slate-400 hover:text-slate-600 font-bold text-sm active:scale-95 transition-all text-center rounded-2xl items-center justify-center">
            Отмена
          </Button>
          <SubmitButton form="transfer-item-form" isLoading={isSubmitting} text="Переместить" loadingText="Перемещение..." variant="action" disabled={isSubmitting || !fromLocationId || !toLocationId || !reason.trim() || amount <= 0} className="h-11 flex-1 lg:flex-none lg:w-auto lg:px-10 rounded-2xl font-bold text-sm shadow-sm transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 border-none" />
        </div>
      </div>
    </ResponsiveModal>
  );
