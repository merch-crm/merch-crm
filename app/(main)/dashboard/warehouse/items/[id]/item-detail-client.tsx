...
function ItemDetailContent() {
  ...
  return (
    <>
      <ResponsiveDetailLayout isArchived={item.isArchived ?? false} hideLeftSidebarOnMobile={tabletTab !== 'characteristic'} header={ <ItemHeader item={item} isEditing={isEditing} isSaving={isSaving} isAnyUploading={isAnyUploading} editName={editData.name || ""} onEditNameChange={(name) => setEditData(prev => ({ ...prev, name }))}
            onCancel={() => {
              if (hasChanges) {
                handleCancelEdit();
              } else {
      ...
      <ItemEditingBar isEditing={isEditing} isMounted={isMounted} hasChanges={hasChanges} editData={editData} item={item} isSaving={isSaving} onCancel={() => {
          if (hasChanges) {
...
function ItemGalleryWrapper() {
  ...
  return (
    <ItemGalleryOverlay isOpen={isMainImageZoomed} onClose={() => setIsMainImageZoomed(false)}
      itemName={item.name}
