...
export default async function ItemPage({ params }: PageParams) {
  ...
  return (
    <div>
      ...
      <ItemDetailClient item={item as InventoryItem} storageLocations={locations as StorageLocation[]} categories={categories as Category[]} attributeTypes={attributeTypes as AttributeType[]} allAttributes={attributes as InventoryAttribute[]} user={session} />
    </div>
  );
}
