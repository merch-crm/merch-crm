  ...
  return (
    <div className="flex flex-col min-h-0 h-full !overflow-visible">
          ...
          <div className="flex-1 flex flex-col min-h-0 !overflow-visible">
            <div className="flex-1 flex flex-col md:flex-row min-h-0 py-1 md:py-2 gap-3">
              ...
              <div className="w-full md:w-[400px] h-full flex flex-col min-h-0 shrink-0 space-y-1.5">
                ...
                <MainPhotoUploader category={category} preview={formData.imagePreview ?? null} uploading={loadingIndex === 0} progress={uploadStates.front?.progress ?? 0} zoom={thumbSettings.zoom ?? 1} x={thumbSettings.x ?? 0} y={thumbSettings.y ?? 0} baseScale={baseScale} aspectRatio={aspectRatio} containerRef={containerRef} onSetAspectRatio={setAspectRatio} onChange={handleMainImageChange} onRemove={removeMainImage} />

                <ImageThumbControls disabled={!formData.imagePreview} containerDims={containerDims} zoom={thumbSettings.zoom ?? 1} x={thumbSettings.x ?? 0} y={thumbSettings.y ?? 0} maxBounds={maxBounds} onUpdatePitch={updateThumb} onReset={resetThumbSettings} />
              </div>
              ...
              <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar pr-[4px]">
                <div className="space-y-3 flex-1 flex flex-col min-h-0 pt-0 pb-12">
                  ...
                  <AdditionalPhotos previews={formData.imageDetailsPreviews} loadingIndex={loadingIndex} progress={uploadStates.details?.progress ?? 0} onChange={handleDetailImageChange} onReplace={handleDetailImageReplace} onRemove={removeDetailImage} />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mt-auto mx-0 shrink-0 !overflow-visible">
        <StepFooter onBack={onBack} onNext={onNext} isNextDisabled={isProcessing} validationError={undefined} hint={undefined} />
      </div>
    </div>
  );
}
