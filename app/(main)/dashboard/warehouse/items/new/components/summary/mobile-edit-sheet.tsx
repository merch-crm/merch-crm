...
export function MobileEditSheet({
  ...
  return (
    <ResponsiveModal isOpen={isOpen} onClose={onClose} className="w-full sm:max-w-md p-0 overflow-hidden bg-white border-none shadow-2xl rounded-t-[32px] sm:rounded-[32px]" showVisualTitle={false}>
      <div className="flex flex-col overflow-hidden max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-20">
          ...
          <Button variant="ghost" size="icon" onClick={onClose} className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:text-slate-900 transition-colors active:scale-95 p-0">
            <X className="w-5 h-5" />
          </Button>
        </div>
        ...
        <div className="p-6 pt-4 border-t border-slate-100 bg-white/80 backdrop-blur-md sticky bottom-0 z-20">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button onClick={onSaveName} variant="solid" color="purple" className="h-11 w-full sm:flex-1 rounded-[var(--radius-inner)] text-sm font-bold gap-2 shadow-sm active:scale-95">
