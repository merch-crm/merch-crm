      ...
      <div className="space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3">
            <Slider value={[zoom || 1]} onValueChange={([val]) => onUpdatePitch({ zoom: val })}
              min={1}
              max={3}
              step={0.01}
              className="flex-1"
            />
            <Button onClick={onReset} size="icon" variant="ghost" className="w-5 h-5 rounded-full bg-slate-100/50 text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors shrink-0" title="Сбросить">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Slider disabled={maxBounds.x <= 0} value={[x || 0]} onValueChange={([val]) => onUpdatePitch({ x: val })}
          ...
          <div className="space-y-1.5">
            ...
            <Slider disabled={maxBounds.y <= 0} value={[y || 0]} onValueChange={([val]) => onUpdatePitch({ y: val })}
