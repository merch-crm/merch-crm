    ...
    <div className="flex flex-col h-full min-h-0 !overflow-visible">
      <div className="flex-1 flex flex-col min-h-0 p-8">
        ...
        <div className={cn("transition-all duration-700 ease-in-out min-h-0 flex flex-col !overflow-visible",
          ...
          <CategorySelector categories={topLevelCategories} onSelect={onCategorySelect} selectedCategoryId={selectedCategory?.id} isCompact={!!selectedCategory} rightElement={ validationError && ( <div className="flex items-center gap-2 text-rose-500 bg-rose-50 px-3 py-1.5 rounded-full border border-rose-100 animate-in fade-in slide-in-from-top-4 whitespace-nowrap z-[60]">
          ...
          <div className="flex flex-col animate-in fade-in slide-in-from-top-4 duration-500 pt-2 sm:pt-4 border-t border-slate-100">
            ...
            >
              {subCategories.length > 0 ? (
                <CategorySelector categories={subCategories} onSelect={onSubCategorySelect} variant="compact" hideTitle={true} selectedCategoryId={formData.subcategoryId} isCompact={false} />
              ) : (
      ...
      <div className="mt-auto shrink-0">
        <StepFooter onBack={onBack} onNext={handleNextClick} validationError={undefined} />
      </div>
    </div>
  );
}
