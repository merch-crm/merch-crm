    ...
    <div className="flex flex-col h-full min-h-0">
      <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar pr-1">
        <div className="space-y-3 sm:space-y-3 pb-2 p-4 sm:p-6 lg:p-8">
          ...
          <LivePreviewCard formData={formData} category={category} attributeTypes={attributeTypes} dynamicAttributes={dynamicAttributes} activeSubcategory={logic.activeSubcategory} accentColor={logic.accentColor} isMobile={isMobile} nameEdit={{ isEditing: logic.isEditingName, tempName: logic.tempName || "", onEditClick: () => logic.setIsEditingName(true),
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-3 items-stretch">
            ...
            <StorageCard formData={formData} storageLocations={storageLocations} />
          </div>
      ...
      <div className="mt-auto shrink-0">
        <StepFooter onBack={onBack} onNext={onSubmit} nextLabel="Создать позицию" isSubmitting={isSubmitting} validationError={validationError} />
      </div>

      {/* Mobile Name Edit Sheet */}
      <MobileEditSheet isOpen={logic.isEditingName && isMobile} tempName={logic.tempName || ""} onClose={() => logic.setIsEditingName(false)}
        onNameChange={logic.setTempName}
        onSaveName={logic.handleSaveName}
        onCancelName={logic.handleCancelName}
      />
