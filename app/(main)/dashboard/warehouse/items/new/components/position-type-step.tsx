...
const TYPE_OPTIONS = [
  {
    ...
    description: "Уникальный товар или аксессуар в одном экземпляре. Не требует выбора размеров или цветов внутри формы.",
    icon: Box,
    color: "slate",
  },
  {
    ...
    description: "Заготовка (бланк) без принта. Позволяет сразу создать сетку размеров и цветов (например, футболка Muse в 5 цветах) для учета остатков.",
    icon: Package,
    color: "blue",
  },
  {
    ...
    description: "Конечный продукт с нанесенным принтом. Связывает выбранную базу (бланк) с дизайнами из коллекции, создавая именованные позиции.",
    icon: Tag,
    color: "green",
  },
];
          ...
          {creationType !== "single" && (
            <div className="space-y-3 animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <div className="p-6 rounded-2xl bg-slate-50 border border-slate-100">
                ...
                <RadioGroup options={[ { value: "new", label: "Создать новую линейку", }, { value: "existing", label: "Добавить в существующую линейку", disabled: creationType === "base_line" ? baseLines.length === 0 : finishedLines.length === 0, }, ]} value={lineMode} onValueChange={(v: string) => {
                ...
                {lineMode === "existing" && (
                  <div className="mt-4 animate-in fade-in duration-200">
                    <Select options={(creationType === "base_line" ? baseLines : finishedLines ).map((line) => ({
                        id: line.id,
                        title: line.name,
                      }))}
              ...
              {creationType === "finished_line" && (
                <div className="p-6 rounded-2xl bg-green-50 border border-green-100">
                  ...
                  <Select options={collections.map((c) => ({
                      id: c.id,
                      title: c.name,
                    }))}
                  ...
                  {collections.length === 0 && (
                    <p className="mt-3 text-xs text-slate-500">
                      ...
                      <Link href="/dashboard/design/prints" className="text-green-600 hover:underline">
                        Принты
                      </Link>
                    </p>
                  )}
