    ...
    <>
      ...
      <div className="xl:hidden crm-card !rounded-[24px] px-5 py-6 min-h-[80px] mb-5 flex items-center justify-between gap-3 sticky top-4 z-50 shadow-md">
        <div className="flex items-center gap-3 min-w-0">
          <Button variant="ghost" size="icon" onClick={handleBack} aria-label="Вернуться назад" className="w-8 h-8 shrink-0 rounded-[10px] text-slate-400 hover:text-slate-900 transition-colors active:scale-95 hover:bg-slate-200">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          <Button variant="ghost" size="icon" onClick={handleReset} className="w-8 h-8 rounded-[10px] text-slate-400 hover:text-slate-900 transition-colors active:scale-95 hover:bg-slate-200" title="Начать зановo">
      ...
      <aside className={cn("hidden xl:flex flex-col shrink-0 z-40 transition-all duration-300 text-medium", "crm-card !rounded-3xl shadow-sm p-8", "w-[360px] h-full"
        ...
        <div className="pb-6 shrink-0">
          <Button variant="ghost" onClick={handleBack} className="inline-flex items-center gap-2 text-slate-400 hover:text-slate-900 font-bold mb-4 transition-all group text-sm h-auto p-0 hover:bg-transparent justify-start">
        ...
        <div className="flex-1 space-y-2 overflow-y-auto overflow-x-visible pb-10 -mx-4 px-4 pt-2 custom-scrollbar">
          {steps.map((s, idx) => {
            ...
            return (
              <Button key={idx} variant="ghost" onClick={() => !isLocked && onStepClick(s.id)}
                disabled={isLocked}
