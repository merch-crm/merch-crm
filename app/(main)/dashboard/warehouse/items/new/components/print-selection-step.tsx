    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 p-8 space-y-3">
        ...
        <div className="flex items-center justify-between gap-3 py-2 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={selectAll} className="text-xs font-bold text-primary hover:text-primary hover:bg-primary/10">
              Выбрать все
            </Button>
            <Button variant="ghost" size="sm" onClick={selectNone} className="text-xs font-bold text-slate-400 hover:text-slate-900">
              Сбросить
            </Button>
          </div>
        </div>
            ...
            {prints.map((print) => {
              ...
              return (
                ...
                >
                  {/* Preview */}
                  {print.preview ? (
                    <Image src={print.preview} alt={print.name} fill className={cn( "object-cover transition-transform duration-500", isSelected ? "scale-105" : "group-hover:scale-110" )} />
                  ) : (
      ...
      <StepFooter onBack={onBack} onNext={onNext} isNextDisabled={selectedPrintIds.length === 0} validationError={errors?.prints} />
    </div>
  );
}
