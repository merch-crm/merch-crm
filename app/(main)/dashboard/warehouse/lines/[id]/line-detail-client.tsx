...
export function LineDetailClient({ line, items }: LineDetailClientProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Link href="/dashboard/warehouse/lines" className="hover:text-foreground transition-colors">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <span>/</span>
            {line.category && (
              <>
                <Link href={`/dashboard/warehouse/categories/${line.categoryId}`} className="hover:text-foreground transition-colors">
                  {line.category.name}
                </Link>
                <span>/</span>
              </>
      ...
      {/* Диалоги */}
      <LineFormDialog open={isEditOpen} onOpenChange={setIsEditOpen} line={line} onSuccess={() => router.refresh()}
      />
