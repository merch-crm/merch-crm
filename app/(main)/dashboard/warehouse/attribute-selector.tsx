    ...
    if (type === "color") {
        return (
            <div className="space-y-4 w-full">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    {filteredOptions.map(c => (
                        <button
                            ...
                            className={cn(
                                "group relative aspect-square flex flex-col items-center justify-center gap-1.5 rounded-[var(--radius-inner)] border transition-all duration-300",
                                value === c.code
                        ...
                        >
                            <div
                                className="w-10 h-10 rounded-full border border-black/5 shadow-inner shrink-0 transition-all duration-300 group-hover:shadow-md"
                    ...
                    {allowCustom && (
                        <button
                            ...
                            className={cn(
                                "group aspect-square flex flex-col items-center justify-center gap-1.5 rounded-[var(--radius-inner)] border border-dashed transition-all duration-300",
                                showCustom
                ...
                {showCustom && (
                    <div className="p-6 bg-white rounded-[var(--radius-inner)] border border-slate-200 shadow-crm-xl space-y-5 animate-in slide-in-from-top-4 duration-300">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            <div className="relative w-12 h-12 rounded-[var(--radius-inner)] overflow-hidden border-2 border-slate-200 shrink-0 shadow-inner">
                                <input
                                    type="color"
                            ...
                            <div className="flex-1 space-y-3">
                                <input
                                    ...
                                    className="w-full h-11 px-5 text-sm font-bold rounded-[var(--radius-inner)] border border-slate-200 focus:border-slate-900 outline-none transition-all"
                                    value={customName}
                                ...
                                <button
                                    ...
                                    className="w-full h-11 bg-slate-900 text-white rounded-[var(--radius-inner)] text-xs font-bold hover:bg-black shadow-lg shadow-slate-200 transition-all active:scale-95 disabled:opacity-50"
                                >
                                    {isSaving ? "..." : "Добавить"}
                                </button>
                            </div>
    ...
    if (type === "material") {
        return (
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {filteredOptions.map(m => (
                        <button
                            ...
                            className={cn(
                                "h-11 px-5 rounded-[var(--radius-inner)] text-xs font-medium transition-all border",
                                value === m.code
                    ...
                    {allowCustom && (
                        <button
                            ...
                            onClick={() => setShowCustom(true)}
                            className={cn(
                                "h-11 px-5 rounded-[var(--radius-inner)] border border-dashed transition-all flex items-center gap-2",
                                showCustom
                                    ? "bg-slate-100 border-slate-300 text-slate-900"
                                    : "bg-white border-slate-200 text-slate-400 hover:border-slate-400 hover:text-slate-900 hover:bg-slate-50"
                            )}
                ...
                {showCustom && (
                    <div className="w-full flex gap-2 animate-in fade-in slide-in-from-top-1 p-4 bg-white rounded-[var(--radius-inner)] border border-slate-200">
                        <input
                            value={customName}
                            onChange={e => setCustomName(e.target.value)}
                            className="flex-1 h-11 px-4 rounded-[var(--radius-inner)] border border-slate-200 text-sm font-bold placeholder:font-medium focus:border-slate-900 outline-none"
                        ...
                        <button
                            ...
                            onClick={handleCustomSubmit}
                            disabled={isSaving || !customName.trim()}
                            className="h-11 px-4 bg-slate-900 text-white rounded-[var(--radius-inner)] text-xs font-bold hover:bg-black disabled:opacity-50"
                        >
                            {isSaving ? "..." : "OK"}
                        </button>
                    </div>
                ...
                >
                    {filteredOptions.map(s => (
                        <button
                            ...
                            className={cn(
                                "h-11 flex-1 min-w-max px-3 flex items-center justify-center rounded-[var(--radius-inner)] text-xs font-bold transition-all border",
                                value === s.code
                                    ? "bg-slate-900 border-slate-900 text-white shadow-xl shadow-slate-200"
                                    : "bg-white border-slate-200 text-slate-400 hover:border-slate-300 hover:bg-slate-50/50"
                            )}
                    ...
                    {allowCustom && (
                        <button
                            ...
                            onClick={() => setShowCustom(true)}
                            className={cn(
                                "h-11 flex-1 min-w-max px-3 rounded-[var(--radius-inner)] border border-dashed transition-all flex items-center justify-center gap-2",
                                showCustom
                                    ? "bg-slate-100 border-slate-300 text-slate-900"
                                    : "bg-white border-slate-200 text-slate-400 hover:border-slate-400 hover:text-slate-900 hover:bg-slate-50"
                            )}
        ...
        return (
            <div className="space-y-4">
                ...
                <div className="relative group">
                    {selectedOption ? (
                        <div className="w-full h-11 px-5 rounded-[var(--radius-inner)] border border-slate-900 bg-slate-900 flex items-center justify-between shadow-lg shadow-slate-200 transition-all">
                        ...
                        <>
                            <input
                                ...
                                className="w-full h-11 px-5 rounded-[var(--radius-inner)] border border-slate-200 bg-slate-50 text-sm font-medium placeholder:text-slate-300 focus:bg-white focus:border-slate-200 focus:ring-4 focus:ring-slate-900/5 outline-none transition-all"
                            />
                            {(isFocused || searchQuery.length > 0) && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-[var(--radius-inner)] border border-slate-200 shadow-crm-xl z-[100] max-h-[220px] overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2 duration-200">
                                    {filteredOptions.length > 0 ? (
                                        filteredOptions.map(b => (
                                            <button
                                                ...
                                                className="w-full px-5 py-3 text-left hover:bg-slate-50 border-b border-slate-200 last:border-none transition-colors"
                                            >
