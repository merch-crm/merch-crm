...
export function AdjustStockDialog({ item, locations, itemStocks, onClose, initialType = "in", user }: AdjustStockDialogProps) {
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" role="dialog" aria-modal="true" data-dialog-open="true">
            <div
            ...
            <div className="relative w-full max-w-lg bg-white rounded-[var(--radius-outer)] shadow-2xl border-none animate-in zoom-in-95 fade-in duration-300 flex flex-col my-auto shrink-0 max-h-[92vh] overflow-visible">
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-6 overflow-y-auto custom-scrollbar flex-1 overflow-visible">
                    <div className="grid grid-cols-2 gap-5 overflow-visible">
                        <div className="space-y-6">
                            <div className="bg-slate-50/50 rounded-[var(--radius-inner)] p-3.5 flex items-center justify-between border border-slate-200 shadow-sm relative overflow-hidden group">
                                ...
                                <div className="space-y-0.5 relative z-10">
                                    <span className="text-[9px] font-bold text-slate-400 block">Текущий остаток</span>
                                ...
                                <div className="flex items-baseline gap-1 relative z-10">
                                    ...
                                    <span className="text-[9px] font-bold text-slate-400">{(item.unit === 'pcs' || item.unit === 'шт') ? 'шт.' : item.unit}</span>
                                </div>
                            </div>

                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-500 ml-1">Операция</label>
                            ...
                            <div className="space-y-2 overflow-visible">
                                <label className="text-sm font-bold text-slate-500 ml-1">Целевой склад</label>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    <input
                                        ...
                                        className="w-full h-11 pl-4 pr-24 rounded-[var(--radius-inner)] border border-slate-200 bg-white text-2xl font-bold focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all tabular-nums text-slate-900 shadow-sm"
                                    />
                                    <div className="absolute right-1 top-1 bottom-1 flex gap-1.5">
                                        <div className="flex flex-col gap-0.5">
                                            <button type="button" onClick={() => setAmount(prev => prev + 1)} className="flex-1 w-7 bg-slate-50 border border-slate-200 rounded-[var(--radius-inner)] flex items-center justify-center hover:bg-primary hover:text-white hover:border-primary transition-all shadow-sm">
                                                <ChevronUp className="w-3 h-3 stroke-[3]" />
                                            </button>
                                            <button type="button" onClick={() => setAmount(prev => Math.max(0, prev - 1))} className="flex-1 w-7 bg-slate-50 border border-slate-200 rounded-[var(--radius-inner)] flex items-center justify-center hover:bg-primary hover:text-white hover:border-primary transition-all shadow-sm">
                                                <ChevronDown className="w-3 h-3 stroke-[3]" />
                                            </button>
                                        </div>
                                        <div className="flex items-center justify-center px-3 bg-slate-100 rounded-[var(--radius-inner)] text-[10px] font-bold text-slate-500 shadow-sm">
                                ...
                                <div className="space-y-2 animate-in slide-in-from-top-2 duration-300">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <input
                                            ...
                                            className="h-11 w-full bg-white border border-slate-200 rounded-[var(--radius-inner)] px-3 text-lg font-bold text-primary focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none text-center shadow-sm"
                                        />
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-500 ml-1">Основание / Причина <span className="text-rose-500">*</span></label>
                                <textarea
                                    value={reason}
                                    required
                                    onChange={(e) => setReason(e.target.value)}
                                    placeholder="Напр: Приемка от поставщика..."
                                    className="w-full min-h-[64px] p-3 rounded-[var(--radius-inner)] border border-slate-200 bg-white text-[13px] font-bold text-slate-900 focus:bg-white focus:border-primary outline-none focus:ring-4 focus:ring-primary/5 transition-all resize-none placeholder:text-slate-300 leading-snug shadow-sm"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-6 pt-2 bg-white rounded-b-[var(--radius-outer)]">
                    <button
                        type="submit"
                        disabled={isSubmitting || amount <= 0 || !reason.trim()}
                        onClick={handleSubmit}
                        className={cn(
                            "w-full h-11 rounded-[var(--radius-inner)] font-bold text-sm shadow-lg transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3 border-none",
                            type === "in"
                                ? "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-500/20 text-white"
                                : type === "out" ? "bg-rose-600 hover:bg-rose-700 shadow-rose-500/20 text-white" : "btn-dark"
                        )}
                    >
                        {isSubmitting ? (
                            ...
                            <>
                                {type === "in" ? <Plus className="w-5 h-5 stroke-[3]" /> : type === "out" ? <Minus className="w-5 h-5 stroke-[3]" /> : <Check className="w-5 h-5 stroke-[3]" />}
                                {type === "in" ? "Принять на склад" : type === "out" ? "Подтвердить списание" : "Обновить остаток"}
                            </>
                        )}
                    </button>
                </div>
