...
export function CategoryDetailClient({
  ...
  return (
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Header Group */}
      <CategoryHeader category={category} mounted={ui.mounted} />

      {/* Sub-header with Search and Filters */}
      <CategoryFilters searchQuery={filters.search} setSearchQuery={(val) => setFilters(prev => ({ ...prev, search: val }))}
      ...
      <SubCategoriesSection data={{ subCategories, currentSubCategories, subsPerPage, subCurrentPage: ui.subCurrentPage, activeId: ui.activeId, }} handlers={{ setSubCurrentPage: (page) => setUi(prev => ({ ...prev, subCurrentPage: page })),
          handleDragStart,
          handleDragEnd,
      ...
      <div className="space-y-3 mt-8">
        {dialogs.massActions.showArchiveReason && (
          <ArchiveReasonDialog isOpen={dialogs.massActions.showArchiveReason} onClose={() => setDialogs(prev => ({ ...prev, massActions: { ...prev.massActions, showArchiveReason: false } }))}
          ...
          <div className="space-y-3">
            {groupedData.map((group) => (
              <div key={group.id || 'standalone'} className="space-y-3">
                ...
                <CategoryItemsList items={group.items} selectedIds={ui.selectedIds} toggleSelectAll={toggleSelectAll} toggleSelectItem={(id) => toggleSelectItem(id)}
            ...
            <div className="pt-2">
              <Pagination currentPage={Number(searchParams.get("page")) || 1} totalItems={totalItems} pageSize={itemsPerPage} onPageChange={(page) => updateUrl({ page: page.toString() })}
      ...
      {ui.mounted && (
        <>
          <ConfirmDialog isOpen={dialogs.delete.ids.length> 0}
          ...
          <ConfirmDialog isOpen={!!dialogs.delete.category} onClose={() => setDialogs(prev => ({ ...prev, delete: { ...prev.delete, category: null } }))}
          ...
          {dialogs.edit.category && (
            <EditCategoryDialog category={dialogs.edit.category} categories={allCategories} user={user} isOpen={!!dialogs.edit.category} onClose={() => setDialogs(prev => ({ ...prev, edit: { ...prev.edit, category: null } }))}
            />
          )}

          {dialogs.label.item && (
            <LabelPrinterDialog item={dialogs.label.item} isOpen={dialogs.label.isOpen} onClose={() => setDialogs(prev => ({ ...prev, label: { ...prev.label, isOpen: false } }))}
              attributeTypes={attributeTypes}
              allAttributes={allAttributes}
            />
          )}

          <MassActionsBar selectedIds={ui.selectedIds} setSelectedIds={(val: string[] | ((prev: string[]) => string[])) => {
          ...
          <BulkMoveDialog isOpen={dialogs.massActions.isBulkMoveOpen} selectedIds={ui.selectedIds} storageLocations={storageLocations} onClose={() => setDialogs(prev => ({ ...prev, massActions: { ...prev.massActions, isBulkMoveOpen: false } }))}
            onSuccess={() => {
          ...
          {dialogs.massActions.isBulkCategoryOpen && (
            <BulkCategoryDialog isOpen={dialogs.massActions.isBulkCategoryOpen} selectedIds={ui.selectedIds} categories={allCategories} onClose={() => setDialogs(prev => ({ ...prev, massActions: { ...prev.massActions, isBulkCategoryOpen: false } }))}
              onSuccess={() => {
          ...
          {(dialogs.adjust.item || (items?.length || 0) > 0) && (
            <AdjustStockDialog item={dialogs.adjust.item || (items || [])[0]} locations={storageLocations} itemStocks={dialogs.adjust.stocks} user={user} isOpen={dialogs.adjust.isOpen} onClose={() => {
