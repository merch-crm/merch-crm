...
function ActionBtn({ icon: Icon, label, title, ariaLabel, onClick }: ActionBtnProps) {
  return (
    <Button type="button" variant="ghost" size="icon" onClick={onClick} className="flex items-center gap-2 px-3 md:px-4 py-2 md:py-2.5 rounded-full hover:bg-slate-100 transition-all group shrink-0" title={title} aria-label={ariaLabel}>
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center gap-0.5 md:gap-1 flex-1 justify-around md:justify-start">
              <ActionBtn icon={MapPin} label="Переместить" title="Переместить" ariaLabel="Переместить выбранные товары" onClick={onBulkMove} />
              <ActionBtn icon={Tag} label="Этикетки" title="Этикетки" ariaLabel="Печать этикеток для выбранных товаров" onClick={onBulkPrint} />
              <ActionBtn icon={Download} label="Экспорт" title="Экспорт" ariaLabel="Экспортировать выбранные товары в CSV" onClick={() => {
