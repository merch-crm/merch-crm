...
export function SubCategoriesSection({
  ...
  return (
    <div className="space-y-3 mt-8">
      ...
      >
        <SortableContext items={subCategories.map(s => s.id)}
          strategy={rectSortingStrategy}
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 w-full">
            {currentSubCategories.map((subcat) => (
              <div key={subcat.id} className="min-w-0">
                <SortableSubCategoryCard subcat={subcat} router={router} setEditingCategory={setEditingCategory} setDeletingCategory={setDeletingCategory} />
              </div>
            ))}
          </div>
        </SortableContext>
        {subCategories.length > subsPerPage && (
          <div className="pt-4">
            <Pagination currentPage={subCurrentPage} totalItems={subCategories.length} pageSize={subsPerPage} onPageChange={setSubCurrentPage} itemNames={['подкатегория', 'подкатегории', 'подкатегорий']} variant="card" />
          </div>
        )}
          ...
          {activeId ? (
            <div className="w-full h-full pointer-events-none z-[100]">
              <div className="bg-white border border-primary/30 rounded-[var(--radius)] p-5 shadow-2xl shadow-primary/15 flex items-center justify-between">
                <SubCategoryCardContent subcat={subCategories.find(s => s.id === activeId)!}
                  isDragging={true}
                />
              </div>
            </div>
