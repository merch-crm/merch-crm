...
export default async function WarehouseCategoriesPage() {
  ...
  return (
    <PageContainer>
      ...
      <Suspense fallback={ <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {[1, 2, 3, 4].map(i => (
...
async function InventoryListContainer({ session }: { session: Session | null }) {
  ...
  return (
    <InventoryClient categories={sortedCategories} user={session} />
  );
}
