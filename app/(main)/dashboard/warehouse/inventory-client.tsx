...
export function InventoryClient({ categories: initialCategories = [], user }: InventoryClientProps) {
  ...
  if (itemsByCategory.length === 0) {
    return (
      <EmptyState icon={LayoutGrid} title="Категории не созданы" description="Создайте первую категорию через кнопку «Добавить категорию»." className="py-24" />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        <SortableContext items={itemsByCategory.map((c) => c.id)} strategy={rectSortingStrategy}>
          <div className="flex flex-wrap gap-3" data-testid="categories-list">
            {itemsByCategory.map((category) => (
              ...
              >
                <SortableCategoryCard category={category} router={router} setEditingCategory={setEditingCategory} isAnyDragging={!!activeId} />
              </div>
            ))}
          </div>
        </SortableContext>

        <DragOverlay adjustScale={false} dropAnimation={{ duration: 200, easing: "cubic-bezier(0.18, 0.67, 0.6, 1.22)", }}>
      ...
      {editingCategory && (
        <EditCategoryDialog category={editingCategory} categories={categories} isOpen={!!editingCategory} onClose={() => setEditingCategory(null)}
          user={user}
        />
      )}
    </div>
