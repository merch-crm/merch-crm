...
export function ArchiveTable({ items = [], total }: ArchiveTableProps) {
  ...
  if ((items?.length || 0) === 0 && searchQuery ==="") {
    return (
      <EmptyState icon={Clock} title="Архив пуст" description="Здесь будут отображаться позиции, выведенные из оборота." className="py-20" />
    );
  }

  return (
    <div data-testid="archive-table" className="space-y-3">
      <div className="crm-card flex flex-col md:flex-row gap-3 items-center">
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-2 duration-300">
            <Button type="button" onClick={() => handleRestore(selectedIds)}
          ...
          <thead className="crm-thead">
            <tr>
              <th className="crm-th w-[50px]">
                <Checkbox checked={(currentItems?.length || 0)> 0 && (currentItems || []).every(i => selectedIds.includes(i.id))}
                  onChange={() => {
            ...
            {(currentItems || []).map((item) => (
              ...
              >
                <td className="crm-td" onClick={(e) => e.stopPropagation()}>
                  <Checkbox checked={selectedIds.includes(item.id)} onChange={() => {
                ...
                <td className="crm-td text-right" onClick={(e) => e.stopPropagation()}>
                  <div className="flex items-center justify-end gap-2">
                    <Button type="button" variant="ghost" size="icon" onClick={() => router.push(`/dashboard/warehouse/items/${item.id}`)}
                    ...
                    <Button type="button" variant="ghost" size="icon" onClick={() => handleRestore([item.id])}
                    ...
                    <Button type="button" variant="ghost" size="icon" onClick={() => setIdsToDelete([item.id])}
      ...
      <MobileArchiveList items={currentItems || []} selectedIds={selectedIds} onSelect={(id) => setSelectedIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id])}
      ...
      {total !== undefined && total > 0 && (
        <div className="pt-2">
          <Pagination currentPage={currentPage} totalItems={total} pageSize={itemsPerPage} onPageChange={(page) => updateUrl({ page: page.toString() })}
            itemNames={['позиция', 'позиции', 'позиций']}
          />
        </div>
      )}

      <ConfirmDialog isOpen={idsToDelete.length> 0}
      ...
      >
        <div className="mt-4">
          <Input type="password" placeholder="Пароль от своей учетной записи" value={password} onChange={(e) => setPassword(e.target.value)}
                  ...
                  <div className="px-3 pb-3 pt-0 flex flex-col gap-3">
                    <div className="bg-slate-50 rounded-[var(--radius-inner)] p-3 border border-slate-100 flex flex-col gap-2">
                      ...
                      <div className="pt-2 mt-1 border-t border-slate-200/60 flex gap-2">
                        <Button type="button" variant="outline" size="sm" onClick={(e: React.MouseEvent) => {
                            e.stopPropagation();
                        ...
                        <Button type="button" variant="ghost" size="sm" onClick={(e: React.MouseEvent) => {
                            e.stopPropagation();
                            onRestore([item.id]);
                          }}
                        ...
                        <Button type="button" variant="ghost" size="icon" onClick={(e: React.MouseEvent) => {
                            e.stopPropagation();
                            onDelete(item.id);
                          }}
