...
export function HistoryToolbar({
  ...
  return (
    <div className={cn("crm-filter-tray w-full overflow-hidden flex items-center p-1.5 gap-1"
      ...
      {/* Desktop: Search Box */}
      <div
        className={cn("hidden md:block relative h-11 bg-slate-50 rounded-[18px] flex-1"
        )}
      >
      ...
      <div className={cn("md:hidden relative min-w-0 flex-1"
        ...
        <Input type="text" placeholder="Поиск..." value={searchQuery} onChange={(e) => {
            setSearchQuery(e.target.value);
            setCurrentPage(1);
          }}
          onBlur={() => {
          ...
          onFocus={() => setIsMobileSearchExpanded(true)}
          className={cn("crm-filter-tray-search w-full border-none shadow-none focus-visible:ring-0 min-w-0 !rounded-[18px]", "pl-11 pr-10 opacity-100 bg-slate-50"
          )}
        />
        {searchQuery && (
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {FILTERS.map(f => {
            ...
            return (
              <Button key={f.id} type="button" variant="ghost" role="tab" aria-selected={isActive} onClick={() => {
