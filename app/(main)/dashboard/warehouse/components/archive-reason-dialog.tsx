...
export function ArchiveReasonDialog({
  ...
  const footerContent = (
    <div className="p-5 pt-3 flex items-center justify-end gap-3 bg-white/95 backdrop-blur-md border-t border-slate-100">
      <Button type="button" variant="ghost" onClick={onClose} className="flex h-11 text-slate-400 hover:text-slate-600 font-bold text-sm px-6">
        Отмена
      </Button>
      <Button type="button" onClick={handleConfirm} disabled={isLoading || !reason.trim()} className="h-11 flex-1 sm:flex-none sm:px-10 rounded-[var(--radius-inner)] bg-amber-500 hover:bg-amber-600 shadow-lg shadow-amber-500/20 text-white font-bold text-sm transition-all active:scale-95 disabled:opacity-40">
        {isLoading ? (
