...
export function LabelPrinterSettings({
  ...
  return (
    <div className="md:flex-1 md:min-h-0 flex flex-col md:overflow-hidden relative bg-white">
      {/* Top Fixed Area (Size, Style) */}
      <div className="flex-none px-6 pt-4 pb-2 space-y-3 shrink-0 bg-white z-10">
        {/* Paper Size */}
      ...
      {/* Scrollable Characteristics Area */}
      <div className="flex-1 md:min-h-0 px-6 py-2 overflow-y-auto md:custom-scrollbar relative">
        <div className={cn("space-y-0.5 pb-2 transition-opacity", config.layoutStyle === 'minimal' && "opacity-40 pointer-events-none")}>
          ...
          <div className="-mx-2 grid grid-cols-2 gap-x-1 gap-y-0.5">
            ...
            {resolvedParams
              ...
              .map(p => (
                <ToggleItem key={p.slug} label={p.label} checked={displayOptions.extra[p.slug] ?? true} onChange={(val) => setDisplayOptions(prev => ({ ...prev, extra: { ...prev.extra, [p.slug]: val } }))}
                  compact
                />
              ))}
      ...
      {/* Bottom Fixed Area (Custom text, alignment) */}
      <div className="flex-none px-6 pt-4 pb-2 space-y-3 bg-white shrink-0 z-10 w-full relative">
        {/* Custom Text */}
        <div className={cn("space-y-3 transition-opacity", config.layoutStyle === 'minimal' && "opacity-40 pointer-events-none")}>
