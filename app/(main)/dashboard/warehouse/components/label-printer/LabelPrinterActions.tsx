    ...
    <div className="flex-none px-6 pt-5 pb-6 bg-white border-t border-slate-200 z-50 space-y-3 shadow-[0_-12px_30px_-15px_rgba(0,0,0,0.08)]">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-[var(--radius-inner)]">
          ...
          <Input type="number" value={config.quantity} onChange={(e) => setConfig(prev => ({ ...prev, quantity: Math.max(1, parseInt(e.target.value) || 1) }))}
      ...
      <div className="flex gap-3 w-full">
        <Button type="button" variant="outline" className="flex-1 h-11 rounded-[var(--radius-inner)] bg-white border-2 border-slate-200 text-slate-900 flex items-center justify-center hover:bg-slate-50 hover:border-slate-300 transition-all active:scale-95 group shadow-sm px-0" aria-label="Скачать">
          <Download className="w-5 h-5 text-slate-400 group-hover:text-primary transition-colors" />
        </Button>
        <Button type="button" onClick={handlePrint} variant="action" className="flex-1 h-11 rounded-[var(--radius-inner)] font-bold text-sm flex items-center justify-center gap-2.5 shadow-xl transition-all active:scale-[0.98] group border-none">
