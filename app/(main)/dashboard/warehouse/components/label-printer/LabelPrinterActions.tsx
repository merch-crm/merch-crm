    ...
    <div className="flex-none px-6 pt-5 pb-6 bg-white border-t border-slate-200 z-50 space-y-3 shadow-[0_-12px_30px_-15px_rgba(0,0,0,0.08)]">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-2xl">
          <Button type="button" size="icon" variant="outline" onClick={() => setConfig(prev => ({ ...prev, quantity: Math.max(1, prev.quantity - 1) }))} className="w-8 h-8 rounded-xl bg-white border-slate-200 hover:bg-slate-100 text-slate-900" aria-label="Уменьшить количество">
          ...
          />
          <Button type="button" size="icon" variant="outline" onClick={() => setConfig(prev => ({ ...prev, quantity: prev.quantity + 1 }))} className="w-8 h-8 rounded-xl bg-white border-slate-200 hover:bg-slate-100 text-slate-900" aria-label="Увеличить количество">
