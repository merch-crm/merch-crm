  ...
  return (
    ...
    >
      ...
      <div className="flex-1 mt-5 grid grid-cols-2 gap-3">
        <SummaryStatCard icon={Building2} iconColor="text-blue-600" label="Всего складов" value={totalStorages} />

        <SummaryStatCard icon={Package} iconColor="text-indigo-500" label="Всего SKU" value={totalItems} />

        <SummaryStatCard icon={PackageCheck} iconColor="text-emerald-500" label="Всего товаров" value={totalQuantity} />

        <SummaryStatCard icon={Layers} iconColor="text-blue-500" label="Всего категорий" value={totalCategories} />

        <SummaryStatCard icon={Clock} iconColor="text-orange-500" label="В резерве" value={totalReserved} />

        <SummaryStatCard icon={Archive} iconColor="text-slate-400" label="Архив" value={archivedCount} />
      </div>
    </div>
  );
});
