    ...
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title="Печать" showVisualTitle={false} hideClose={true} footer={isMobile ? <LabelPrinterActions config={config} setConfig={setConfig} handlePrint={handlePrint} /> : undefined} desktopBreakpoint={1280} className="sm:max-w-[1240px]">
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden h-full">
        <div className="w-full md:w-[380px] lg:w-[420px] bg-white border-r border-slate-200 flex flex-col shrink-0 min-h-0 overflow-hidden">
          ...
          <LabelPrinterSettings config={config} setConfig={setConfig} displayOptions={displayOptions} setDisplayOptions={setDisplayOptions} item={item} resolvedParams={resolvedParams} branding={branding} />
          {!isMobile && <LabelPrinterActions config={config} setConfig={setConfig} handlePrint={handlePrint} />}
        </div>
        <LabelPrinterPreview config={config} displayOptions={displayOptions} item={item} resolvedParams={resolvedParams} getAttrLabel={getAttrLabel} branding={branding} onClose={onClose} isMobile={isMobile} />
      </div>
    </ResponsiveModal>
  );
}
