  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 min-[480px]:grid-cols-3 sm:grid-cols-5 gap-3 w-full xl:w-auto">
        <ActivityStatItem icon={ArrowUpRight} label="Поставки" value={activity.ins} hoverColor="group-hover/item:text-emerald-500" />
        <ActivityStatItem icon={ShoppingBag} label="Продажи" value={activity.usage} hoverColor="group-hover/item:text-blue-500" />
        <ActivityStatItem icon={Trash2} label="Расход" value={activity.waste} hoverColor="group-hover/item:text-rose-500" />
        <ActivityStatItem icon={ArrowLeftRight} label="Перемещения" value={activity.transfers} hoverColor="group-hover/item:text-indigo-500" />
        <ActivityStatItem icon={RefreshCw} label="Корректировки" value={activity.adjustments} hoverColor="group-hover/item:text-amber-500" />
      </div>
    </div>
  );
});
