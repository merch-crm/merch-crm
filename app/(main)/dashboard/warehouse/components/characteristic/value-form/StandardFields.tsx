  ...
  return (
    <div className="space-y-3">
      <div className="space-y-1.5">
        ...
        <Input value={valueForm.fullName} onChange={e => {
      ...
      {showShortName && (
        <div className="space-y-1.5 pt-1">
          ...
          <Input value={valueForm.name} onChange={e => setValueForm(prev => ({ ...prev, name: e.target.value }))}
      ...
      {isConsumable && (
        <div className="grid grid-cols-2 gap-2 pt-1">
          <CompactToggle icon={Palette} title="Выбор цвета" checked={typeForm.hasColor} onCheckedChange={val => setTypeForm(prev => ({ ...prev, hasColor: val }))}
          />
          <CompactToggle icon={Layers} title="Состав / Компоненты" checked={typeForm.hasComposition} onCheckedChange={val => setTypeForm(prev => ({ ...prev, hasComposition: val }))}
          />
        </div>
      )}
    </div>
