  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <SortableContext items={localTypes.map((t) => t.id)}
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-stretch justify-center w-full gap-3">
              {localTypes.map((type) => (
                ...
                >
                  <SortableCharacteristicCard type={type} attributes={attributes} openEditType={openEditType} openEditValue={openEditValue} />
                </div>
              ))}
              {localTypes.length === 0 && (
                <div className="w-full">
                  <EmptyState icon={Book} title="Нет характеристик" description={ activeCategoryId === "uncategorized" ? "Все характеристики распределены по категориям." : `В категории «${activeCategoryName}» пока нет созданных типов характеристик.` } />
                </div>
              )}
            </div>
          </div>
        ...
        <DragOverlay adjustScale={true}>
          {activeType ? (
            <div className="w-full h-full pointer-events-none">
              <CharacteristicCardContent type={activeType} attributes={attributes} openEditType={() => { }}
                openEditValue={() => { }}
                isOverlay
              />
            </div>
