...
export function QuickTransferModal({ item, currentLocationId, locations, onClose, onSuccess }: QuickTransferModalProps) {
  ...
  return (
    <ResponsiveModal isOpen={!!item} onClose={onClose} showVisualTitle={false}>
      <div className="p-6 pt-2 flex flex-col gap-3 text-left">
        ...
        <form action={handleTransfer} className="space-y-3">
          ...
          <div className="space-y-2 overflow-visible">
            <label className="text-sm font-bold text-slate-700 block mb-2 ml-1">Куда переместить <span className="text-rose-500">*</span></label>
            <Select options={locations.filter(l => l.id !== currentLocationId).map(l => ({ id: l.id, title: l.name, icon: <Building className="w-4 h-4 opacity-50" /> }))}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Input name="comment" placeholder="..." className={cn("w-full h-11 px-4 rounded-[var(--radius-inner)] border bg-slate-50 text-sm font-bold outline-none transition-all shadow-sm", state.errors.comment ?"border-rose-200 bg-rose-50/50" :"border-slate-200 bg-slate-50 focus:border-primary" )} />
            </div>
          </div>

          <div className="pt-2 flex items-center gap-3 w-full">
            <Button type="button" variant="ghost" onClick={onClose} className="flex h-11 w-full max-w-[120px] text-slate-400 font-bold text-sm hover:text-slate-600 transition-all items-center justify-center p-0">
              Отмена
            </Button>
