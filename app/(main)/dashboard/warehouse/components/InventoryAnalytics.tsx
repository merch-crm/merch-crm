          ...
          {topSoldItems.length === 0 ? (
            ...
            <ol className="flex flex-col gap-2">
              {topSoldItems.map((item, idx) => (
                <li key={item.id}>
                  <Link href={`/dashboard/warehouse/items/${item.id}`} className="group flex items-center gap-3 p-2.5 rounded-[10px] hover:bg-slate-50 transition-colors">
          ...
          {stagnantItems.length === 0 ? (
            ...
            <div className="flex flex-col gap-2 overflow-y-auto max-h-[260px] custom-scrollbar">
              {stagnantItems.map((item) => (
                <Link key={item.id} href={`/dashboard/warehouse/items/${item.id}`} className="group flex items-center gap-3 p-2.5 rounded-[10px] hover:bg-amber-50/50 border border-transparent hover:border-amber-100 transition-all">
