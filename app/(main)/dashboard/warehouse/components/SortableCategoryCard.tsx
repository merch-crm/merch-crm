...
export const SortableCategoryCard = React.memo(({
  ...
  return (
    ...
    >
      ...
      />
      <CategoryCardContent category={category} isDragging={isDragging} isWide={isWide} isExtraWide={isExtraWide} dragHandleProps={{ ...attributes, ...listeners }} onEdit={() => setEditingCategory(category)}
      />
    </div>
  );
});
...
export const DragPreview = React.memo(({ category }: DragPreviewProps) => {
  ...
  return (
    <div className="w-[420px] shadow-2xl rounded-[32px] pointer-events-none select-none overflow-hidden border border-primary/20 bg-white/80 backdrop-blur-sm">
      <CategoryCardContent category={category} isDragging={false} isWide={true} isExtraWide={false} onEdit={() => { }}
      />
    </div>
  );
});
