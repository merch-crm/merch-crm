          ...
          <div className="max-h-[280px] overflow-y-auto p-2 custom-scrollbar">
            {filteredCategories.length > 0 ? (
              filteredCategories.map(cat => {
                ...
                return (
                  <Button key={cat.id} type="button" onClick={() => handleSelect(cat.id)}
                    variant="ghost"
              ...
              <GlassEmptyState icon={Package} title="Ничего не найдено" description="Попробуйте изменить поисковый запрос" className="p-4" />
            )}
          </div>
        </div>
      )}
