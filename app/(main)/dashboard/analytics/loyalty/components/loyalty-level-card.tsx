    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          >
            <LoyaltyLevelBadge level={level} size="lg" showDiscount={false} className="!bg-transparent !px-0" />
          </div>
          <div>
        ...
        <div className="flex gap-1">
          <Button type="button" variant="ghost" size="sm" onClick={() => onEdit(level)}
            className="h-8 w-8 p-0 rounded-lg outline-none focus:ring-2 focus:ring-primary/20"
          >
            <Pencil className="w-4 h-4" />
          </Button>
          <Button type="button" variant="ghost" size="sm" onClick={() => onDelete(level)}
