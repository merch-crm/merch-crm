...
export function LoyaltySettingsClient({ initialLevels, initialStats }: LoyaltySettingsClientProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        <AnimatePresence mode="popLayout">
          {levels
            ...
            .map((level, index) => (
              <LoyaltyLevelCard key={level.id} level={level} index={index} stat={getStatForLevel(level.id)} onEdit={openEditModal} onDelete={(l) => setUi(prev => ({ ...prev, deletingLevel: l }))}
              />
            ))}
        </AnimatePresence>
      </div>
      ...
      {/* Create/Edit Modal */}
      <LoyaltyLevelModal isOpen={ui.isCreateOpen || !!ui.editingLevel} onClose={() => {
          setUi(prev => ({ ...prev, isCreateOpen: false, editingLevel: null }));
          resetForm();
        }}
      ...
      {/* Delete Confirmation */}
      <ConfirmDialog isOpen={!!ui.deletingLevel} onClose={() => setUi(prev => ({ ...prev, deletingLevel: null }))}
