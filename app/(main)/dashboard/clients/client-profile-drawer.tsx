...
export function ClientProfileDrawer({ clientId, isOpen, onClose, onEdit, showFinancials = true, userRoleName }: ClientProfileDrawerProps) {
  ...
  return (
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title={loading ? "Загрузка..." : fullName} description="Профиль клиента" className="max-w-2xl max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-white">
          {loading ? (
            ...
            <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {activeTab === "general" && (
                <OverviewTab client={client} userRoleName={userRoleName} onLoyaltyChange={handleLoyaltyChange} onRecalculate={handleRecalculate} clientChats={clientChats} />
              )}

              {activeTab === "contacts" && client.clientType === "b2b" && (
                <ClientContactsList clientId={client.id} clientType={client.clientType as "b2c" | "b2b"} />
              )}

              {activeTab === "orders" && (
                <OrdersTab client={client} showFinancials={showFinancials} currencySymbol={currencySymbol || "₽"} />
              )}
