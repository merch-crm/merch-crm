...
export function ClientsTable({ userRoleName, showFinancials, initialData }: { userRoleName?: string | null, showFinancials?: boolean, initialData?: ClientsInitialData }) {
  ...
  return (
    <div className="space-y-3">
      <ClientListHeader filters={filters} setFilters={setFilters} typeCounts={typeCounts} activityCounts={activityCounts} showAtRiskBanner={showAtRiskBanner} uiState={uiState} setUiState={setUiState} referenceData={{ regions, managers, sources }} handlers={{ onClientTypeChange: handleClientTypeChange, onActivityStatusChange: handleActivityStatusChange, onDismissAtRiskBanner: () => setShowAtRiskBanner(false),
      ...
      <ClientTable clients={clientsList} selectedIds={selectedIds} onSelectRow={handleSelectRow} onSelectAll={handleSelectAll} isAllSelected={isAllSelected} filters={filters} setFilters={setFilters} managers={managers} onUpdateField={handleUpdateField} onEdit={(client) => setDialogs(prev => ({ ...prev, editingClient: client }))}
...

      {
        (viewState.data?.total || 0) > 0 && (
          <Pagination totalItems={viewState.data?.total || 0} pageSize={10} currentPage={currentPage} itemNames={['клиент', 'клиента', 'клиентов']} />
        )
      }

      <ClientProfileDrawer clientId={dialogs.selectedClientId || ""} isOpen={!!dialogs.selectedClientId} onClose={() => setDialogs(prev => ({ ...prev, selectedClientId: null }))}
        onEdit={() => {
...

      {
        dialogs.editingClient && (
          <EditClientDialog client={dialogs.editingClient} isOpen={!!dialogs.editingClient} onClose={() => {
      ...
      <ClientBulkActions selectedIds={selectedIds} setSelectedIds={setSelectedIds} managers={managers} userRoleName={userRoleName} fetchClients={fetchClients} handleExportClick={handleExportClick} showArchived={filters.showArchived} mounted={viewState.mounted} />

      <ExportDialog open={uiState.showExportDialog} onClose={() => setUiState(prev => ({ ...prev, showExportDialog: false }))}
