...
export function OverviewTab({
  ...
  return (
    <div className="space-y-3">
      <ClientStatsWidget stats={{ totalOrdersCount: client.totalOrders || 0, totalOrdersAmount: client.totalSpent || 0, averageCheck: client.stats?.total && client.totalOrders ? client.stats.total / client.totalOrders : 0, lastOrderAt: client.lastOrderAt ? new Date(client.lastOrderAt) : null, firstOrderAt: client.firstOrderAt ? new Date(client.firstOrderAt) : null, daysSinceLastOrder: client.daysSinceLastOrder || null, }} />
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <RFMSegmentBadge segment={client.rfmSegment || null} score={client.rfmScore || null} size="sm" showScore />
        </div>
        <LoyaltyLevelSelect value={client.loyaltyLevelId || null} onChange={onLoyaltyChange} isManual={client.loyaltyLevelSetManually} />
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-2">
          <InfoRow icon={Smartphone} label="Телефон" value={client.phone} />
          {client.email && (
          ...
          {client.company && client.clientType !== "b2b" && (
            <InfoRow icon={Building2} label="Компания" value={client.company} />
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="space-y-3">
          ...
          <InfoRow icon={User} label="Менеджер" value={client.manager?.name || "Не назначен"} />
        </div>
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-slate-100">
          ...
          <div className="space-y-2">
            {clientChats.slice(0, 3).map(chat => {
              ...
              return (
                <Link key={chat.id} href={`/dashboard/communications?chat=${chat.id}`} className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-primary/20 hover:shadow-sm transition-all group">
                  <div
          ...
          <Link href={`/dashboard/communications?client=${client.id}`} className="mt-3 px-2 text-xs font-bold text-primary hover:underline flex items-center gap-1.5 transition-all">
