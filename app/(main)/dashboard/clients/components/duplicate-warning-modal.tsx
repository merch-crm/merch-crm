...
export const DuplicateWarningModal = memo(function DuplicateWarningModal({
  ...
  return (
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title="" className="max-w-lg">
      <div className="p-6 space-y-3">
          ...
          {duplicates.map((dup, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {/* Open Button */}
                <Button type="button" variant="ghost" size="sm" className="shrink-0 h-8 px-3 rounded-lg text-xs font-bold text-slate-400 group-hover:text-primary group-hover:bg-white transition-all" onClick={(e) => {
                    e.stopPropagation();
                    onOpenClient(dup.id);
                  }}
                >
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-slate-100">
          <Button type="button" variant="ghost" onClick={onClose} className="flex-1 h-11 rounded-xl text-sm font-bold text-slate-500 hover:text-slate-900 hover:bg-slate-100">
            Отмена
          </Button>
          <Button type="button" onClick={onIgnore} disabled={isLoading} className="flex-1 h-11 rounded-xl text-sm font-bold bg-amber-500 hover:bg-amber-600 text-white border-none">
