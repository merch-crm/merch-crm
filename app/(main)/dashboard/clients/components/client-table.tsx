    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2">
        ...
        {client.daysSinceLastOrder !== null && client.daysSinceLastOrder !== undefined && client.daysSinceLastOrder >= 60 && (
          <Tooltip side="right" content={ client.daysSinceLastOrder>= 180
      ...
      {(client as ClientSummary & { rfmSegment?: string }).rfmSegment && (
        <div className="mt-1">
          <RFMSegmentBadge segment={(client as ClientSummary & { rfmSegment?: string }).rfmSegment!} size="sm" />
        </div>
      )}
    </div>
  );
              ...
              <thead className="crm-thead">
                <tr>
                  <th className="crm-th crm-td-selection">
                    <Checkbox checked={isAllSelected} onChange={onSelectAll} aria-label="Выбрать всех" />
                  </th>
                  <th
                ...
                {clients?.map(client => (
                  ...
                  >
                    <td className="crm-td crm-td-selection" onClick={(e) => e.stopPropagation()}>
                      <Checkbox checked={selectedIds.includes(client.id)} onChange={() => onSelectRow(client.id)}
                      />
                    </td>
                    <td className="crm-td">{renderClientInfo(client)}</td>
                    <td className="crm-td" onClick={(e) => e.stopPropagation()}>
                      <FunnelStageSelect value={client.funnelStage || null} onChange={async (stage) => {
                    ...
                    <td className="crm-td" onClick={(e) => e.stopPropagation()}>
                      <LoyaltyLevelSelect value={client.loyaltyLevelId || null} onChange={async (levelId, setManually) => {
                    ...
                    <td className="crm-td" onClick={(e) => e.stopPropagation()}>
                      <Select value={client.managerId || ""} onChange={(val: string) => onUpdateField(client.id, "managerId", val)}
                        options={[
                          { id: "", title: "Общий" },
                    ...
                    <td className="crm-td crm-td-actions" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <Button type="button" variant="ghost" size="icon" onClick={() => router.push(`/dashboard/clients/${client.id}`)}
                          className="h-8 w-8 text-slate-400 hover:text-primary hover:bg-primary/10 rounded-xl"
                        >
                          <Eye className="w-4 h-4" />
                        </Button>
                        <Button type="button" variant="ghost" size="icon" onClick={() => onEdit(client)}
