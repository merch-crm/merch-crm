...
import { activityStatusLabels } from "../validation";
import {
  Search, SlidersHorizontal, RotateCcw, X, Megaphone,
  Users as UsersIcon, Clock, Target, Users, UserCheck,
  AlertCircle, AlertTriangle, UserX
} from "lucide-react";
...
export const ClientFilterPanel = memo(function ClientFilterPanel({
  ...
  const activeFiltersCount = [
    ...
    filters.rfmSegment !== undefined && filters.rfmSegment !== "all",
            ...
            <div className="bg-white border-2 border-slate-100 rounded-3xl p-6 shadow-sm space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                <div className="space-y-2">
                  ...
                  <Select value={filters.period || "all"} onChange={(val: string) => setFilters(prev => ({ ...prev, period: val as ClientFilters['period'] }))}
                    options={[
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filters.orderCount || "any"} onChange={(val: string) => setFilters(prev => ({ ...prev, orderCount: val as ClientFilters['orderCount'] }))}
                    options={[
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filters.region || "all"} onChange={(val: string) => setFilters(prev => ({ ...prev, region: val }))}
                    options={[
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filters.managerId || "all"} onChange={(val: string) => setFilters(prev => ({ ...prev, managerId: val }))}
                    options={[
                      { id: "all", title: "Все менеджеры" },
                      { id: "none", title: "Без менеджера" },
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filters.acquisitionSource || "all"} onChange={(val: string) => setFilters(prev => ({ ...prev, acquisitionSource: val }))}
                    options={[
                      { id: "all", title: "Все источники" },
                      { id: "none", title: "Не указан" },
                      ...sources.map(s => ({
                        id: s,
                        title: `${SOURCE_ICONS[s] || "📍"} ${s}`
                      }))
                    ]}
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
                <div className="space-y-2">
                  ...
                  <Select value={filters.status || "all"} onChange={(val: string) => setFilters(prev => ({ ...prev, status: val as ClientFilters['status'] }))}
                    options={[
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filters.activityStatus || "all"} onChange={(val: string) => setFilters(prev => ({
                      ...prev,
                      activityStatus: val as ClientFilters['activityStatus']
                    }))}
                    options={[
                      { id: "all", title: "Любая активность", icon: <Users className="w-4 h-4" /> },
                      {
                        id: "active",
                        title: `Активные${activityCounts?.active ? ` (${activityCounts.active})` : ""}`,
                        icon: <UserCheck className="w-4 h-4 text-emerald-500" />
                      },
                      {
                        id: "attention",
                        title: `Требуют внимания${activityCounts?.attention ? ` (${activityCounts.attention})` : ""}`,
                        icon: <AlertCircle className="w-4 h-4 text-amber-500" />
                      },
                      {
                        id: "at_risk",
                        title: `В зоне риска${activityCounts?.atRisk ? ` (${activityCounts.atRisk})` : ""}`,
                        icon: <AlertTriangle className="w-4 h-4 text-orange-500" />
                      },
                      {
                        id: "inactive",
                        title: `Неактивные${activityCounts?.inactive ? ` (${activityCounts.inactive})` : ""}`,
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filters.rfmSegment || "all"} onChange={(val: string) => setFilters(prev => ({
                      ...prev,
                      rfmSegment: val === "all" ? undefined : val
                    }))}
                    options={[
                      { id: "all", title: "Все сегменты" },
                      { id: "champions", title: "🏆 Чемпионы" },
                      { id: "loyal", title: "💎 Лояльные" },
                      { id: "potential", title: "⭐️ Потенциальные" },
                      { id: "new", title: "✨ Новички" },
                      { id: "at_risk", title: "⚠️ В зоне риска" },
                      { id: "lost", title: "👻 Потерянные" },
                    ]}
                  />
                </div>
          ...
          {filters.managerId && filters.managerId !== "all" && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-full text-xs font-black">
              ...
              {filters.managerId === "none"
                ? "Без менеджера"
                : managers.find(m => m.id === filters.managerId)?.name || "Менеджер"
              }
              <button type="button" onClick={() => setFilters(prev => ({ ...prev, managerId: "all" }))}><X className="w-3 h-3 hover:text-blue-200" /></button>
            </div>
          )}
          {/* === НОВОЕ: Чип источника === */}
          {filters.acquisitionSource !== "all" && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white rounded-full text-xs font-black">
              ...
              <button type="button" onClick={() => setFilters(prev => ({ ...prev, acquisitionSource: "all" }))}><X className="w-3 h-3 hover:text-emerald-200" /></button>
            </div>
          )}
          {/* === Чип активности === */}
