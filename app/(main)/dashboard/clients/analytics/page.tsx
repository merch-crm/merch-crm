...
export default async function ClientAnalyticsPage() {
  ...
  return (
    <div className="space-y-3">
      <PageHeader title="Аналитика клиентов" description="Ключевые метрики и показатели клиентской базы" className="px-1" />

      <Suspense fallback={<LoadingSpinner />}>
        <ClientAnalyticsDashboard overview={overview} funnelData={funnelData} growthData={growthData} rfmRevenueData={rfmRevenueData} managersData={managersData} topClients={topClients} sourcesData={sourcesData} loyaltyData={loyaltyData} rfmDistribution={rfmDistribution} currencySymbol={currencySymbol} showFinancials={showFinancials} />
      </Suspense>
    </div>
  );
}
