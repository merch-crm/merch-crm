  ...
  return (
    <>
      <div className="flex items-center gap-2">
        <Button variant="outline" className="rounded-2xl gap-2 border-slate-200 hover:bg-slate-50 transition-all" onClick={() => setIsEditOpen(true)}
        >
          <Edit2 className="w-4 h-4" />
          <span className="hidden sm:inline">Редактировать</span>
        </Button>

        <Button variant="outline" disabled={isArchiving} className="rounded-2xl gap-2 border-slate-200 hover:bg-slate-50 transition-all text-slate-400 hover:text-rose-500" onClick={handleToggleArchive}>
      ...
      <EditClientDialog client={client} isOpen={isEditOpen} onClose={() => setIsEditOpen(false)}
      />
    </>
  );
}
