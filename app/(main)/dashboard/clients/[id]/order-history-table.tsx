...
export function OrderHistoryTable({ orders, currencySymbol }: OrderHistoryTableProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      {safeOrders.length > 0 ? (
        <ResponsiveDataView data={safeOrders} renderTable={() => (
          ...
          renderCard={(order) => {
            ...
            return (
              <Link key={order.id} href={`/dashboard/orders/${order.id}`} className="block p-4 bg-white border border-slate-100 rounded-2xl shadow-sm active:scale-[0.98] transition-all">
