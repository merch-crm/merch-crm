  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3">
        <SearchInput placeholder="Поиск по имени или должности..." value={search} onChange={(e) => setSearch(e.target.value)}
        />
      </div>
      ...
      {filteredStaff.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {filteredStaff.map((member) => (
            <StaffCard key={member.id} member={member} lines={lines} applicationTypes={applicationTypes} onUpdate={handleUpdate} onDelete={handleDelete} />
          ))}
        </div>
      ) : (
      ...
      {/* Create Dialog */}
      <StaffFormDialog open={isCreateOpen} onOpenChange={setIsCreateOpen} lines={lines} applicationTypes={applicationTypes} onSuccess={handleCreateSuccess} />
    </div>
  );
}
