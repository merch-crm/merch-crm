      ...
      <DialogContent className="max-w-2xl overflow-y-auto max-h-[90vh]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {/* Specializations */}
              <FormField control={form.control} name="specializationIds" render={() => (
                  <FormItem>
                    <div className="mb-2">
                    ...
                    <div className="grid grid-cols-1 gap-2 p-3 border rounded-lg max-h-[200px] overflow-y-auto">
                      {applicationTypes.map((type) => (
                        <FormField key={type.id} control={form.control} name="specializationIds" render={({ field }) => {
                            return (
                              <FormItem key={type.id} className="flex flex-row items-start space-x-3 space-y-0">
                                <FormControl>
                                  <Checkbox checked={field.value?.includes(type.id)} onCheckedChange={(checked) => {
                                      return checked
              ...
              {/* Lines */}
              <FormField control={form.control} name="lineIds" render={() => (
                  <FormItem>
                    <div className="mb-2">
                    ...
                    <div className="grid grid-cols-1 gap-2 p-3 border rounded-lg max-h-[200px] overflow-y-auto">
                      {lines.map((line) => (
                        <FormField key={line.id} control={form.control} name="lineIds" render={({ field }) => {
                            return (
                              <FormItem key={line.id} className="flex flex-row items-start space-x-3 space-y-0">
                                <FormControl>
                                  <Checkbox checked={field.value?.includes(line.id)} onCheckedChange={(checked) => {
                                      return checked
