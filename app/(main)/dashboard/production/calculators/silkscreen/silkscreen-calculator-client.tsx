...
export function SilkscreenCalculatorClient() {
 ...
 return (
  <BaseCalculatorForm calculatorType="silkscreen" calculator={calculator} beforeFilesContent={beforeFilesContent}>
   <CalculatorSection title="Основные параметры">
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
     <div className="space-y-3">
      <Label htmlFor="silk-quantity">Тираж (шт)</Label>
      <Input id="silk-quantity" type="number" min={1} value={params.quantity || 1} onChange={(e) => updateParams({ quantity: parseInt(e.target.value) || 1 })}
      />
     </div>
     ...
     <div className="space-y-3">
      ...
      <Slider value={[colorCount]} onValueChange={([value]) => updateParams({ colorCount: value as SilkscreenCalculatorParams['colorCount'] })}
       min={1}
       max={8}
       step={1}
      />
     ...
     <div className="space-y-3">
      ...
      <Select options={SILKSCREEN_INK_TYPES.map(t => ({ id: t.value, title: t.label }))}
     ...
     <div className="space-y-3">
      ...
      <Select options={MESH_SIZE_OPTIONS.map(o => ({ id: String(o.value), title: o.label }))}
     ...
     <div className="space-y-3">
      ...
      <Select options={SUBSTRATE_TYPES.map(t => ({ id: t.value, title: t.label }))}
     ...
     <div className="flex items-center justify-between p-3 border rounded-lg bg-slate-50/50">
      ...
      <Switch id="whiteBase" checked={params.needsWhiteBase} onCheckedChange={(checked) => updateParams({ needsWhiteBase: checked })}
