        ...
        <div className="flex justify-between items-start gap-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 flex-1">
            <div className="space-y-2">
              <Label>Изделие</Label>
              <Select value={order.garmentId} onChange={(val) => onUpdate({ garmentId: val })}
                options={SILKSCREEN_GARMENTS.map((g) => ({ id: g.id, title: g.name }))}
              />
            </div>
            <div className="space-y-2">
              <Label>Количество (шт)</Label>
              <Input type="number" min={1} value={order.quantity === 0 ? '' : order.quantity} onChange={(e) => onUpdate({ quantity: e.target.value === '' ? 0 : parseInt(e.target.value) || 0 })}
              />
            </div>
            <div className="flex items-center space-x-2 pt-8">
              <Checkbox id={`dark-${order.id}`} checked={order.isDarkGarment} onCheckedChange={(val) => onUpdate({ isDarkGarment: !!val })}
              />
            ...
            <div key={pIndex} className="bg-muted/50 p-4 rounded-lg space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <Select value={pos.positionId} onChange={(val) => onUpdatePosition(pIndex, { positionId: val })}
                    options={SILKSCREEN_POSITIONS.map((p) => ({ id: p.id, title: p.name }))}
                  />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs">Размер</Label>
                  <Select value={pos.sizeId} onChange={(val) => onUpdatePosition(pIndex, { sizeId: val })}
                  ...
                  {pos.colors.map((color, cIndex) => (
                    <Badge key={cIndex} className="pr-1 gap-2 bg-background border flex items-center" color="neutral">
                      ...
                      <div className="flex items-center gap-1 border-l pl-2">
                        <Checkbox className="w-3 h-3" checked={color.isUnderbase} onCheckedChange={(val) => onUpdateColor(pIndex, cIndex, { isUnderbase: !!val })}
                        />
