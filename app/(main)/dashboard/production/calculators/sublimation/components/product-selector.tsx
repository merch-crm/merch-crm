...
const ProductCard = memo(function ProductCard({
 ...
 return (
  <Card className={cn( "p-4 transition-all cursor-pointer border-slate-200 shadow-sm hover:shadow-md", isSelected ? "ring-2 ring-primary bg-primary/5 border-transparent" : "hover:bg-slate-50" )} role="button" tabIndex={isSelected ? -1 : 0} onClick={() => !isSelected && onAdd()}
   onKeyDown={(e) => {
   ...
   <div className="flex items-start gap-3">
    ...
    {isSelected ? (
     <div role="button" tabIndex={0} className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
      <Button variant="outline" size="icon" onClick={onRemove} className="w-7 h-7 rounded-lg bg-white border-slate-200">
       {quantity === 1 ? <Trash2 className="w-3.5 h-3.5 text-rose-500" /> : <Minus className="w-3.5 h-3.5" />}
      </Button>
      
      <Input type="number" value={quantity} onChange={(e) => onQuantityChange(parseInt(e.target.value) || 0)}
       className="w-12 h-7 text-center p-0 text-xs font-bold bg-white border-slate-200 rounded-lg"
       min={0}
      />
      
      <Button variant="outline" size="icon" onClick={onAdd} className="w-7 h-7 rounded-lg bg-white border-slate-200">
  ...
  <div className="space-y-3">
   ...
   <div className="flex flex-col sm:flex-row gap-3">
    ...
    <div className="flex gap-1 overflow-x-auto pb-1 sm:pb-0 scrollbar-hide">
      {productCategories.map(cat => (
        <Button key={cat.id} variant={activeCategory === cat.id ? "solid" : "outline"} size="sm" onClick={() => setActiveCategory(cat.id)}
          className={cn(
   ...
   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
    {filteredProducts.map(product => (
     <ProductCard key={product.id} product={product} quantity={getQuantity(product.id)} onAdd={() => handleAdd(product.id)}
