...
const GarmentCard = memo(function GarmentCard({
    ...
    return (
      <Card className="p-4 ring-2 ring-primary bg-primary/5">
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => setShowColorChoice(false)}
  ...
  return (
    <Card 
      className="p-4 cursor-pointer hover:shadow-md hover:bg-slate-50 transition-all"
      onClick={handleClick}
    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-1 custom-scrollbar">
          {filteredGarments.map(garment => (
            <GarmentCard
              key={garment.id}
              garment={garment}
              onSelect={(color) => handleSelect(garment.id, color)}
            />
          ))}
        </div>
