...
export const DtgOrderCard = memo(function DtgOrderCard({
  ...
  return (
    <Card className={cn(
      "overflow-hidden border-slate-200 shadow-sm rounded-[24px] transition-all",
      order.garmentColor === 'dark' ? "bg-slate-900 border-slate-800" : "bg-white"
    )}>
      {/* Заголовок */}
      <div 
        className={cn(
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <Badge 
              className={cn(
                "rounded-full font-bold text-xs px-2",
                order.garmentColor === 'dark' ? "bg-slate-800 text-slate-300 border-slate-700" : "bg-amber-50 text-amber-600 border-amber-100"
              )}
            >
        ...
        <div role="button" tabIndex={0} className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
          <Button variant="ghost" size="sm" onClick={onDuplicate} className="rounded-xl">
            <Copy className="w-4 h-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={onDelete} className="rounded-xl text-rose-500 hover:text-rose-600 hover:bg-rose-50">
        ...
        <div className={cn(
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <Button
              ...
              className={cn(
                "rounded-xl font-bold h-10 px-4 transition-all shadow-sm",
                order.garmentColor === 'dark' ? "bg-slate-800 border-slate-700 text-white hover:bg-slate-700" : "bg-white border-slate-200 text-slate-700"
              )}
            >
            ...
            <div className="grid gap-3">
                ...
                return (
                    ...
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-3">
                        <Badge variant="secondary" className="rounded-lg font-bold bg-white/10 text-xs">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="relative">
                            <Input
                                type="number"
                                value={pos.widthMm}
                                onChange={(e) => handleUpdatePosition(pos.positionId, {
            ...
            {availablePositions.length > 0 && (
              <div className="mt-4">
                  <Select
                    ...
                    placeholder="+ Добавить позицию печати"
                    triggerClassName="h-12 border-2 border-dashed border-slate-700 rounded-2xl bg-transparent text-slate-400 font-bold hover:border-indigo-500 hover:text-indigo-500 transition-all px-6"
                  />
              </div>
            )}
