...
export function UVDTFCalculatorClient() {
 ...
 return (
  <BaseCalculatorForm calculatorType="uv-dtf" calculator={calculator} showRollVisualizer={true}>
   <CalculatorSection title="Основные параметры">
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
     
     <div className="space-y-3">
      <Label htmlFor="uv-quantity">Количество изделий</Label>
      <Input id="uv-quantity" type="number" min={1} value={params.quantity === 0 ? '' : params.quantity} onChange={(e) => updateParams({ quantity: e.target.value === '' ? 0 : parseInt(e.target.value) || 0 })}
      />
     </div>

     <div className="space-y-3">
      <Label>Тип плёнки</Label>
      <Select options={UV_DTF_FILM_TYPES.map(t => ({ id: t.value, title: t.label }))}
     ...
     <div className="space-y-3">
      ...
      <Select options={UV_DTF_FINISH_TYPES.map(t => ({ id: t.value, title: t.label }))}
     ...
     <div className="flex items-center justify-between p-3 border rounded-lg bg-slate-50/50">
      ...
      <Switch id="uv-lamination" checked={params.withLamination} onCheckedChange={(checked) => updateParams({ withLamination: checked })}
      />
     </div>
