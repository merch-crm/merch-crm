      ...
      <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between bg-muted/30 p-4 rounded-2xl border">
        <div className="relative w-full lg:max-w-md">
          ...
          <Input 
            ...
            className="pl-10 h-11 bg-background border-2 rounded-xl"
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <Select 
            value={initialFilters.calculatorType || 'all'} 
            onChange={handleTypeChange}
            options={[
              { id: 'all', title: 'Все типы' },
              ...Object.values(CALCULATOR_TYPES_CONFIG).map((type) => ({
                id: type.type,
                title: type.label
              }))
            ]}
          ...
          <Select 
            value={`${initialFilters.sortBy}-${initialFilters.sortOrder}`} 
            onChange={handleSortChange}
            options={[
              { id: 'createdAt-desc', title: 'Новые сначала' },
              { id: 'createdAt-asc', title: 'Старые сначала' },
              { id: 'sellingPrice-desc', title: 'Дорогие сначала' },
              { id: 'sellingPrice-asc', title: 'Дешевые сначала' },
              { id: 'totalCost-desc', title: 'По себестоимости ↓' },
              { id: 'name-asc', title: 'По названию (А-Я)' },
            ]}
            className="w-[200px]"
            triggerClassName="h-11 border-2 rounded-xl bg-background font-semibold"
          />
          ...
          <Button 
            variant="outline" 
            size="icon" 
            className="h-11 w-11 border-2 rounded-xl bg-background"
          ...
          {selectedIds.length > 0 ? (
            <Button 
              variant="destructive" 
              className="h-11 px-4 rounded-xl font-bold animate-in fade-in slide-in-from-right-2"
            ...
            >
             ...
             <Button 
              variant="outline" 
              className="h-11 px-4 rounded-xl text-muted-foreground hover:text-destructive hover:border-destructive border-2"
      ...
      {(initialData?.pagination?.totalPages ?? 0) > 1 && (
        <div className="flex items-center justify-center gap-3 py-8">
           <Button
            ...
            disabled={!initialData?.pagination?.hasPrevPage || isPending}
            onClick={() => handlePageChange((initialData?.pagination?.page ?? 1) - 1)}
          >
          ...
          <div className="flex items-center gap-2">
             ...
             <Badge variant="secondary" className="px-3 h-8 text-sm rounded-lg border-2">
