    ...
    <Card className={cn(
      ...
      <CardHeader className="p-4 pb-2 space-y-2">
        <div className="flex items-start justify-between">
          <Badge variant="outline" className={cn("capitalize", `bg-${config.color}-50 text-${config.color}-700 border-${config.color}-200`)}>
            {config.label}
          </Badge>
      ...
      <CardFooter className="p-4 pt-0 border-t bg-muted/20 flex items-center justify-between text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" onClick={() => onView(item)}>
