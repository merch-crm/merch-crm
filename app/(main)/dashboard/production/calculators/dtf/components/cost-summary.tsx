            ...
            <CardContent className="p-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Badge
                            variant={result.efficiencyPercent >= 75 ? "default" : "secondary"}
                ...
                <div className="mt-4 pt-4 border-t border-primary/10">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                        <CostItem
                            label="Цена за метр"
                            value={formatCurrency(result.pricePerMeter)}
                        />
                        <CostItem
                            label="Печать"
                            value={formatCurrency(result.printCost)}
                            percentage={getPercentage(result.printCost, result.totalCost)}
                        />
                        <CostItem
                            label="Нанесение"
                            value={formatCurrency(result.placementCost)}
                            percentage={getPercentage(result.placementCost, result.totalCost)}
                        />
                        <CostItem
                            label="Материалы"
                            value={formatCurrency(result.materialsCost)}
                            percentage={getPercentage(result.materialsCost, result.totalCost)}
                        />
                        {result.blanksCost !== undefined && result.blanksCost > 0 && (
                            <CostItem
                                label="Заготовки"
                                value={formatCurrency(result.blanksCost)}
                                percentage={getPercentage(result.blanksCost, result.totalCost)}
                            />
                        )}
                        <CostItem
                            label="Средняя за шт"
                            value={formatCurrency(result.avgCostPerPrint)}
                            highlight
                        />
                    </div>
                </div>
