                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <SortButton
                            ...
                            currentField={sortField}
                            direction={sortDirection}
                            onClick={() => handleSort("costPerPrint")}
                        />
                    </div>
                </div>
            </CardHeader>
                ...
                <div className="space-y-2">
                    {sortedSections.map((section) => (
                        <SectionRow
                            key={section.groupId}
                            section={section}
                            isExpanded={expanded === section.groupId}
                            onToggle={() => toggleExpanded(section.groupId)}
                        />
                    ))}
                </div>
            ...
            {isExpanded && (
                <div className="px-3 pb-3 pt-1 border-t border-slate-100">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                        ...
                        <DetailItem
                            label="Длина секции"
                            value={`${(section.sectionLengthMm / 1000).toFixed(3)} м`}
                        />
                        <DetailItem
                            label="Площадь"
                            value={`${section.sectionAreaM2.toFixed(4)} м²`}
                        />
                        <DetailItem
                            label="Нанесение"
                            value={section.placementCost > 0 ? formatCurrency(section.placementCost) : "—"}
                        />
                    </div>
                </div>
            )}
