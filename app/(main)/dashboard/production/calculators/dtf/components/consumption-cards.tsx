...
function StockStatusBadge({ status, available }: StockStatusBadgeProps) {
    ...
    const { variant, icon: StatusIcon, text } = config[status];

    // Using basic colors since Shadcn UI variant for 'success' isn't explicitly requested 
    // Usually 'default' or a standard brand color works well for success
    return (
        <Badge variant={variant} className="w-full justify-center text-xs gap-1">
