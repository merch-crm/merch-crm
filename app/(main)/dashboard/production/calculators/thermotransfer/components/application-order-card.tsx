 ...
 return (
  <Card>
    ...
    <div className="flex items-center justify-between">
     ...
     <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-1">
       ...
       <Input type="number" min="1" value={order.quantity} onChange={(e) => onUpdate({ 
         ...order, 
   ...
   <CardContent className="space-y-3">
    ...
    {order.applications.map((app, index) => (
     ...
     >
      ...
      <div className="flex-1 min-w-0 grid grid-cols-2 gap-2">
       <Select options={positionOptions.map((opt: { id: string; title: string }) => ({
         ...opt,
       ...
       <Select options={printOptions} value={app.printId} onChange={(v) => handleApplicationChange(index, 'printId', v)}
        compact
        variant="minimal"
       />
      </div>

      <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={() => handleRemoveApplication(index)}
      >
    ...
    <Button variant="outline" size="sm" className="w-full" onClick={handleAddApplication} disabled={ prints.length === 0 || order.applications.length>= availablePositions.length
     }
    >
