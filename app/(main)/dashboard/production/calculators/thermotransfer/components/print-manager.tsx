      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            {prints.length > 0 && (
              <Badge variant="secondary">{prints.length}</Badge>
            )}
          </CardTitle>
          ...
          <div className="p-4 border rounded-lg bg-muted/30 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  placeholder="Например: Логотип компании"
                  value={newPrint.name}
                  onChange={(e) => setNewPrint(prev => ({ ...prev, name: e.target.value }))}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  options={typeOptions}
                  value={newPrint.type}
                  onChange={(v) => setNewPrint(prev => ({ ...prev, type: v as PrintType }))}
                  compact
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  options={sizeOptions}
                  value={newPrint.sizeId}
                  onChange={(v) => setNewPrint(prev => ({ ...prev, sizeId: v }))}
                  compact
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  min="0"
                  value={newPrint.purchasePrice || ''}
                  onChange={(e) => setNewPrint(prev => ({ 
                    ...prev, 
