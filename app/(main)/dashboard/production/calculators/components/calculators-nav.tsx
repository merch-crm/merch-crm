  ...
  return (
    <nav className="flex items-center gap-1 p-1.5 bg-slate-100/50 rounded-[20px] border border-slate-200/30 shadow-inner overflow-x-auto">
      {NAV_ITEMS.filter((item) => item.enabled).map((item) => {
        ...
        return (
          <Link key={item.type} href={item.href} className={cn( "relative flex items-center gap-2 px-4 py-2.5 rounded-[14px] text-sm font-bold transition-colors whitespace-nowrap", isActive ? "text-primary" : "text-slate-500 hover:text-slate-700 hover:bg-white/50" )}>
            {isActive && (
              <motion.div
