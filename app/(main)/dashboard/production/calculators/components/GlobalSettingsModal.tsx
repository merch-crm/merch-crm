 ...
 return (
  <div className="space-y-2">
   ...
   <div className="relative group">
    <Input type="number" value={value} disabled={disabled} onChange={(e) => onChange(parseFloat(e.target.value) || 0)}
...
export function GlobalSettingsModal({
 ...
 const { items: _warehouseItems } = useWarehouseItems(calculatorType);
 ...
 return (
  <ResponsiveModal isOpen={isOpen} onClose={onClose} showVisualTitle={false} className="max-w-2xl bg-white rounded-[40px] overflow-hidden">
   <div className="flex flex-col bg-white">
    ...
    <div className="flex items-center justify-between p-10 pb-4">
     ...
     <Button variant="ghost" size="icon" onClick={onClose} className="h-10 w-10 rounded-full hover:bg-slate-50 text-slate-300">
    ...
    <div className="px-10 flex gap-3 mt-4">
       <ActionCard active={activeTab === 'main'} onClick={() => setActiveTab('main')} 
        icon={IconWallet} 
        label="Цены и Срочность"
        sublabel="Сколько мы берем"
       />
       <ActionCard active={activeTab === 'materials'} onClick={() => setActiveTab('materials')} 
        icon={IconBox} 
        label="Расходники"
    ...
    <div className="p-10 pt-8 space-y-3">
        ...
        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-400">
         ...
         <div className="space-y-3">
           ...
           <div className="grid grid-cols-2 gap-3 p-6 bg-slate-50/40 rounded-[32px] border border-slate-100/50">
             <SimpleInput label="Цена для клиента" description="Базовая цена за 1 м2" value={localSettings.printConfig.basePrice || 0} onChange={(v) => updatePrintConfig({ basePrice: v })}
              unit="₽/М2"
             />
             <SimpleInput label="Желаемая маржа" description="Ваша прибыль с заказа" value={localSettings.marginConfig.defaultMargin} onChange={(v) => updateMarginConfig({ defaultMargin: v })}
              unit="%"
             />
           </div>
         </div>
         ...
         <div className="space-y-3">
           ...
           <div className="grid grid-cols-2 gap-3">
             <div className="p-6 bg-white rounded-[28px] border border-slate-100 shadow-sm space-y-3 transition-all hover:border-slate-200">
              ...
              <SimpleInput value={localSettings.urgencyConfig.expressMarkup} onChange={(v) => updateUrgencyConfig({ expressMarkup: v })}
                unit="%"
              />
             </div>
             <div className="p-6 bg-white rounded-[28px] border border-slate-100 shadow-sm space-y-3 transition-all hover:border-slate-200">
              ...
              <SimpleInput value={localSettings.urgencyConfig.urgentMarkup} onChange={(v) => updateUrgencyConfig({ urgentMarkup: v })}
                unit="%"
              />
             </div>
           </div>
         ...
         <div className="p-6 bg-slate-50/40 rounded-[24px] border border-slate-100/50">
           <SimpleInput label="Минимальный чек (₽)" description="Для маленьких заказов" value={localSettings.printConfig.minOrderFee || 0} onChange={(v) => updatePrintConfig({ minOrderFee: v })}
            unit="₽"
           />
         </div>
        </div>
        ...
        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-400">
          ...
          <div className="grid gap-3">
            {localSettings.consumablesConfig?.items?.map((item: ConsumableItem) => (
              <ConsumableItemEditor key={item.id} item={item} defaultItem={item} // В реальности тут должен быть DEFAULT_CONSUMABLES showAdvanced={showAdvanced} warehouseItems={warehouseItems?.filter((w) => w.category === item.category || !item.category)}
                onChange={(updated) => {
                 setLocalSettings(prev => ({
                  ...prev,
                  consumablesConfig: {
     ...
     <div className="flex items-center justify-between pt-10 border-t border-slate-100 mt-2">
       <Button variant="ghost" onClick={onClose} className="text-xs font-black text-slate-400 hover:text-slate-900 transition-colors px-0">
        Отмена
       </Button>
       <div className="flex items-center gap-3">
         <Button variant="ghost" onClick={handleReset} className="h-16 w-16 rounded-[24px] text-slate-200 hover:text-indigo-600 transition-all border border-transparent hover:border-slate-100 group">
          <IconReset className="h-6 w-6 group-active:rotate-[-90deg] transition-transform duration-500" />
         </Button>
         <SubmitButton onClick={handleSave} isLoading={isSaving} loadingText="Сохранение..." className="h-16 min-w-[240px] rounded-[28px] bg-slate-900 hover:bg-slate-800 text-white font-black text-xl transition-all shadow-2xl shadow-slate-200 active:scale-95">
