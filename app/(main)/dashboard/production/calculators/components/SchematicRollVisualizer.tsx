...
export function SchematicRollVisualizer({
 ...
 return (
  <div className="crm-card border border-border/50 overflow-hidden transition-all hover:shadow-lg">
   <VisualizerHeader hasDesigns={designs.length> 0}
   ...
   {!hideControls && (
    <VisualizerControls settings={settings} updateSettings={updateSettings} readonly={readonly} showGrid={showGrid} setShowGrid={setShowGrid} scale={scale} baseScale={baseScaleRef.current} handleZoomIn={handleZoomIn} handleZoomOut={handleZoomOut} handleFitAll={handleFitAll} MIN_SCALE={MIN_SCALE} MAX_SCALE={MAX_SCALE} />
   )}
   ...
   >
    {designs.length === 0 ? (
     ...
     <div className="p-8 mx-auto w-fit origin-top">
      {/* Job Summary Overlay (Impeccable Pattern) */}
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 pointer-events-none">
       <div className="bg-card/80 backdrop-blur-md border border-border/40 p-3 rounded-2xl shadow-crm-md animate-in fade-in slide-in-from-right-4 duration-500">
        <div className="flex items-center gap-3">
          <div className={`h-2.5 w-2.5 rounded-full animate-pulse ${layoutResult.stats.efficiency > 85 ? 'bg-emerald-500' : 'bg-amber-500'}`} />
          <span className="text-xs font-bold text-foreground/80">Оптимизация</span>
          <span className="ml-auto text-xs font-bold font-mono">{layoutResult.stats.efficiency}%</span>
        </div>
        <div className="mt-2 h-1.5 w-full bg-muted rounded-full overflow-hidden">
         <div 
          className={`h-full transition-all duration-1000 ease-out ${layoutResult.stats.efficiency > 85 ? 'bg-emerald-500' : 'bg-amber-500'}`}
          style={{ width: `${layoutResult.stats.efficiency}%` }}
         />
        </div>
       </div>
      </div>
      ...
      <svg
       ...
       className="drop-shadow-2xl transition-all duration-500 ease-in-out"
      >
       <defs>
        <marker id="arrowhead" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto" markerUnits="strokeWidth">
         <path d="M0,0 L0,6 L6,3 z" fill="oklch(0.45 0.03 281.33)" />
        </marker>
        ...
        <clipPath id="roll-clip">
         <rect x={0} y={0} width={settings.rollWidthMm} height={rollHeight} rx={8} />
        </clipPath>
       </defs>

       {/* Film background */}
       <rect x={0} y={0} width={settings.rollWidthMm} height={rollHeight} fill="var(--card)" stroke="var(--border)" strokeWidth={1.5} rx={8} className="transition-all duration-500" />

       {/* Grid - Clipped to roll bounds */}
       {showGrid && (
        <g clipPath="url(#roll-clip)" opacity={0.15}>
         {gridLines.vertical.map((x) => (
          <line key={`grid-v-${x}`} x1={x} y1={0} x2={x} y2={rollHeight} stroke="oklch(0.45 0.03 281.33)" strokeWidth={x % 100 === 0 ? 1 : 0.5} strokeDasharray={x % 100 === 0 ? undefined : '4,4'} />
         ))}
         {gridLines.horizontal.map((y) => (
          <line key={`grid-h-${y}`} x1={0} y1={y} x2={settings.rollWidthMm} y2={y} stroke="oklch(0.45 0.03 281.33)" strokeWidth={y % 100 === 0 ? 1 : 0.5} strokeDasharray={y % 100 === 0 ? undefined : '4,4'} />
         ))}
        </g>
       )}
       ...
       {settings.edgeMarginMm > 0 && (
        <g opacity={0.1} clipPath="url(#roll-clip)">
         <rect x={0} y={0} width={settings.edgeMarginMm} height={rollHeight} fill="var(--dept-production)" />
         <rect x={settings.rollWidthMm - settings.edgeMarginMm} y={0} width={settings.edgeMarginMm} height={rollHeight} fill="var(--dept-production)" />
         <rect x={0} y={0} width={settings.rollWidthMm} height={settings.edgeMarginMm} fill="var(--dept-production)" />
         <rect x={0} y={rollHeight - settings.edgeMarginMm} width={settings.rollWidthMm} height={settings.edgeMarginMm} fill="var(--dept-production)" />
        </g>
       )}

       {/* Dimensions */}
       <g className="font-mono font-bold" style={{ fontSize: `${fontSizeDisplay}px` }}>
        <line x1={0} y1={-lineOffset} x2={settings.rollWidthMm} y2={-lineOffset} stroke="oklch(0.15 0.02 281.33)" strokeWidth={strokeW} markerStart="url(#arrowhead-rev)" markerEnd="url(#arrowhead)" />
        <text x={settings.rollWidthMm / 2} y={-textOffsetVertical} fill="oklch(0.15 0.02 281.33)" textAnchor="middle" stroke="none">{String(settings.rollWidthMm)} мм</text>
        <line x1={-lineOffset} y1={0} x2={-lineOffset} y2={rollHeight} stroke="oklch(0.15 0.02 281.33)" strokeWidth={strokeW} markerStart="url(#arrowhead-rev)" markerEnd="url(#arrowhead)" />
        <text x={-textOffsetHorizontal} y={rollHeight / 2} fill="oklch(0.15 0.02 281.33)" textAnchor="middle" transform={`rotate(-90, -${textOffsetHorizontal}, ${rollHeight / 2})`} stroke="none">{String(rollHeight)} мм</text>
       </g>
        ...
        return (
         <g key={`${placed.designId}-${placed.instanceIndex}`} onMouseEnter={() => setHoveredDesignId(placed.designId)} onMouseLeave={() => setHoveredDesignId(null)} className="cursor-pointer">
          <rect 
           ...
           fill={color.fill} stroke={isHovered ? 'var(--primary)' : color.border} 
           strokeWidth={isHovered ? 2 * invScale : 0.75 * invScale} rx={4} 
          ...
          {Math.max(placed.width, placed.height) > 28 && Math.min(placed.width, placed.height) > 12 && (() => {
           ...
           return (
            <text 
             ...
             className="pointer-events-none select-none transition-all duration-300 font-bold"
            >
             {showName && (
              <tspan x="0" dy="-0.5em" fontSize="0.75em" fontWeight="800" fillOpacity={0.9}>
             ...
             <tspan x="0" dy={showName ? "1.25em" : "0"} fontSize="0.95em" fillOpacity={1}>
              <tspan fontWeight="900">{dimsStr}</tspan>
