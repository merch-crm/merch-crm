    ...
    <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl group hover:bg-slate-100 transition-colors">
      ...
      <div className="flex-1 grid grid-cols-3 gap-2">
        ...
        <div className="relative">
          <Input
            type="text"
            value={tier.toMeters === null ? '∞' : String(tier.toMeters)}
    ...
    <div className="space-y-3">
        ...
        {existingWidths.length > 1 && (
          <div className="flex gap-1.5 ml-auto">
            {existingWidths.map(width => (
              <Badge
                key={width}
                variant={width === selectedRollWidth ? 'default' : 'secondary'}
                className="cursor-pointer"
                onClick={() => setSelectedRollWidth(width)}
              >
                {width} мм
              </Badge>
            ))}
      ...
      <div className="space-y-2">
        {filteredTiers.map((tier, index) => (
          <TierRow
            key={`${tier.rollWidthMm}-${tier.fromMeters}`}
            tier={tier}
            index={index}
            isLast={index === filteredTiers.length - 1}
            onUpdate={handleTierUpdate}
            onDelete={handleTierDelete}
            canDelete={filteredTiers.length > 1}
          />
        ))}
      </div>
