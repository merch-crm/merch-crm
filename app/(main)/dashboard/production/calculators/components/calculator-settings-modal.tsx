...
export const CalculatorSettingsModal = memo(function CalculatorSettingsModal({
 ...
 return (
  <ResponsiveModal isOpen={isOpen} onClose={handleClose} title={`Настройки калькулятора — ${APPLICATION_TYPE_LABELS[applicationType]}`} footer={footer}>
    ...
    <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabValue)}>
     <div className="crm-filter-tray mb-4">
      <TabsList className="w-full grid grid-cols-3 bg-transparent">
       {tabConfig.map(tab => (
        <TabsTrigger key={tab.value} value={tab.value} className="relative data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-2xl">
     ...
     <TabsContent value="pricing" className="mt-0">
      ...
      >
       <MeterPricingEditor tiers={meterPricing} applicationType={applicationType} onUpdate={handleMeterPricingUpdate} isLoading={isSaving} />
      </motion.div>
     </TabsContent>
     ...
     <TabsContent value="placements" className="mt-0">
      <motion.div
       ...
       transition={{ duration: 0.2 }}
      >
       <PlacementsEditor placements={placements} applicationType={applicationType} onUpdate={handlePlacementsUpdate} isLoading={isSaving} />
      </motion.div>
     </TabsContent>
     ...
     <TabsContent value="consumables" className="mt-0">
      <motion.div
       ...
       transition={{ duration: 0.2 }}
      >
       <ConsumablesEditor config={consumables} applicationType={applicationType} onUpdate={handleConsumablesUpdate} isLoading={isSaving} />
      </motion.div>
     </TabsContent>
    </Tabs>
   </div>

   <ConfirmDialog isOpen={isConfirmCloseOpen} onClose={() => setIsConfirmCloseOpen(false)}
    onConfirm={() => {
