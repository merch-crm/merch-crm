  ...
  <div className={cn('w-full', className)}>
   ...
   <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
    ...
    <div className="sm:col-span-2 space-y-1.5">
     ...
     <Input placeholder={term.placeholder} value={name} onChange={(e) => { setName(e.target.value); setErrors(p => ({ ...p, name: '' })); }}
    ...
    <div className="space-y-1.5">
     ...
     <Input type="number" min={1} placeholder="1" value={quantity} onChange={(e) => { setQuantity(e.target.value); setErrors(p => ({ ...p, quantity: '' })); }}
      onKeyDown={handleKeyDown}
      className={cn(
       "bg-slate-50/80 border-slate-200 rounded-2xl shadow-none hover:border-slate-300 hover:bg-slate-100/50 transition-all h-12 text-sm placeholder:text-slate-400", 
    ...
    <div className="space-y-1.5">
     ...
     <Input type="number" min={1} placeholder="напр. 200" value={widthMm} onChange={(e) => { setWidthMm(e.target.value); setErrors(p => ({ ...p, widthMm: '' })); }}
      onKeyDown={handleKeyDown}
      className={cn(
       "bg-slate-50/80 border-slate-200 rounded-2xl shadow-none hover:border-slate-300 hover:bg-slate-100/50 transition-all h-12 text-sm placeholder:text-slate-400", 
    ...
    <div className="space-y-1.5">
     ...
     <Input type="number" min={1} placeholder="напр. 300" value={heightMm} onChange={(e) => { setHeightMm(e.target.value); setErrors(p => ({ ...p, heightMm: '' })); }}
      onKeyDown={handleKeyDown}
      className={cn(
       "bg-slate-50/80 border-slate-200 rounded-2xl shadow-none hover:border-slate-300 hover:bg-slate-100/50 transition-all h-12 text-sm placeholder:text-slate-400", 
    ...
    {calculatorType === 'embroidery' && (
     <div className="sm:col-span-3 space-y-1.5">
      ...
      <Input type="number" min={1} placeholder="напр. 5000" value={stitchCount} onChange={(e) => setStitchCount(e.target.value)}
       onKeyDown={handleKeyDown}
   ...
   <div className="flex justify-center mt-6 pt-4 border-t border-slate-100">
    <Button type="button" onClick={handleAdd} className="rounded-xl h-10 px-8 font-bold bg-slate-900 hover:bg-slate-800 text-white transition-all shadow-sm flex items-center gap-2">
