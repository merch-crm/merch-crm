...
export function CreateWarehouseItemModal({
 ...
 return (
  <ResponsiveModal isOpen={isOpen} onClose={onClose} title="Новый материал на складе" description="Быстрое добавление материала для использования в калькуляторах" footer={ <div className="flex justify-end gap-3 w-full">
     <Button variant="ghost" onClick={onClose} className="rounded-md">
      Отмена
     </Button>
     <SubmitButton onClick={handleSubmit} isLoading={isLoading} loadingText="Создание..." className="rounded-md">
  ...
  >
   <div className="space-y-3 p-6">
    ...
    <div className="space-y-2">
     ...
     <Select value={formData.category} onChange={(v) => handleCategoryChange(v as ConsumableCategory)}
    ...
    <div className="grid grid-cols-2 gap-3">
     <div className="space-y-2">
      ...
      <Input type="number" value={formData.stock} onChange={(e) =>
