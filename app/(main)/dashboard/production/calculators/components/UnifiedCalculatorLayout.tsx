  ...
  <div className="crm-card flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-none hover:shadow-none p-4 sm:p-6">
   ...
   <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-between sm:justify-end">
    ...
    <div className="flex items-center gap-2 shrink-0">
     {showSettings && (
      <Button variant="outline" size="icon" onClick={onSettingsClick} className="rounded-xl h-10 w-10 sm:h-12 sm:w-12 border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all shadow-sm shrink-0" title="Настройки расходников">
...
export function CalculatorActions({
 ...
 return (
  <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
   <Button onClick={onSave} disabled={!canSave || isLoading} className="rounded-xl h-10 sm:h-11 px-4 sm:px-6 font-bold bg-slate-900 hover:bg-slate-800 text-white transition-all shadow-sm flex items-center gap-2 flex-1 sm:flex-initial text-sm sm:text-base">
    <Save className="h-4 w-4 shrink-0" />
    <span className="truncate">Сохранить</span>
   </Button>

   {onDownloadClient && (
    <Button variant="outline" onClick={onDownloadClient} disabled={!canSave || isLoading} className="rounded-xl h-10 sm:h-11 px-4 sm:px-6 font-bold border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 transition-all shadow-sm flex items-center gap-2 flex-1 sm:flex-initial text-sm sm:text-base">
   ...
   {onPrint && (
    <Button variant="outline" onClick={onPrint} disabled={!canSave || isLoading} className="rounded-xl h-10 w-10 sm:h-11 sm:w-11 font-bold border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 transition-all shadow-sm flex items-center justify-center shrink-0" title="Печать">
...
export function UnifiedCalculatorLayout({
 ...
 return (
  <div 
   ...
   suppressHydrationWarning
  >
   <CalculatorHeader calculatorType={calculatorType} onSettingsClick={onSettingsClick} headerActions={headerActions} />
