  ...
  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
   <div className="flex flex-wrap items-center gap-3 flex-1 min-w-0">
    <div className="flex items-center gap-3 shrink-0">
     ...
     <Select value={settings.rollWidthMm.toString()} onChange={(v) => updateSettings({ rollWidthMm: parseInt(v, 10) })}
      disabled={readonly}
      ...
      <PopoverContent className="w-64 p-5 rounded-[24px] border border-slate-200 bg-white/95 backdrop-blur-xl shadow-crm-xl animate-in fade-in zoom-in-95 duration-200 space-y-3">
       <div className="space-y-3">
        <div className="space-y-3">
         ...
         <Slider value={[settings.edgeMarginMm]} onValueChange={([v]) => updateSettings({ edgeMarginMm: v })}
          min={0}
          max={50}
          step={1}
          className="py-2"
        ...
        <div className="space-y-3">
         ...
         <Slider value={[settings.gapMm]} onValueChange={([v]) => updateSettings({ gapMm: v })}
          min={0}
          max={50}
          step={1}
          className="py-2"
    ...
    <div className="flex items-center gap-3 shrink-0">
     <div className="flex items-center gap-3">
      ...
      <Switch checked={settings.allowRotation} onCheckedChange={(v) => updateSettings({ allowRotation: v })} 
       disabled={readonly} 
       className="scale-90"
      />
     </div>
     <div className="flex items-center gap-3">
      ...
      <Switch checked={showGrid} onCheckedChange={setShowGrid} className="scale-90" />
     </div>
    </div>
