  ...
  >
   <div className="flex flex-col sm:flex-row gap-3 p-[var(--radius-padding)]">
    ...
    <div className="flex gap-3 items-center">
     ...
     >
      {isManual ? (
       ...
       <Image src={file.thumbnailUrl || file.embroideryData?.svgPreview || ''} alt={file.originalName} fill className={cn( "object-cover", file.embroideryData?.svgPreview && !file.thumbnailUrl && "p-2 bg-white" )} unoptimized />
      ) : (
    ...
    {!isManual && (file.thumbnailUrl || file.embroideryData?.svgPreview) && (
     <ModernImageGallery isOpen={galleryOpen} onClose={() => setGalleryOpen(false)}
      images={[{ 
    ...
    <div className="flex-1 min-w-0 space-y-2">
     ...
     <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
      <div className="space-y-1">
       ...
       <Input type="number" className="h-8 text-xs font-semibold bg-background/50 focus:bg-background transition-colors" value={file.userDimensions?.widthMm || ''} onChange={(e) => {
      ...
      <div className="space-y-1">
       ...
       <Input type="number" className="h-8 text-xs font-semibold bg-background/50 focus:bg-background transition-colors" value={file.userDimensions?.heightMm || ''} onChange={(e) => {
         const val = e.target.value === '' ? 0 : Number(e.target.value);
         onUpdate(file.id, { 
          userDimensions: { 
           ...file.userDimensions!, 
      ...
      <div className="space-y-1">
       ...
       <Input type="number" className="h-8 text-xs font-semibold bg-background/50 focus:bg-background transition-colors" value={file.quantity || ''} onChange={(e) => {
      ...
      {file.calculatorType === 'embroidery' && (
       <div className="space-y-1">
        ...
        <Input type="number" className="h-8 text-xs font-semibold bg-primary/5 border-primary/20 text-primary" value={file.embroideryData?.stitchCount || ''} onChange={(e) => 
          onUpdate(file.id, { 
           embroideryData: { 
            ...file.embroideryData!, 
