...
function SortableProductCard({
 ...
 return (
  <div ref={setNodeRef} style={style} className="relative group">
   ...
   <PlacementProductCard product={product} viewMode={viewMode} onEdit={onEdit} onDelete={onDelete} onDuplicate={onDuplicate} isDeleting={isDeleting} />
  </div>
 );
}
  ...
  <div className="space-y-3">
   ...
   <div className="flex flex-col lg:flex-row lg:items-center gap-3 sticky top-0 z-10 py-2 bg-slate-50/80 backdrop-blur-md -mx-2 px-2 rounded-xl border border-transparent hover:border-slate-200/50 transition-all">
    ...
    <div className="flex flex-wrap items-center gap-3">
     <Select value={filterType} onChange={(v) => setFilterType(v as PlacementProductType | 'all')}
      options={typeOptions}
      className="w-56"
      compact
     />
     ...
     <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-[12px] p-1 shadow-sm">
      <Button variant={viewMode === 'grid' ? 'solid' : 'ghost'} size="icon" onClick={() => setViewMode('grid')}
       className="h-9 w-9 rounded-[8px]"
      >
       <LayoutGrid className="h-4 w-4" />
      </Button>
      <Button variant={viewMode === 'list' ? 'solid' : 'ghost'} size="icon" onClick={() => setViewMode('list')}
       className="h-9 w-9 rounded-[8px]"
      >
   ...
   {filteredProducts.length === 0 ? (
    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
     <SortableContext items={(filteredProducts || []).map((p: PlacementProduct) => p.id)}
      strategy={
     ...
     >
      ...
      >
       {(filteredProducts || []).map((product) => (
        <SortableProductCard key={product.id} product={product} viewMode={viewMode} onEdit={() => handleEdit(product)}
   ...
   <PlacementProductModal isOpen={isModalOpen} onClose={() => {
   ...
   >
    <AlertDialogContent className="rounded-[24px] border-none shadow-2xl p-8 max-w-md">
     ...
     <AlertDialogFooter className="mt-8 gap-3 sm:gap-0">
      ...
      <AlertDialogAction onClick={handleDeleteConfirm} className="h-12 px-6 rounded-xl bg-rose-500 text-white font-bold hover:bg-rose-600 shadow-lg shadow-rose-200/50 transition-all border-none" disabled={isDeleting}>
