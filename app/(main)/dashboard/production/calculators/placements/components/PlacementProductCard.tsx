...
export function PlacementProductCard({
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="rounded-[10px] h-9 w-9 bg-white border border-slate-200 shadow-sm hover:border-slate-300 transition-all"
        >
      ...
      <DropdownMenuContent align="end" className="w-56 p-2 rounded-[14px]">
        ...
        <DropdownMenuItem
          onClick={onDelete}
          disabled={isDeleting}
          className="rounded-[8px] h-10 gap-2 text-rose-500 focus:text-rose-600 focus:bg-rose-50 font-bold"
        >
  ...
  if (viewMode === 'list') {
    return (
      <Card className={cn(
        "rounded-[16px] border-slate-200 transition-all hover:border-slate-300 hover:shadow-md",
        !product.isActive && "bg-slate-50/50 grayscale-[0.5] opacity-80"
      )}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                {!product.isActive && (
                  <Badge variant="secondary" className="text-xs h-5 bg-slate-200 text-slate-600 border-none font-black">
                    Скрыт
                  </Badge>
                )}
              </div>
  ...
  return (
    <Card className={cn(
      "rounded-[24px] border-slate-200 transition-all hover:border-slate-300 hover:shadow-xl group",
      !product.isActive && "bg-slate-50/50 grayscale-[0.5] opacity-80"
    )}>
      ...
      <CardContent className="p-6 space-y-3">
        {/* Статус */}
        {!product.isActive && (
          <Badge variant="secondary" className="w-full justify-center text-xs h-7 bg-slate-100 text-slate-500 border-none font-bold">
          ...
          <div className="flex flex-wrap gap-1.5">
            {activeAreas.slice(0, 4).map((area) => (
              <Badge
                key={area.id}
                variant="outline"
                className="text-xs font-bold py-0.5 px-2 bg-slate-50/50 border-slate-200 text-slate-600 rounded-lg"
              >
                {area.name}
              </Badge>
            ))}
            {activeAreas.length > 4 && (
              <Badge variant="outline" className="text-xs font-black py-0.5 px-2 bg-indigo-50 border-indigo-100 text-indigo-600 rounded-lg">
                +{activeAreas.length - 4}
              </Badge>
            )}
