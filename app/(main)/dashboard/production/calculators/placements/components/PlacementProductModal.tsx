     ...
     <div className="grid gap-3 py-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
       <div className="md:col-span-2 space-y-2">
        ...
        <Input className="h-12 rounded-2xl bg-slate-50 border-slate-200 focus:bg-white transition-all font-bold text-lg" placeholder="Например: Футболка Premium (Cotton 100%)" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
        />
       </div>

       <div className="space-y-2">
        <Label className="text-xs font-bold text-slate-500 ml-1">Тип изделия</Label>
        <Select value={formData.type} onChange={(val) => setFormData({ ...formData, type: val as PlacementProductType })}
         options={typeOptions}
         className="h-12"
        />
       </div>
       ...
       <div className="md:col-span-2 space-y-2">
        ...
        <Textarea placeholder="Дополнительные примечания к изделию для сотрудников..." className="resize-none h-24 rounded-2xl bg-slate-50 border-slate-200 focus:bg-white transition-all font-medium" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
        />
       </div>
      ...
      <div className="space-y-3">
       ...
       <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={formData.areas.map((_, i) => `area-${i}`)}
         ...
         <div className="space-y-3 pb-8">
          {formData.areas.length === 0 ? (
           ...
           formData.areas.map((area, index) => (
            <SortableAreaItem key={`area-${index}`} id={`area-${index}`} index={index} area={area} onUpdate={updateArea} onRemove={removeArea} />
           ))
          )}
         </div>
        </SortableContext>
