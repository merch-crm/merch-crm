    ...
    <div role="button" tabIndex={0} className="flex items-center gap-3" onClick={e => e.stopPropagation()}>
     ...
     <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl border border-slate-200">
      ...
      <div className="flex flex-col items-center px-2 min-w-[3rem]">
       ...
       <Input type="number" value={order.quantity === 0 ? '' : order.quantity} onChange={(e) => handleQuantityChange(e.target.value === '' ? 0 : parseInt(e.target.value) || 0)}
        ...
        {order.positions.map(pos => {
         ...
         return (
          ...
          >
           ...
           <Select value={pos.designId} onChange={(value) => handleUpdatePosition(pos.positionId, { designId: value })}
      ...
      {availablePositions.length > 0 && designs.length > 0 && (
       <div className="flex items-center gap-3">
        <div className="flex-1">
          <Select value="" onChange={handleAddPosition} options={positionOptions} placeholder="+ Выберите место нанесения" />
        </div>
       </div>
      )}
