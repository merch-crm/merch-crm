...
export function ProductionBoard({ items }: ProductionBoardProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 overflow-x-auto pb-4 custom-scrollbar lg:overflow-visible">
                    ...
                    >
                      ...
                      {item.order.attachments && item.order.attachments.length > 0 && (
                        <div className="mb-4 relative group/mockup overflow-hidden rounded-[20px] bg-slate-100 border border-slate-200 aspect-video shadow-inner">
                          <Image src={item.order.attachments[0].fileUrl} alt="Mockup" fill className="object-cover transition-transform duration-700 group-hover/mockup:scale-110" />
                          <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover/mockup:opacity-100 transition-all duration-300 flex flex-col items-center justify-center p-4">
                            <Button variant="ghost" onClick={(e: React.MouseEvent) => {
                                e.stopPropagation();
                      ...
                      <div className="flex items-center justify-between mb-5">
                        ...
                        <ProductionTimer orderItemId={item.id} stage={stage.id} />
                      ...
                      <div className="flex gap-2 pt-4 border-t border-slate-100">
                        {currentStatus === 'pending' && (
                          <Button variant="solid" color="purple" onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleStageUpdate(item.id, stage.id, 'in_progress'); }}
                        ...
                        {currentStatus === 'in_progress' && (
                          <Button variant="solid" color="purple" onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleStageUpdate(item.id, stage.id, 'done'); }}
                            disabled={loading === item.id}
                        ...
                        <DefectDialog orderItemId={item.id} maxQuantity={item.quantity} itemName={item.description || "Товар"} />
                      </div>
                    </div>
                  );
                })
      ...
      <ModernImageGallery images={[{ src: lightboxImage, label: "Вложение" }]} isOpen={!!lightboxImage} onClose={() => setLightboxImage(null)}
        itemName="Макет"
      />
    </div>
  );
