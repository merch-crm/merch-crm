...
function DeadlineRow({ deadline }: DeadlineRowProps) {
 ...
 return (
  <Link href={`/dashboard/production/tasks?dueDate=${deadline.date}`} className={cn( "flex items-center gap-3 p-3 rounded-xl border transition-all", "hover:shadow-md hover:-translate-y-0.5 group", isOverdue ? "bg-rose-50/50 border-rose-200" : isDateToday ? "bg-amber-50/50 border-amber-200" : "bg-white border-slate-200" )}>
   {/* Дата */}
   <div className={cn(
