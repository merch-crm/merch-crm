...
export function AttentionCard({ stats, className }: AttentionCardProps) {
 ...
 return (
  ...
  >
    ...
    {hasIssues ? (
     <>
      ...
      <div className="flex flex-wrap gap-2 mt-3">
       {overdue > 0 && (
        <Link href="/dashboard/production/tasks?filter=overdue" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-rose-100 text-rose-700 text-xs font-bold hover:bg-rose-200 transition-colors">
         <AlertTriangle className="w-3.5 h-3.5" />
         <span>{overdue} {pluralize(overdue, "просрочена", "просрочены", "просрочено")}</span>
        </Link>
       )}
       {urgent > 0 && (
        <Link href="/dashboard/production/tasks?filter=urgent" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-100 text-amber-700 text-xs font-bold hover:bg-amber-200 transition-colors">
   ...
   {/* Ссылка */}
   {hasIssues && (
    <Link href="/dashboard/production/tasks?filter=attention" className={cn( "absolute bottom-4 right-4 w-10 h-10 rounded-full flex items-center justify-center", "bg-rose-100 text-rose-600 hover:bg-rose-200 transition-colors", "opacity-0 group-hover:opacity-100" )}>
