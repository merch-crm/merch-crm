...
export function NotificationsCard({ data, onDismiss, className }: NotificationsCardProps) {
 ...
 return (
  <div className={cn(
    ...
    {!hasData ? (
     ...
     <div className="space-y-2">
      {(notifications || []).slice(0, 4).map((notification) => (
       <NotificationRow key={notification.id} notification={notification} onDismiss={onDismiss} />
      ))}
     </div>
    )}
   </div>
   ...
   {hasData && (
    <div className="mt-4">
     <Link href="/dashboard/notifications" className="flex items-center justify-between gap-2 p-3 rounded-xl bg-slate-50 text-xs font-bold text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-all group">
 ...
 return (
  <div className={cn(
   ...
   <div className="absolute right-2 top-2 flex items-center gap-1">
    {notification.href && (
     <Link href={notification.href} className={cn( "p-1.5 rounded-lg transition-all opacity-0 group-hover:opacity-100 hover:bg-white shadow-sm", priority.color )}>
