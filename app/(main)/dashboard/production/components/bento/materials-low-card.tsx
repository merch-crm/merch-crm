 ...
 return (
  <div className={cn(
   ...
   <div className="flex items-center justify-between mb-4">
    ...
    <Link href="/dashboard/warehouse/items?filter=low" className="text-xs font-bold text-primary hover:text-primary/80 flex items-center gap-1 transition-colors">
     <span>На склад</span>
...
function MaterialRow({ material }: MaterialRowProps) {
 ...
 return (
  <Link href={`/dashboard/warehouse/items/${material.id}`} className={cn( "block p-3 rounded-xl border transition-all", "hover:shadow-md hover:-translate-y-0.5 group", isCritical ? "bg-rose-50/50 border-rose-100" : isWarning ? "bg-amber-50/50 border-amber-100" : "bg-slate-50 border-slate-100" )}>
