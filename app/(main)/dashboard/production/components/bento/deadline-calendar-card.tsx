 ...
 return (
  <div className={cn("crm-card flex flex-col", className)}>
   ...
   <div className="flex-1">
    ...
    <div className="grid grid-cols-7 gap-1">
     ...
     {calendarDays.days.map((day) => {
      ...
      return (
       <CalendarDay key={dateStr} day={day} dateStr={dateStr} deadline={deadline} hasDeadline={hasDeadline} isCurrentDay={isCurrentDay} isPast={isPast} isSelected={isSelected} isWeekend={isWeekend} onClick={() => handleDayClick(dateStr)}
       />
      );
     })}
    </div>
   ...
   <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-50">
    ...
    <Link href="/dashboard/production/tasks?view=calendar" className="text-xs font-bold text-primary hover:text-primary/80 flex items-center gap-1 transition-colors">
 ...
 if (hasDeadline && deadline) {
  return (
   <Popover open={isSelected} onOpenChange={(open) => !open && onClick()}>
    ...
    <PopoverContent className="w-72 p-0" align="center" sideOffset={4}>
 ...
 return (
  <div>
   ...
   <div className="p-2 border-t border-slate-100">
    <Link href={`/dashboard/production/tasks?dueDate=${deadline.date}`} className="flex items-center justify-center gap-1 py-2 rounded-lg bg-slate-50 text-xs font-bold text-slate-600 hover:bg-slate-100 transition-colors">
...
function OrderRow({ order }: OrderRowProps) {
 ...
 return (
  <Link href={`/dashboard/production/tasks/${order.id}`} className="flex items-center gap-3 p-2 hover:bg-slate-50 transition-colors">
   <div className={cn(
