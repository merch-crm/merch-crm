...
export function WarehouseMaterialPicker({
 ...
 return (
  <ResponsiveModal isOpen={open} onClose={handleClose} title="Выбор материала со склада" footer={ <div className="flex gap-3 justify-end w-full">
     <Button variant="outline" onClick={handleClose}>
      Отмена
     </Button>
     <Button onClick={handleSelect} disabled={!selectedMaterial || quantity <= 0} className="gap-2">
    ...
    <div className="p-4 space-y-3">
     ...
     <div className="flex gap-3">
      <div className="relative flex-1">
       ...
       <Input placeholder="Поиск по названию или артикулу..." value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))}
        className="pl-9"
       />
       {filters.search && (
        <button
      ...
      <Select options={[ { id: "all", title: "Все категории" }, ...categories.map(cat => ({
         id: cat.id,
         title: cat.name,
         badge: cat.count
        }))
     ...
     <ScrollArea className="h-[300px] border rounded-xl">
      {fetchState.loading ? (
       ...
       <div className="flex flex-col items-center justify-center h-full text-center p-4">
        ...
        <Button type="button" variant="outline" size="sm" onClick={loadMaterials} className="mt-3">
         Повторить
        </Button>
       </div>
     ...
     {selectedMaterial && (
      <div className="p-4 bg-slate-50 rounded-xl space-y-3">
       ...
       <div className="flex items-center gap-3">
        ...
        <Input type="number" min={1} max={selectedMaterial.quantity} value={quantity} onChange={(e) => setQuantity(Number(e.target.value))}
         className="w-24"
        />
