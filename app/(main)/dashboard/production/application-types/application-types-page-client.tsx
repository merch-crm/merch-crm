  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Всего типов" value={stats.total} icon={<Layers className="w-4 h-4" />} 
          iconBg="bg-blue-50" 
        ...
        {Object.entries(stats.byCategory).slice(0, 2).map(([category, count]) => (
          <StatCard key={category} label={categoryLabels[category]} value={count} icon={categoryIcons[category]} />
        ))}
      </div>
      ...
      {filteredTypes.length > 0 ? (
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={filteredTypes.map((t) => t.id)}
            strategy={rectSortingStrategy}
            disabled={!isSorting}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {filteredTypes.map((type) => (
                <ApplicationTypeCard key={type.id} type={type} isSorting={isSorting} onUpdate={(updated) => {
      ...
      {/* Create Dialog */}
      <ApplicationTypeFormDialog open={isCreateOpen} onOpenChange={setIsCreateOpen} onSuccess={handleCreateSuccess} />
    </div>
  );
}
