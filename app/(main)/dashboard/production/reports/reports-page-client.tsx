    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={loadReportData} disabled={isLoading} className="rounded-xl">
            <RefreshCw className={cn("h-4 w-4 mr-2", isLoading && "animate-spin")} />
            Обновить
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} disabled={isExporting} className="rounded-xl">
            <FileText className="h-4 w-4 mr-2" />
            PDF
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportExcel} disabled={isExporting} className="rounded-xl">
      ...
      {metrics && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <MetricCard title="Выполнено" value={metrics.totalCompleted} suffix="шт" icon={CheckCircle} trend={metrics.trend} color="emerald" />
          <MetricCard title="В работе" value={metrics.totalInProgress} suffix="шт" icon={Clock} color="blue" />
          <MetricCard title="Брак" value={metrics.totalDefects} suffix="шт" icon={AlertTriangle} trend={-metrics.defectRate} color="rose" invertTrend />
          <MetricCard title="В срок" value={metrics.onTimeDeliveryRate} suffix="%" icon={TrendingUp} color="violet" />
        </div>
      )}
