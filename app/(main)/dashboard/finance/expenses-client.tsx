    ...
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="crm-card flex-1 flex min-h-0 gap-3 px-8 pb-8 pt-4 items-center shadow-sm">
        <div className="relative flex-1 w-full group">
          ...
          <Input type="text" placeholder="Поиск по описанию или категории..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
      ...
      {isAdding && (
        <AddExpenseDialog onClose={() => setIsAdding(false)}
          onSuccess={() => {
            setIsAdding(false);
            router.refresh();
          }}
    ...
    <ResponsiveModal isOpen={true} onClose={onClose} title="Новый расход">
      <form onSubmit={handleSubmit} className="p-4 space-y-3">
        <div className="space-y-1.5">
          ...
          {isClient ? (
            <Input name="date" type="date" defaultValue={todayDate} required className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-rose-500 outline-none font-bold text-sm" />
          ) : (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Select name="category" options={[ { id:"purchase", title:"Закупки" }, { id:"rent", title:"Аренда" }, { id:"salary", title:"Зарплата" }, { id:"tax", title:"Налоги" }, { id:"other", title:"Прочее" } ]} value={category} onChange={setCategory} />
          </div>
          <div className="space-y-1.5">
