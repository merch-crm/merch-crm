...
export function SalaryClient({ salaryData }: SalaryClientProps) {
 ...
 <div className="text-xs font-black text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100/50">Авто-расчет</div>
 </div>
 </div>
 <div className="mt-auto relative z-10">
 <p className="text-slate-400 text-xs font-black leading-none mb-3 opacity-70">Общий Фонд (ФОТ)</p>
 <div className="text-4xl font-black text-slate-900 leading-none">
 {salaryData.totalBudget.toLocaleString('ru-RU')} <span className="text-base font-bold text-slate-400 ml-1">{currencySymbol} <span className="text-xs opacity-60">/мес</span></span>
 </div>
 </div>
 </motion.div>
 ...
 <div className="text-right">
 <div className="text-xs font-black text-slate-400 opacity-60">Активный штат</div>
 </div>
 </div>
 <div className="mt-auto relative z-10">
 <p className="text-slate-400 text-xs font-black leading-none mb-3 opacity-70">Сотрудников в базе</p>
 <div className="text-4xl font-black text-slate-900 leading-none">
 ...
 <div className="text-right">
 <div className="text-xs font-black text-white/80 bg-white/10 px-3 py-1.5 rounded-full border border-white/20">KPI Активен</div>
 </div>
 </div>
 <div className="mt-auto relative z-10 text-white">
 <p className="text-white/60 text-xs font-black leading-none mb-3 opacity-70">Средняя выплата</p>
 ...
 <p className="text-xs font-black text-slate-400 mb-1">{payload[0].payload?.name}</p>
 ...
 <div className="flex gap-3 p-4 rounded-3xl bg-slate-50 border border-slate-100 hover:border-primary/20 transition-colors group">
 ...
 <p className="text-xs font-bold text-slate-500 mt-1">Фиксированная ставка за полный рабочий месяц согласно штатному расписанию.</p>
 </div>
 </div>
 <div className="flex gap-3 p-4 rounded-3xl bg-slate-50 border border-slate-100 hover:border-emerald-200 transition-colors group">
 ...
 <p className="text-xs font-bold text-slate-500 mt-1">Премия в размере <span className="text-emerald-600 font-black">300 {currencySymbol}</span> за каждый успешно завершенный заказ.</p>
 </div>
 </div>
 </div>
 <div className="space-y-3">
 <div className="flex gap-3 p-4 rounded-3xl bg-primary/5 border border-primary/10 hover:border-primary/30 transition-colors group">
 <div className="h-10 w-10 shrink-0 rounded-2xl bg-white flex items-center justify-center text-primary shadow-sm border border-slate-100 group-hover:scale-110 transition-transform">
 <Percent className="w-4 h-4" />
 </div>
 <div>
 <h4 className="text-sm font-black text-slate-900">Удержания</h4>
 <p className="text-xs font-bold text-slate-500 mt-1">Штрафы за брак или нарушение сроков вычитаются из бонусной части.</p>
 </div>
 </div>
 <div className="flex gap-3 p-4 rounded-3xl bg-amber-50 border border-amber-100 hover:border-amber-200 transition-colors group">
 ...
 <p className="text-xs font-bold text-slate-500 mt-1">Осуществляются 5-го и 20-го числа каждого месяца согласно ТК.</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <span className="text-xs font-black text-primary">{emps.length} {pluralize(emps.length, 'сотрудник', 'сотрудника', 'сотрудников')}</span>
 </div>
 </div>
 </div>
 <div className="text-right">
 <div className="text-xs font-black text-slate-400 mb-1 opacity-60">Месячный ФОТ</div>
 ...
 <tr>
 <th className="px-8 py-5 text-left text-xs font-black text-slate-400 border-b border-slate-100">Сотрудник</th>
 <th className="px-4 py-5 text-center text-xs font-black text-slate-400 border-b border-slate-100">Роль</th>
 <th className="px-4 py-5 text-center text-xs font-black text-slate-400 border-b border-slate-100">Оклад</th>
 <th className="px-4 py-5 text-center text-xs font-black text-slate-400 border-b border-slate-100">KPI & Бонусы</th>
 <th className="px-8 py-5 text-right text-xs font-black text-slate-400 border-b border-slate-100">К выплате</th>
 </tr>
 </thead>
 ...
 <div className="flex items-center gap-3">
 <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 font-bold text-xs group-hover:scale-105 transition-transform">
 {emp.name.split(' ').map(n => n[0]).join('')}
 </div>
 <div>
 <div className="font-black text-slate-900 text-[14px] leading-tight">{emp.name}</div>
 <div className="text-xs text-primary font-black mt-0.5 opacity-60">ID-{emp.id.split('-')[0]}</div>
 </div>
 </div>
 </td>
 <td className="px-4 py-5 text-center">
 <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-black whitespace-nowrap">
 {emp.role}
 </span>
 </td>
 ...
 renderCard={(emp) => (
 <div key={emp.id} className="p-6 flex flex-col gap-3 active:bg-slate-50 transition-colors">
 ...
 <span className="text-xs font-black text-slate-400 block mb-1">Оклад</span>
 ...
 <span className="text-xs font-black text-emerald-600 block mb-1">KPI</span>
 <span className="font-black text-emerald-600 text-sm">+{emp.bonus.toLocaleString('ru-RU')} {currencySymbol}</span>
 <span className="text-xs font-bold text-slate-400 mt-1 block">({emp.ordersCount} заказов)</span>
 </div>
 </div>
 <div className="flex justify-between items-center pt-2">
 <span className="text-xs font-black text-slate-400">К выплате</span>
