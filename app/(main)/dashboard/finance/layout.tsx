...
const TAB_INFO: Record<string, { title: string, description: string }> = {"/dashboard/finance/sales": { title:"Аналитика продаж", description:"Мониторинг выручки, прибыли и эффективности по категориям" },"/dashboard/finance/salary": { title:"Управление ФОТ", description:"Расчет заработных плат, бонусов и KPI сотрудников" },"/dashboard/finance/funds": { title:"Фонды компании", description:"Распределение капитала и управление резервами" },"/dashboard/finance/promocodes": { title:"Промокоды", description:"Управление маркетинговыми акциями и скидками" },"/dashboard/finance/transactions": { title:"Транзакции", description:"Полный журнал всех денежных движений в системе" },"/dashboard/finance/expenses": { title:"Расходы", description:"Учет косвенных и операционных затрат компании" },"/dashboard/finance/pl": { title:"P&L Отчет", description:"Отчет о прибылях и убытках для комплексного анализа" }
};
...
export default function FinanceLayout({
  ...
  return (
    <div className="flex flex-col gap-3 animate-in fade-in duration-700">
      {/* 1. Page Header */}
      <PageHeader title={currentTabInfo.title} description={currentTabInfo.description} actions={<FinanceDateFilter />}
      />
      ...
      <div className="flex w-full h-[58px] items-center gap-2 p-[6px] !rounded-[22px] bg-white border border-slate-200/50 shadow-sm relative z-20">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar flex-1 h-full relative z-0">
          {TABS.map((tab) => {
            ...
            return (
              <Link key={tab.id} href={tab.href} className={cn("flex-1 h-full relative flex items-center justify-center gap-2.5 px-6 !rounded-[16px] text-[13px] font-bold group min-w-[130px] whitespace-nowrap transition-all", isActive ?"text-white" :"text-slate-500 hover:text-slate-900" )}>
                {isActive && (
                  <motion.div
                    layoutId="activeTabFinanceNav"
                    className={cn("absolute inset-0 !rounded-[16px] shadow-lg z-0",
