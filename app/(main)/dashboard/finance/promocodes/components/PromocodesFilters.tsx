  ...
  return (
    <div className="flex flex-wrap items-center gap-3 bg-slate-50/50 p-4 rounded-[20px] border border-slate-100">
      ...
      <div className="w-[180px]">
        <Select options={[ { id:"all", title:"Все статусы" }, { id:"active", title:"Только активные" }, { id:"paused", title:"На паузе" } ]} value={filters.status} onChange={(val) => setFilters({ ...filters, status: val })}
          variant="minimal"
        />
      </div>

      <div className="w-[200px]">
        <Select options={[ { id:"all", title:"Все типы скидок" }, { id:"percentage", title:"Процент %" }, { id:"fixed", title: `Фикс. сумма ${currencySymbol}` }, { id:"free_shipping", title:"Беспл. доставка" }, { id:"gift", title:"Подарок" } ]} value={filters.type} onChange={(val) => setFilters({ ...filters, type: val })}
          variant="minimal"
        />
      </div>
