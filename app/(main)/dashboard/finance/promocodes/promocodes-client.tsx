...
export function PromocodesClient({ initialData }: { initialData: Promocode[] }) {
  ...
  return (
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-700">
      <PromocodesHeader searchQuery={ui.searchQuery} setSearchQuery={(query) => setUi(prev => ({ ...prev, searchQuery: query }))}
        onOpenBulk={() => setUi(prev => ({ ...prev, dialogs: { ...prev.dialogs, isBulkOpen: true } }))}
        onOpenCreate={handlers.handleOpenCreate}
        onExport={handlers.handleExport}
      />

      <PromocodesFilters filters={filters} setFilters={setFilters} filteredCount={data?.length || 0} currencySymbol={currencySymbol} />

      <PromocodesGrid data={data} currencySymbol={currencySymbol} onEdit={handlers.handleOpenEdit} onToggle={handlers.handleToggle} onDelete={handlers.handleDelete} />

      <PromocodeFormModal isOpen={ui.dialogs.isCreateOpen} onClose={() => setUi(prev => ({ ...prev, dialogs: { ...prev.dialogs, isCreateOpen: false } }))}
      ...
      <PromocodeBulkModal isOpen={ui.dialogs.isBulkOpen} onClose={() => setUi(prev => ({ ...prev, dialogs: { ...prev.dialogs, isBulkOpen: false } }))}
        isLoading={ui.isLoading}
        bulk={bulk}
        setBulk={setBulk}
      ...
      <ConfirmDialog isOpen={!!ui.dialogs.deleteId} onClose={() => setUi(prev => ({ ...prev, dialogs: { ...prev.dialogs, deleteId: null } }))}
