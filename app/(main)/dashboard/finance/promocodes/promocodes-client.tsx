        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <Button
                    ...
                    className="h-11 btn-dark rounded-[var(--radius-inner)] px-6 gap-2 font-bold shadow-xl shadow-slate-900/10 transition-all active:scale-95 border-none"
                >
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center bg-white p-6 rounded-[24px] border border-slate-200/60 shadow-sm">
                <div className="relative flex-1 w-full group">
                    ...
                    <input
                        ...
                        className="w-full h-12 pl-14 pr-6 rounded-[var(--radius)] bg-slate-50 border border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none font-medium text-slate-900 transition-all"
                    />
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl p-10 animate-in zoom-in-95 overflow-hidden">
                ...
                <div className="space-y-6 relative">
                    <div className="space-y-1.5">
                        <label className="text-sm font-bold text-slate-500 ml-1">Код</label>
                        <input name="code" required placeholder="SUMMER2024" className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-primary outline-none font-bold text-lg  tracking-normal placeholder:text-slate-300" />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-500 ml-1">Тип скидки</label>
                            <select name="discountType" className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-primary outline-none font-bold text-sm">
                                <option value="percentage">Процент %</option>
                                <option value="fixed">Фиксированная ₽</option>
                            </select>
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-500 ml-1">Значение</label>
                            <input name="value" type="number" required placeholder="10" className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-primary outline-none font-bold text-sm" />
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-500 ml-1">Мин. сумма заказа</label>
                            <input name="minOrderAmount" type="number" placeholder="0" className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-primary outline-none font-bold text-sm" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-500 ml-1">Лимит использований</label>
                            <input name="usageLimit" type="number" placeholder="∞" className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-primary outline-none font-bold text-sm" />
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-500 ml-1">Дата начала</label>
                            <div className="relative">
                                <Calendar className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                <input name="startDate" type="date" className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-primary outline-none font-bold text-sm" />
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-sm font-bold text-slate-500 ml-1">Дата окончания</label>
                            <div className="relative">
                                <Calendar className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                <input name="endDate" type="date" className="w-full h-12 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-primary outline-none font-bold text-sm" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <Button type="submit" disabled={isLoading} className="h-11 flex-1 btn-dark rounded-[var(--radius-inner)] font-bold shadow-xl shadow-slate-900/10 border-none">{isLoading ? "Создание..." : "Создать"}</Button>
                    </div>
                </div>
            </form>
        </div>
