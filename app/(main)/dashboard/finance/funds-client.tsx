...
interface FundCardProps {
    fund: FundStats["funds"][number];
    currencySymbol: string;
}

function FundCard({ fund, currencySymbol }: FundCardProps) {
    ...
    return (
        <div className="crm-card p-10 bg-white flex flex-col justify-between hover:scale-[1.02] transition-all duration-500 border-none shadow-md group overflow-hidden relative h-full">
            ...
            <div>
                <h4 className="text-slate-400 text-xs font-bold mb-3">{fund.name}</h4>
                ...
                <div className="h-20 w-full flex rounded-[var(--radius-inner)] overflow-hidden shadow-inner bg-slate-50 border border-slate-200/50 p-1.5 mb-14">
                    {fundsData.funds.map((fund, i) => (
                        <div
                            ...
                            style={{ width: `${fund.percentage}%` }}
                        >
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/10 text-white text-xs font-black">
                                {fund.percentage}%
                            </div>
                        </div>
                    ))}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 p-8 bg-slate-50/50 rounded-[var(--radius-inner)] border border-slate-100">
                    {fundsData.funds.map((fund, i) => (
                        <div key={i} className="flex items-center gap-3 group/item">
                            ...
                            <span className="text-xs font-bold text-slate-700">{fund.name}</span>
                        </div>
                    ))}
                </div>
            </div>
