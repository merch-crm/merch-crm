  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="flex items-center gap-1 p-1 bg-slate-100/50 rounded-2xl w-fit border border-slate-200/50 shadow-sm">
        {ranges.map((range) => (
          <Button key={range.value} variant="ghost" onClick={() => handleRangeChange(range.value)}
        ...
        <Button variant="ghost" onClick={() => handleRangeChange("all")}
          className={cn("px-4 py-2 h-auto text-sm font-bold rounded-2xl transition-all",
      ...
      <div className="flex items-center gap-2">
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" className={cn("flex items-center gap-2 px-6 py-2.5 h-auto text-sm font-bold rounded-2xl transition-all border border-slate-200 shadow-sm", isCustom ?"bg-white text-slate-900 hover:bg-white" :"bg-white text-slate-500 hover:text-slate-700 hover:bg-slate-50" )}>
          ...
          <PopoverContent className="w-auto p-0 rounded-[1.5rem] overflow-hidden border-slate-200 shadow-2xl bg-white" align="end" sideOffset={8}>
            <div className="p-4 bg-white">
              <Calendar initialFocus mode="range" defaultMonth={dateRange?.from} selected={dateRange} onSelect={setDateRange} numberOfMonths={1} locale={ru} className="rounded-2xl" />
              ...
              <div className="mt-4 pt-4 border-t border-slate-200 flex items-center justify-between gap-3">
                ...
                <Button size="sm" onClick={handleApplyCustomRange} disabled={!dateRange?.from} className="bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-bold px-6">
                  Применить
                </Button>
              </div>
            </div>
