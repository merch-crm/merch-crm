  ...
  <TooltipProvider>
   <div className="flex -space-x-2">
    {displayUsers.map((user, index) => (
     <Tooltip key={user.id} content={<p className="font-medium">{user.name}</p>}
     >
       <Avatar className={cn( sizeClasses[size], "border-2 border-white cursor-pointer transition-all duration-200", "hover:z-20 hover:scale-110 hover:border-violet-200", "ring-0 hover:ring-2 hover:ring-violet-400/30" )} style={{ zIndex: displayUsers.length - index }}>
    ...
    {remainingCount > 0 && (
     <Tooltip content={ <p className="font-medium">
