    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={disabled}
          className={cn(
            "w-full justify-between min-h-12 h-auto py-2.5 px-4 rounded-xl border-slate-200 bg-slate-50/50 hover:bg-white transition-colors",
               ...
               <>
                {selectedUsers.slice(0, maxDisplay).map((user) => (
                  <Badge
                    key={user.id}
                    variant="secondary"
                    className="flex items-center gap-1.5 pr-1 bg-violet-100 text-violet-700 hover:bg-violet-200 rounded-lg"
                  >
