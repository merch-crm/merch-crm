...
export function TaskStatsGrid({ stats, className }: TaskStatsGridProps) {
 return (
  <div className={cn("grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3", className)}>
   {/* Новые задачи */}
   <StatCard title="Новые" count={stats.new} icon={Circle} bgColor="bg-violet-50 border-violet-100" gradient="from-violet-500 to-purple-500" />
   
   {/* В работе */}
   <StatCard title="В работе" count={stats.inProgress} icon={Clock} bgColor="bg-blue-50 border-blue-100" gradient="from-blue-500 to-indigo-500" />
   
   {/* На проверке */}
   <StatCard title="Проверка" count={stats.review} icon={AlertCircle} bgColor="bg-amber-50 border-amber-100" gradient="from-amber-400 to-orange-500" />
   
   {/* Готово (За сегодня / Всего) */}
   <StatCard title="Готово" count={stats.done} icon={CheckCircle2} bgColor="bg-emerald-50 border-emerald-100" gradient="from-emerald-400 to-green-500" />
  </div>
 );
}
