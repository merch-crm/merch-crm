...
export function TaskDetailsDialog({
 ...
 return (
  <>
      ...
      <div className="flex items-start justify-between gap-3 relative z-10">
       <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
         {/* Status select */}
         <div className="w-48">
          <Select options={statusOptions} value={task.status} onChange={handleStatusChange} disabled={!canChangeStatus || isChangingStatus} compact />
         </div>
     ...
     <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
      <div className="border-b border-white/20 px-6 backdrop-blur-md bg-white/40 relative z-10">
       <TabsList className="h-12 bg-transparent p-0 gap-1">
        {tabs.map((tab) => (
         <TabsTrigger key={tab.id} value={tab.id} className="relative h-12 px-4 rounded-none border-b-2 border-transparent data-[state=active]:border-violet-500 data-[state=active]:text-violet-700 font-medium text-slate-500 hover:text-slate-700 data-[state=active]:bg-transparent">
      ...
      <div className="flex-1 overflow-y-auto p-6">
       <TabsContent value="info" className="m-0">
        <TaskInfoSection task={task} departments={departments} canEdit={canEdit ?? false} onTaskUpdated={onTaskUpdated} />
       </TabsContent>

       <TabsContent value="assignees" className="m-0">
        <TaskAssigneesSection task={task} users={users} currentUserId={currentUserId} canEdit={canEdit ?? false} onTaskUpdated={onTaskUpdated} />
       </TabsContent>

       <TabsContent value="checklist" className="m-0">
        <TaskChecklistSection task={task} canEdit={canEdit ?? false} onTaskUpdated={() => { onTaskUpdated?.(); router.refresh(); }}
        />
       </TabsContent>

       <TabsContent value="comments" className="m-0">
        <TaskCommentsSection task={task} currentUserId={currentUserId} onTaskUpdated={() => { onTaskUpdated?.(); router.refresh(); }}
        />
       </TabsContent>

       <TabsContent value="dependencies" className="m-0">
        <TaskDependenciesSection task={task} allTasks={allTasks} canEdit={canEdit ?? false} onTaskUpdated={() => { onTaskUpdated?.(); router.refresh(); }}
        />
       </TabsContent>
   ...
   {/* Delegate dialog */}
   <DelegateDialog task={task} open={showDelegateDialog} onOpenChange={setShowDelegateDialog} users={users} currentUserId={currentUserId} onTaskUpdated={onTaskUpdated} />
  </>
 );
}
