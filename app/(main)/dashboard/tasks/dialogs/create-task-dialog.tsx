  ...
  >
   <div className="flex flex-col relative bg-white overflow-hidden">
    ...
    <div className="p-6 min-h-[400px]">
     {activeStep === 1 && (
      <BasicInfoStep data={{ title, description, priority, type }} actions={{ setTitle, setDescription, setPriority, setType }} options={{ priorityOptions, typeOptions }} />
     )}

     {activeStep === 2 && (
      <ParticipantsStep data={{ assigneeIds, watcherIds, departmentId }} actions={{ setAssigneeIds, setWatcherIds, setDepartmentId }} options={{ users: (users || []).map(u => ({ ...u, image: u.image || undefined, email: u.email || undefined })), departmentOptions }}
      />
     )}

     {activeStep === 3 && (
      <DetailsStep data={{ deadline, checklist, newChecklistItem, title, priority, assigneeIds }} actions={{ setDeadline, handleAddChecklistItem, handleRemoveChecklistItem, setNewChecklistItem }} />
     )}
    </div>
