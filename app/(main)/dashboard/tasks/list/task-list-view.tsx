  ...
  <div className="space-y-3 animate-in fade-in duration-500">
   ...
   <div className="flex items-center justify-between gap-3">
    <div className="relative flex-1 max-w-sm group">
     ...
     <Input placeholder="Поиск задач..." value={search} onChange={(e) => {
    ...
    <Table>
     <TableHeader>
      <TableRow className="bg-muted/30 hover:bg-muted/30 border-none">
       <TableHead className="w-12 text-center">
        <Checkbox checked={ paginatedTasks.length> 0 &&
          selectedIds.size === paginatedTasks.length
         }
         onCheckedChange={toggleSelectAll}
        />
       </TableHead>
       <TableHead className="cursor-pointer hover:text-primary transition-colors font-semibold text-xs py-4" onClick={() => handleSort("title")}
       >
       ...
       <TableHead className="cursor-pointer hover:text-primary transition-colors font-semibold text-xs " onClick={() => handleSort("status")}
       >
        <div className="flex items-center gap-2">
         Статус
       ...
       <TableHead className="cursor-pointer hover:text-primary transition-colors font-semibold text-xs " onClick={() => handleSort("priority")}
       >
        <div className="flex items-center gap-2">
         Приоритет
       ...
       <TableHead className="cursor-pointer hover:text-primary transition-colors font-semibold text-xs " onClick={() => handleSort("deadline")}
       >
        <div className="flex items-center gap-2">
     ...
     <TableBody>
      {paginatedTasks.length === 0 ? (
       <TableRow>
        <TableCell colSpan={7} className="text-center py-20 text-muted-foreground italic border-none">
       ...
       paginatedTasks.map((task) => {
        ...
        return (
         <TableRow key={task.id} className={cn( "group cursor-pointer border-border/50 hover:bg-muted/20 transition-all", selectedIds.has(task.id) && "bg-primary/5" )} onClick={() => onTaskClick(task)}
         >
          ...
          <TableCell>
           <div className="flex -space-x-2.5">
            {task.assignees?.slice(0, 4).map((assignee, _i) => (
             <Avatar key={assignee.userId} className="h-8 w-8 border-2 border-background ring-1 ring-border/30 shadow-sm">
