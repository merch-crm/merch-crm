     ...
     {currentSection ? (
      <div className="max-w-4xl mx-auto px-10 py-10 animate-in fade-in slide-in-from-bottom-2 duration-300">
       ...
       <article className="prose prose-slate max-w-none 
        ...
        <ReactMarkdown key={activeTab} remarkPlugins={[remarkGfm]} components={{ code({ className, children, ...props }) { const match = /language-(\w+)/.exec(className || ""); const lang = match ? match[1] : ""; if (lang === "mermaid") { const codeString = String(children); return ( <div className="my-10 w-full overflow-hidden bg-white border border-slate-200 rounded-2xl shadow-sm">
