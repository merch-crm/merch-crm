  ...
  return (
    <div className="bg-white rounded-[32px] shadow-xl overflow-hidden border border-slate-200/60 flex flex-col h-[calc(100vh-220px)] min-h-[600px]">
      <div className="p-4 border-b border-slate-100 bg-slate-50/30">
        <ChannelTabs activeChannel={activeChannel} onChange={setActiveChannel} stats={channelStats} />
      </div>

      <div className="flex flex-1 overflow-hidden">
        <div className="w-80 lg:w-96 flex-shrink-0 border-r border-slate-100 flex flex-col overflow-hidden">
          <ConversationList conversations={conversations} selectedId={selectedConversation?.id} onSelect={handleSelectConversation} onSearch={handleSearch} isLoading={ui.isLoadingConversations} />
        </div>

        <ChatWindow conversation={selectedConversation} messages={messages} templates={templates} onSendMessage={handleSendMessage} onLoadMore={handleLoadMore} hasMore={ui.hasMoreMessages} isLoading={ui.isLoadingMessages} isSending={ui.isSending} />
      </div>
    </div>
  );
}
