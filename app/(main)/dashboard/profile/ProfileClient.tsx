...
export function ProfileClient({ user, activities, tasks }: ProfileClientProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col md:flex-row bg-slate-50 font-sans overflow-hidden">
      <ProfileSidebar user={user} view={view} isMobileMenuOpen={isMobileMenuOpen} setIsMobileMenuOpen={setIsMobileMenuOpen} handleNavClick={handleNavClick} getRoleName={getRoleName} />
      ...
      <div className="flex-1 p-6 md:p-6 lg:p-[--padding-xl] relative z-0 overflow-y-auto bg-slate-50/50 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent h-full">
        <ProfileHeader user={user} view={view} setIsMobileMenuOpen={setIsMobileMenuOpen} handleNavClick={handleNavClick} />
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {view ==="profile" && (
              <OverviewTab statsData={statsData} currencySymbol={currencySymbol} departmentName={getDepartmentName()} roleName={getRoleName()} tasks={tasks} handleNavClick={handleNavClick} />
            )}
