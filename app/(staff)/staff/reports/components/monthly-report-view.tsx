  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard title="Рабочих дней" value={data.workDays} icon={Calendar} color="indigo" />
        <StatCard title="Ср. присутствие" value={`${data.summary.avgAttendance.toFixed(0)}%`} icon={Users} color="blue" />
        <StatCard title="Ср. часов/месяц" value={`${data.summary.avgMonthlyHours.toFixed(0)} ч`} icon={Clock} color="indigo" />
        <StatCard title="Всего опозданий" value={data.summary.totalLateArrivals} icon={AlertTriangle} color="orange" />
      </div>
