  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard title="Присутствовали" value={`${data.presentToday}/${data.totalEmployees}`} icon={Users} color="indigo" />
        <StatCard title="Среднее время" value={`${data.averageWorkHours.toFixed(1)} ч`} icon={Clock} color="blue" />
        <StatCard title="Опоздания" value={data.lateArrivals} icon={AlertTriangle} color="orange" />
        <StatCard title="Посещаемость" value={`${((data.presentToday / (data.totalEmployees || 1)) * 100).toFixed(0)}%`} icon={TrendingUp} color="emerald" />
      </div>
