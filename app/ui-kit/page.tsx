...
const categoryGroups = [
 {
  ...
  items: [
   { slug: 'colors', label: 'Цвета', icon: Palette, color: 'text-indigo-600', bg: 'bg-indigo-50' },
   { slug: 'typography', label: 'Типографика', icon: Type, color: 'text-slate-600', bg: 'bg-slate-50' },
   { slug: 'buttons', label: 'Кнопки', icon: RectangleHorizontal, color: 'text-blue-600', bg: 'bg-blue-50' },
   { slug: 'statuses', label: 'Бейджи и статусы', icon: Activity, color: 'text-indigo-500', bg: 'bg-indigo-50' },
  ]
 },
 {
  title: "Базовые UI",
  ...
  items: [
   { slug: 'inputs', label: 'Поля ввода', icon: TextCursorInput, color: 'text-orange-600', bg: 'bg-orange-50' },
   { slug: 'cards', label: 'Карточки', icon: CreditCard, color: 'text-emerald-600', bg: 'bg-emerald-50' },
   { slug: 'controls', label: 'Переключатели', icon: SlidersHorizontal, color: 'text-indigo-600', bg: 'bg-indigo-50' },
   { slug: 'accordions', label: 'Раскрытие', icon: FoldVertical, color: 'text-violet-600', bg: 'bg-violet-50' },
   { slug: 'tooltips', label: 'Подсказки', icon: MessageCircle, color: 'text-rose-600', bg: 'bg-rose-50' },
  ]
 },
 {
  ...
  items: [
   { slug: 'data-display', label: 'Таблицы и Списки', icon: Table2, color: 'text-indigo-600', bg: 'bg-indigo-50' },
   { slug: 'uploads', label: 'Медиа и Загрузки', icon: UploadCloud, color: 'text-purple-600', bg: 'bg-purple-50' },
  ]
 },
 {
  ...
  items: [
   { slug: 'charts', label: 'Графики и визуализация', icon: BarChart3, color: 'text-emerald-600', bg: 'bg-emerald-50' },
   { slug: 'pricing', label: 'Оплата', icon: Calculator, color: 'text-amber-600', bg: 'bg-amber-50' },
   { slug: 'dates', label: 'Даты и время', icon: CalendarDays, color: 'text-rose-600', bg: 'bg-rose-50' },
   { slug: 'qr', label: 'QR Коды', icon: QrCode, color: 'text-slate-600', bg: 'bg-slate-50' },
   { slug: 'merch', label: 'Производство', icon: Briefcase, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  ]
 },
 {
  title: "Оформление",
  ...
  items: [
   { slug: 'avatars', label: 'Пользователи', icon: UserCircle, color: 'text-teal-600', bg: 'bg-teal-50' },
   { slug: 'navigation', label: 'Навигация', icon: Compass, color: 'text-cyan-600', bg: 'bg-cyan-50' },
   { slug: 'errors', label: 'Системные уведомления', icon: AlertTriangle, color: 'text-red-500', bg: 'bg-red-50' },
  ]
 },
 {
  title: "Декоративные",
  ...
  items: [
   { slug: 'backgrounds', label: 'Фоны', icon: Image, color: 'text-emerald-500', bg: 'bg-emerald-50' },
   { slug: 'effects', label: 'Эффекты', icon: Wand2, color: 'text-pink-500', bg: 'bg-pink-50' },
  ]
 },
];

async function getComponentCount(slug: string) {
  try {
    const dir = path.join(process.cwd(), 'app/ui-kit/(categories)', slug);
    if (!fs.existsSync(dir)) return 0;
    
    let count = 0;
    async function countInDir(currentDir: string) {
      ...
      for (const item of items) {
        ...
        if (item.isDirectory()) {
          ...
          count += (content.match(/<ComponentShowcase/g) || []).length;
        }
      }
    }
...
export default async function UIKitHomePage() {
  // Fetch actual counts
  const groupsWithCounts = await Promise.all(categoryGroups.map(async (group) => {
    const itemsWithCounts = await Promise.all(group.items.map(async (item) => {
      return {
        ...item,
        count: await getComponentCount(item.slug)
      };
    }));
    return { ...group, items: itemsWithCounts };
  }));

  const totalComponents = groupsWithCounts.reduce((acc, group) => {
    ...
    <div className="mb-16 relative">
     ...
     <div className="mt-6 flex flex-wrap gap-3 text-xs font-bold text-gray-400 ">
       <span className="flex items-center gap-1.5 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100 shadow-sm">
        <div className="size-2 rounded-full bg-blue-500" /> ~{totalComponents} {pluralize(totalComponents, 'элемент', 'элемента', 'элементов')}
       </span>
     </div>
    </div>
          ...
          >
           ...
           <div className="relative flex items-center justify-between z-10">
            ...
            <span className="rounded-full bg-gray-50 border border-gray-100 px-3 py-1 text-xs font-bold text-gray-500 group-hover:bg-primary-base group-hover:text-white transition-colors duration-300">
             {c.count} {pluralize(c.count, 'компонент', 'компонента', 'компонентов')}
            </span>
           </div>
