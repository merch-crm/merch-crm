        ...
        <div className="w-full glass-panel p-8 rounded-[var(--radius-outer)] mt-10 overflow-hidden relative min-h-[600px] flex items-center justify-center bg-slate-50/20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 w-full max-w-6xl relative z-10">
                ...
                <div className="lg:col-span-4">
                    <div className="bg-white rounded-[24px] border border-slate-100 shadow-crm-xl overflow-hidden flex flex-col min-h-[500px]">
                        {/* Header part */}
                        <div className="p-8 pb-6">
                            <button className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-colors mb-6 group">
                                <ChevronLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
                                Назад
                            </button>
                            <h2 className="text-2xl font-bold text-slate-900 leading-tight">Новая позиция</h2>
                            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-2">Создание карточки товара</p>
                        </div>
                            ...
                            {steps.map((step) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            "w-full text-left relative transition-all duration-500 rounded-[20px] group overflow-hidden border transition-shadow",
                                            isActive
                        ...
                        <div className="p-6 pt-0 mt-auto">
                            <div className="flex items-center justify-between p-5 bg-slate-50/50 backdrop-blur-md rounded-[20px] border border-slate-100 shadow-sm relative overflow-hidden">
                                <div className="absolute inset-0 bg-white/40 pointer-events-none" />
                                <div className="flex items-center gap-3 relative z-10">
                                    <div className="w-2.5 h-2.5 rounded-full bg-[#00d685] shadow-[0_0_8px_#00d685]" />
                                    <div className="flex flex-col">
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#a1a1aa]">Черновик</span>
                                        <span className="text-[11px] font-extrabold text-[#18181b]">Сохранено</span>
                                    </div>
                                </div>
                                <button className="text-[11px] font-bold uppercase tracking-widest text-[#5d00ff] hover:text-[#4a00cc] transition-colors relative z-10">Начать заново</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 flex flex-col gap-6">
                    <div className="crm-card h-full p-12 flex flex-col justify-center items-center text-center">
                        ...
                        <div className="mt-12 w-full grid grid-cols-2 gap-4">
                            {[1, 2, 3, 4].map(i => (
                                <div key={i} className="h-20 bg-slate-50 rounded-[var(--radius-inner)] border border-slate-100" />
                            ))}
                        </div>
                    </div>
                </div>
