...
export default function TwistyDashboard() {
    return (
        <div className="w-full bg-[#E8EBF2] p-8 rounded-[40px] border border-slate-200/50 mt-10 font-sans text-slate-800">
            ...
            <header className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
                ...
                <div className="relative">
                    <input
                        ...
                        className="pl-4 pr-10 py-2.5 rounded-full bg-white text-sm w-64 border-none shadow-sm focus:ring-2 focus:ring-[#FF4F18]/20 outline-none placeholder:text-slate-300"
                    />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="bg-white rounded-[40px] p-8 shadow-sm relative overflow-hidden">
                        <div className="flex justify-between items-start mb-8">
                            <div>
                                ...
                                <p className="text-slate-400 text-xs max-w-sm leading-relaxed ml-11">
                                    Track changes in income over time and access detailed data on each project and payments received
                                </p>
                            </div>
                            <button className="flex items-center gap-2 px-4 py-2 rounded-full border border-slate-200 text-xs font-medium hover:bg-slate-50 transition-colors">
                                <span>Week</span>
                                    ...
                                    <div className="relative w-full flex justify-center h-40 items-end">
                                        ...
                                        <div
                                            className={cn(
                                                ...
                                                item.active ? "bg-[#0F172A] scale-125" : "bg-slate-300 group-hover:bg-[#FF4F18]"
                                            )}
                                        ...
                                        {/* Active Tooltip Pill */}
                                        {item.active && (
                                            <div className="absolute top-[20%] transition-all bg-[#0F172A] text-white text-xs font-bold py-1.5 px-3 rounded-full shadow-xl">
                                                {item.val}
                                            </div>
                                        )}

                                        {/* Active Background Pill (The slider effect) */}
                                        {item.active && (
                                            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-slate-50/50 to-slate-100/80 rounded-full w-12 mx-auto -z-0 border border-slate-100/50" />
                                        )}
                                    </div>

                                    <span className={cn(
                                        "w-8 h-8 flex items-center justify-center rounded-full text-xs font-semibold z-10 transition-colors",
                                        item.active ? "bg-[#0F172A] text-white" : "text-slate-400 bg-slate-50 group-hover:text-[#FF4F18]"
                                    )}>{item.day}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="absolute bottom-8 left-8">
                            <div className="text-3xl font-bold text-[#0F172A] mb-1">+20%</div>
                            <div className="text-xs text-slate-400">This week&apos;s income is<br />higher than last week&apos;s</div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white rounded-[32px] p-6 shadow-sm">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button className="text-xs font-semibold text-slate-400 hover:text-[#0F172A]">See all</button>
                            </div>
                            <div className="space-y-4">
                                {[
                                    { name: "Randy Gouse", role: "Cybersecurity specialist", tag: "Senior", bg: "bg-[#FF4F18]" },
                                    ...
                                    <div key={idx} className="flex items-center justify-between p-1">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm font-bold text-[#0F172A]">{user.name}</span>
                                                    <span className={cn("text-[8px] text-white px-1.5 py-0.5 rounded-md font-bold", user.bg)}>{user.tag}</span>
                                                </div>
                                                <div className="text-[10px] text-slate-400">{user.role}</div>
                                            </div>
                                        </div>
                                        <button className="w-8 h-8 rounded-full border border-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-50 transition-colors">
                        ...
                        {/* Unlock Premium */}
                        <div className="bg-gradient-to-br from-slate-200 to-slate-300 rounded-[32px] p-6 shadow-sm relative overflow-hidden flex flex-col justify-between">
                            {/* Mesh effect simulation */}
                            <div className="absolute inset-0 opacity-40">
                                <div className="absolute top-0 right-0 w-40 h-40 bg-white rounded-full blur-3xl" />
                                <div className="absolute bottom-0 left-0 w-40 h-40 bg-slate-400 rounded-full blur-3xl" />
                                {/* Pattern grid */}
                            ...
                            <div className="relative z-10">
                                <h3 className="font-bold text-lg text-[#0F172A] mb-2">Unlock Premium Features</h3>
                ...
                <div className="lg:col-span-4 flex flex-col gap-6">
                    ...
                    <h3 className="text-lg font-semibold px-2">Your Recent Projects</h3>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white rounded-[32px] p-6 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                            ...
                            <div className="text-xs text-slate-400 mb-4">$10/hour</div>
                            <div className="flex gap-2 mb-4">
                                <span className="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-semibold text-slate-500">Remote</span>
                                <span className="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-semibold text-slate-500">Part-time</span>
                            </div>
                        ...
                        <div className="bg-white rounded-[32px] p-6 shadow-sm hover:shadow-md transition-shadow cursor-pointer opacity-60 hover:opacity-100">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="text-xs text-slate-400">$10/hour</div>
                            </div>
                        </div>
                    ...
                    {/* Proposals Widget Partial */}
                    <div className="bg-white rounded-[32px] p-6 shadow-sm mt-auto">
                        <div className="flex justify-between items-center">
                            <h4 className="font-semibold text-sm">Proposals</h4>
                        </div>
                        <div className="text-3xl font-bold text-[#0F172A] mt-2">64</div>
                    </div>
