        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <Button
                    ...
                    className="h-12 bg-rose-600 hover:bg-rose-700 text-white rounded-[var(--radius)] px-6 gap-2 font-bold shadow-xl shadow-rose-100 transition-all active:scale-95"
                >
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center bg-white p-6 rounded-[24px] border border-slate-200/60 shadow-sm">
                <div className="relative flex-1 w-full group">
                    ...
                    <input
                        ...
                        className="w-full h-14 pl-14 pr-6 rounded-[var(--radius)] bg-slate-50 border border-transparent focus:bg-white focus:border-rose-500 focus:ring-4 focus:ring-rose-500/5 outline-none font-medium text-slate-900 transition-all"
                    />
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl p-10 animate-in zoom-in-95 overflow-hidden">
                ...
                <div className="space-y-6 relative">
                    <div className="space-y-1.5">
                        ...
                        <input name="date" type="date" defaultValue={new Date().toISOString().split('T')[0]} required className="w-full h-14 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-rose-500 outline-none font-bold text-sm" />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-bold text-slate-400  tracking-normal ml-1">Категория</label>
                            <select name="category" className="w-full h-14 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-rose-500 outline-none font-bold text-sm">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input name="amount" type="number" step="0.01" required placeholder="0.00" className="w-full h-14 px-6 rounded-[var(--radius)] bg-slate-50 border-none focus:ring-2 focus:ring-rose-500 outline-none font-bold text-lg" />
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <Button type="submit" disabled={isLoading} className="h-14 flex-1 bg-rose-600 text-white rounded-[var(--radius)] font-bold shadow-xl shadow-rose-100">{isLoading ? "Добавление..." : "Добавить"}</Button>
                    </div>
                </div>
            </form>
        </div>
