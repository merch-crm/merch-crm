    ...
    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="flex flex-col xl:flex-row gap-4">
                <div className={cn(
                    "flex-1 p-8 rounded-2xl border relative overflow-hidden group transition-all duration-700",
                    ...
                    <div className="relative space-y-6">
                        <div className="flex items-center justify-between">
                            <div className="w-12 h-12 rounded-[var(--radius)] bg-white flex items-center justify-center text-primary shadow-sm border border-primary/20">
                                <Package className="w-6 h-6" />
                            </div>
                            {isCriticalStock && <Badge className="bg-rose-600 text-white rounded-[var(--radius)] font-bold text-[10px] px-4">Критический остаток</Badge>}
                            {isLowStock && !isCriticalStock && <Badge className="bg-amber-600 text-white rounded-[var(--radius)] font-bold text-[10px] px-4">Низкий запас</Badge>}
                        </div>
                        ...
                        <div className={cn("pt-6 border-t", isLowStock || isCriticalStock ? "border-black/5" : "border-white/10")}>
                            <Button
                                ...
                                className={cn("rounded-[var(--radius)] px-8 h-14 text-[13px] font-bold transition-all shadow-md active:scale-95 border-none",
                ...
                <div className="w-full xl:w-[400px] bg-slate-900 p-8 rounded-2xl text-white flex flex-col justify-between group overflow-hidden relative border border-white/5 shadow-xl">
                    {/* Decorative flow */}
                    ...
                    <div className="relative mt-8">
                        <Button
                            variant="outline"
                            className="w-full h-14 rounded-[var(--radius)] border-white/10 bg-white/5 hover:bg-white/10 text-white font-bold text-[13px]"
            ...
            {isEditing && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in slide-in-from-top-4 duration-500">
                    <div className="bg-white p-8 rounded-[var(--radius)] border border-slate-200 shadow-sm space-y-4">
                        ...
                        <input
                            ...
                            className="w-full text-2xl font-bold bg-slate-50 border-none outline-none p-4 rounded-[var(--radius)] focus:ring-4 focus:ring-primary/10 transition-all"
                        />
                    </div>
                    <div className="bg-white p-8 rounded-[var(--radius)] border border-slate-200 shadow-sm space-y-4">
                        ...
                        <input
                            ...
                            className="w-full text-2xl font-bold bg-slate-50 border-none outline-none p-4 rounded-[var(--radius)] focus:ring-4 focus:ring-rose-500/10 transition-all"
                        />
                    </div>
                </div>
            )}
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                    {stocks.map((stock) => (
                        <div
                            key={stock.id}
                            className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm hover:shadow-md transition-all group flex items-center justify-between"
                        >
                            <div className="flex items-center gap-6">
                                <div className="w-14 h-14 bg-slate-50 rounded-lg flex items-center justify-center text-slate-400 group-hover:bg-primary group-hover:text-white transition-all">
                            ...
                            <div className="flex flex-col items-end gap-3">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="p-3 bg-slate-50 rounded-[var(--radius)] hover:bg-slate-900 hover:text-white transition-all text-slate-400"
                                    >
                                        <PlusCircle className="w-4 h-4" />
                                    </button>
                                    <button
                                        onClick={() => onTransferStock(stock.storageLocationId)}
                                        className="p-3 bg-slate-50 rounded-[var(--radius)] hover:bg-primary hover:text-white transition-all text-slate-400"
                                    >
                    ...
                    {stocks.length === 0 && (
                        <div className="col-span-full py-20 text-center bg-slate-50/50 rounded-[var(--radius)] border-2 border-dashed border-slate-200">
