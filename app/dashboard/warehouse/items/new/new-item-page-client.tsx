...
export function NewItemPageClient({
    ...
    const handleCategorySelect = (category: Category) => {
        ...
        setFormData(prev => ({
            ...
            sizeCode: "",
            itemName: "",
            unit: category.name.toLowerCase().includes("одежда") ? "шт" : prev.unit
        }));
    };
    ...
    const validateStep = (currentStep: number): boolean => {
        ...
        if (currentStep === 2) {
            if (isClothing) {
                // Remove subcategoryId check here IF it was selected in step 1, but keep safety check
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // ... (rest of form data appending remains same, just ensure fields are correct)
            submitFormData.append("unit", formData.unit);
            submitFormData.append("description", formData.description);
            submitFormData.append("quantity", formData.quantity);
            submitFormData.append("criticalStockThreshold", formData.criticalStockThreshold);
            submitFormData.append("lowStockThreshold", formData.lowStockThreshold);
            ...
            if (result?.error) {
                ...
                clearDraft(); // IMPORTANT: Clear on success
            ...
            <aside className="w-[300px] bg-white border border-slate-200 rounded-[14px] flex flex-col shrink-0 relative z-20 h-full shadow-lg shadow-slate-200/50 overflow-hidden">
                ...
                <div className="flex-1 px-4 space-y-1 overflow-y-auto no-scrollbar">
                    {steps.map((s, idx) => {
                        ...
                        // Hide subcategory step if no subcategories exist
