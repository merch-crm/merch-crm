...
import { X, Package, MapPin, Info, ArrowUpRight, ArrowDownLeft, Clock, BarChart3, Check, Pencil } from "lucide-react";
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
            ...
            <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col">
                ...
                <div className="relative h-64 w-full bg-slate-100 flex items-center justify-center overflow-hidden shrink-0">
                    ...
                    <button
                        onClick={onClose}
                        className="absolute top-6 right-6 w-12 h-12 flex items-center justify-center text-slate-400 hover:text-slate-900 rounded-[14px] bg-white/80 backdrop-blur-md border border-white/50 shadow-xl transition-all hover:scale-110 active:scale-95 z-20"
                    >
                        <X className="h-6 w-6" />
                    </button>
                </div>
                ...
                <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-white relative">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-[14px] bg-indigo-600 flex items-center justify-center text-white shadow-xl shadow-indigo-100 shrink-0">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="bg-slate-50 rounded-[14px] p-6 border border-slate-100">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-10 h-10 rounded-[14px] bg-white border border-slate-200 flex items-center justify-center text-indigo-500 shadow-sm">
                        ...
                        <div className="bg-rose-50/50 rounded-[14px] p-6 border border-rose-100/50">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-10 h-10 rounded-[14px] bg-white border border-rose-100 flex items-center justify-center text-rose-500 shadow-sm">
                        ...
                        <div className="bg-emerald-50/50 rounded-[14px] p-6 border border-emerald-100/50">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-10 h-10 rounded-[14px] bg-white border border-emerald-100 flex items-center justify-center text-emerald-500 shadow-sm">
                    ...
                    <div className="space-y-4">
                        ...
                        {stocks.length > 0 ? (
                            <div className="grid grid-cols-1 gap-3">
                                {stocks.map(stock => (
                                    <div key={stock.id} className="flex items-center justify-between p-5 rounded-[14px] bg-white border border-slate-100 shadow-sm group hover:border-indigo-100 transition-all">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-[14px] bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-500 transition-colors">
                                        ...
                                        <div className="bg-slate-50 px-4 py-2 rounded-[14px] group-hover:bg-indigo-600 group-hover:text-white transition-all">
                            ...
                            <div className="p-6 rounded-[14px] bg-slate-50 border border-slate-100 text-center text-slate-400 text-sm font-medium">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {Object.entries(item.attributes || {}).map(([key, value]) => {
                                    ...
                                    return (
                                        <div key={key} className="px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-[14px] flex flex-col gap-0.5">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="p-6 rounded-[14px] bg-slate-50 border border-slate-100 text-slate-600 leading-relaxed font-medium">
                    ...
                    <div className="space-y-6">
                        ...
                        {isLoading ? (
                            <div className="space-y-4 animate-pulse">
                                {[1, 2, 3].map(i => <div key={i} className="h-20 bg-slate-50 rounded-[14px]" />)}
                            </div>
                        ) : history.length > 0 ? (
                            <div className="space-y-3">
                                {history.map((t) => {
                                    ...
                                    const isAttributeChange = t.type === "attribute_change";

                                    return (
                                        <div key={t.id} className="flex flex-col gap-4 p-5 rounded-[14px] bg-white border border-slate-100 hover:border-indigo-100 transition-all hover:bg-slate-50/30 group">
                                            <div className="flex items-center gap-4">
                                                <div className={cn(
                                                    "w-10 h-10 rounded-[14px] flex items-center justify-center shadow-sm shrink-0",
                                                    isTransfer ? "bg-indigo-50 text-indigo-600 border border-indigo-100" :
                                                        isAttributeChange ? "bg-amber-50 text-amber-600 border border-amber-100" :
                                                            (isIn ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-rose-50 text-rose-600 border border-rose-100")
                                                )}>
                                                    {isTransfer ? <ArrowUpRight className="w-5 h-5 opacity-50" /> :
                                                        isAttributeChange ? <Pencil className="w-4 h-4" /> :
                                                    ...
                                                    <div className="flex items-center justify-between mb-0.5">
                                                        <div className="text-sm font-black text-slate-900 uppercase tracking-tighter">
                                                            {isTransfer ? "Перемещение" :
                                                                isAttributeChange ? "Изменение свойств" :
                                                                    (isIn ? "Приход" : "Расход")}
                                                        </div>
                                                        {!isAttributeChange && (
                                            ...
                                            <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="flex items-center gap-1.5 px-2 py-0.5 bg-slate-50 rounded-[14px] border border-slate-100">
                            ...
                            <div className="p-12 text-center bg-slate-50 rounded-[14px] border border-slate-100">
                ...
                <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex gap-4">
                    <button
                        onClick={onClose}
                        className="flex-1 h-14 rounded-[14px] bg-white border border-slate-200 text-slate-600 font-black text-xs uppercase tracking-widest shadow-sm hover:shadow-md transition-all active:scale-95"
                    >
                        Закрыть
                    </button>
                </div>
