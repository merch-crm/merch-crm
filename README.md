...
> Таблицы логирования (`audit_logs_*`), безопасности (`security_events_*`) и складских транзакций (`inventory_transactions_*`) управляются **отдельно** и исключены из Drizzle-фильтра (`tablesFilter`). Не удаляйте их из фильтра в `drizzle.config.ts`, чтобы избежать случайного удаления или изменения их структуры через `db:push`.
