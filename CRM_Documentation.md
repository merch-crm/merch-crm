...
Каждое серверное действие (Server Action) начинается с проверки `getSession()`. Проверяется `session.roleName` для определения доступных операций:
- **Администратор / Руководство**: Полный доступ ко всем модулям.
