...
**Стек технологий:** Next.js 15.5.14 (App Router, Server Actions), PostgreSQL, Drizzle ORM, Fabric.js, Redis (кэширование), Better Auth (авторизация).
...
graph TD
    ...
    KB[База знаний / Knowledge Base] --- Staff
...
| Любой переход | Запись аудит-лога с `from` и `to`. Вызов `autoGenerateTasks()` для автоматического создания консолидированных задач (Дизайн/Производство). |
...
**Автоматическая генерация задач:** При смене статуса заказа система вызывает `autoGenerateTasks()`, которая создаёт консолидированную задачу для соответствующего отдела: на **Дизайн** (при переходе в статус `design`) или на **Производство** (при переходе в статус `production`). В текущей реализации задачи не дробятся по стадиям позиций, а создаются на весь заказ целиком для упрощения контроля.

### 8.2 База знаний (Knowledge Base)
...
**Особенности логики (`lib/services/knowledge-base/`):**
...
- Компоненты UI: `kb-sidebar.tsx` (дерево навигации по папкам), `kb-client.tsx` (клиентское состояние редактора и просмотра).
...
### 8.8 NPS Опросы (Feedback)

Система автоматического сбора обратной связи (`lib/services/nps.service.ts`).

- **Триггер:** Статус заказа `deliveryStatus === 'delivered'`.
- **Механика:** Ежедневный крон-скрипт (`app/api/cron/nps/route.ts`) ищет заказы с датой доставки более 3-х дней назад и отправляет Email с уникальной ссылкой.
- **Процесс:**
  1. Генерация UUID токена и запись в `customerFeedback`.
  2. Email клиенту со ссылкой на `/feedback/nps?token={token}`.
  3. Сохранение оценки (0-10) и комментария в БД.
...
graph TD
    ...
    J --> K[Доставка: Статус Delivered]
    K --> L[NPS: Сбор фидбека через 3 дня]
    L --> M[Клиент: Обновление LTV/RFM]
```
