      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={cn(
              "absolute z-[120] min-w-[220px] bg-white rounded-[24px] border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.12)] p-5 overflow-visible",
              variants.classes,
              className
            )}
            role="tooltip"
          >
            <div className="relative z-10 text-[11px] font-black text-slate-900 uppercase tracking-tight leading-relaxed">
              {children}
            </div>
            ...
            {/* Glossy Overlay */}
            <div className="absolute inset-0 pointer-events-none bg-gradient-to-tr from-white/10 via-transparent to-transparent opacity-50 rounded-[24px]" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
