      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={cn(
              "absolute top-full mt-3 right-0 z-[120] min-w-[260px] bg-white rounded-[32px] border border-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.12)] p-2 overflow-visible",
              className
            )}
            role="menu"
          >
              ...
              {items?.map((item) => (
                <div key={item.id} className="relative group/item">
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 transition-all group/item outline-none focus-visible:ring-2 focus-visible:ring-primary-base",
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-black text-slate-700 uppercase tracking-tight group-hover/item:text-slate-900">
                        {item.label}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {item.shortcut && (
                        <kbd className="text-[9px] font-black text-slate-300 uppercase tracking-widest px-2 py-0.5 rounded-lg border border-slate-100 bg-white font-sans">
                          {item.shortcut}
                        </kbd>
                      )}
                      {item.children && (
                        <ChevronRight className={cn("size-3 text-slate-300 group-hover/item:text-primary-base transition-transform duration-300", activeSubMenu === item.id && "rotate-90")} />
                      )}
                    </div>
                  </button>
                  ...
                  <AnimatePresence>
                    {item.children && activeSubMenu === item.id && (
                      <motion.div
                        ...
                        className="absolute left-full top-0 ml-3 min-w-[220px] bg-white rounded-[24px] border border-slate-100 shadow-2xl p-2 z-[130]"
                        role="menu"
                      >
                        {item.children?.map(sub => (
                          ...
                          >
                            ...
                            <span className="text-[11px] font-black text-slate-500 uppercase tracking-tight group-hover/sub:text-slate-900 leading-none">
                               {sub.label}
                            </span>
                          </button>
                        ))}
            ...
            {/* Glossy Overlay */}
            <div className="absolute inset-0 pointer-events-none bg-gradient-to-tr from-white/10 via-transparent to-transparent opacity-50 rounded-[32px]" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
