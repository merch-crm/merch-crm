...
export function SurfaceAccordion({ items: propItems }: { items?: { title: string; icon: React.ReactNode; content: string; status?: string }[] }) {
  ...
  if (!isMounted) {
    return <div className="w-full max-w-sm h-[320px] bg-white rounded-[32px] border border-gray-100 animate-pulse" />;
  }

  return (
    <div className="w-full max-w-sm bg-white rounded-[32px] border border-gray-100 shadow-crm-md p-6 flex flex-col gap-3 group relative overflow-hidden">
      ...
      <div className="space-y-3 relative z-10">
         {(items || []).map((item, i) => (
           <div 
             key={i} 
             className={cn(
               "group/item rounded-[24px] border transition-all duration-500 overflow-hidden",
