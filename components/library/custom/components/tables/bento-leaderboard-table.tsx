...
export function BentoLeaderboardTable({ title, users: propUsers, className }: BentoLeaderboardTableProps) {
  ...
  if (!isMounted) {
    return <div className={cn("bg-card text-card-foreground shadow-crm-md border border-border p-6 rounded-[27px] h-64 animate-pulse", className)} />;
  }
    ...
    <div className={cn("bg-card text-card-foreground shadow-crm-md border border-border p-6 rounded-[27px] flex flex-col", className)}>
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-left text-sm border-separate border-spacing-y-2">
          <tbody>
            {(users || []).map((user) => (
              <tr key={user.id} className="group bg-secondary/20 hover:bg-secondary/50 hover:-translate-y-0.5 transition-all outline outline-1 outline-transparent hover:outline-border/50 rounded-xl overflow-hidden shadow-sm">
