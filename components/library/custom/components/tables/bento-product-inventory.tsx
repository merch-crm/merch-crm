...
export function BentoProductInventory({ items: propItems }: { items?: typeof DEFAULT_ITEMS }) {
  ...
  if (!isMounted) {
     return <div className="w-full max-w-sm h-[380px] bg-white rounded-[32px] border border-gray-100 shadow-sm animate-pulse" />;
  }

  return (
    <div className="w-full max-w-sm bg-white rounded-[32px] border border-gray-100 shadow-crm-md p-6 flex flex-col gap-3 group overflow-hidden relative h-[380px]">
      ...
      <div className="flex-1 space-y-3 relative z-10 overflow-y-auto pr-1 scrollbar-hide">
         {(items || []).map((item, i) => (
           <motion.div
             ...
             className="flex items-center justify-between p-4 rounded-2xl bg-gray-50/50 border border-transparent hover:border-gray-100 hover:bg-white hover:shadow-sm transition-all duration-300 group/item cursor-pointer"
           >
              <div className="flex flex-col gap-0.5">
                 <div className="flex items-center gap-2">
                    <span className="text-[11px] font-black text-primary-base tracking-wider">{item.sku}</span>
                    <span className="text-[11px] font-black text-gray-300">• {item.variant}</span>
                 </div>
      ...
      <button type="button" className="w-full py-3.5 bg-gray-900 text-white rounded-2xl text-[11px] font-black flex items-center justify-center gap-2 hover:bg-black transition-colors shadow-lg active:scale-95 group/btn">
