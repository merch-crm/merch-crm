...
export function BentoMiniSparkline({ title, value, trend, percentage, className }: BentoMiniSparklineProps) {
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden bg-card text-card-foreground shadow-crm-md border border-border p-5 rounded-[27px] flex items-center justify-between",
        className
      )}
    >
      <div>
        <p className="text-xs font-semibold text-muted-foreground   mb-1">{title}</p>
        <h4 className="text-3xl font-black font-heading">{value}</h4>
      </div>
