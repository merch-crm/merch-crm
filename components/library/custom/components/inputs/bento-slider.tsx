...
export function BentoSlider({ 
  ...
  return (
    <div className={cn("w-full flex flex-col gap-3", className)}>
      ...
      <div className="relative h-14 bg-white rounded-[1.5rem] border border-border shadow-sm flex items-center px-4 group">
        <div className="relative w-full h-3 bg-secondary/50 rounded-full overflow-hidden shadow-inner">
          <motion.div
            animate={{ width: `${percentage}%` }}
            className="absolute left-0 top-0 h-full bg-primary shadow-lg shadow-primary/20 transition-colors"
          />
        </div>
        ...
        <motion.div
          ...
          className={cn(
            "absolute size-8 bg-white border-4 border-primary rounded-full shadow-xl pointer-events-none z-10 transition-transform duration-300",
            isDragging ? "shadow-primary/30" : "shadow-gray-950/10"
          )}
        >
          {isDragging && (
            <motion.div
