...
import { Upload, X, File, CheckCircle2, ArrowRight, CloudUpload, HardDrive, FileText, Check } from "lucide-react";
...
export function BentoFileUpload({ 
  ...
  if (!isMounted) {
    return (
      <div className="w-full max-w-xl h-64 rounded-[48px] bg-slate-50 border border-slate-100 animate-pulse" />
    );
  }
  ...
  return (
    <div className={cn("w-full max-w-xl mx-auto flex flex-col gap-6", className)}>
      <motion.button
        ...
        className={cn(
          "relative w-full flex flex-col items-center justify-center gap-6 rounded-[3rem] border-2 border-dashed p-16 cursor-pointer transition-all duration-700 overflow-hidden group shadow-premium outline-none focus-visible:ring-4 focus-visible:ring-primary-base/10",
      ...
      >
        ...
        <div className="size-24 rounded-[32px] bg-slate-50 shadow-inner flex items-center justify-center text-primary-base group-hover:scale-110 group-hover:rotate-3 transition-all duration-700 relative border border-slate-100">
          <CloudUpload className={cn("size-10", dragActive && "animate-bounce")} />
          <div className="absolute inset-0 bg-primary-base/10 rounded-[32px] blur-2xl group-hover:opacity-100 opacity-0 transition-opacity" />
        </div>
        
        <div className="text-center relative z-10 flex flex-col gap-3">
          <h4 className="text-[12px] font-black text-slate-900 uppercase tracking-[0.3em]">Payload Dropzone</h4>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">
            Initialize asset transmission via Drag & Drop<br/>
            <span className="text-primary-base opacity-60">PDF, JPG, PNG // MAX {maxSize}MB</span>
          </p>
        </div>

        <div className="text-[11px] font-black text-white bg-slate-950 px-10 py-4 rounded-2xl shadow-2xl shadow-black/20 group-hover:bg-primary-base group-hover:shadow-primary-base/30 transition-all duration-500 uppercase tracking-[0.2em] flex items-center gap-3 border border-slate-900">
          Initialize <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
        </div>
      </motion.button>
          ...
          >
            {files.map((f, i) => (
              <motion.div
                ...
                layout
                className="flex items-center gap-5 p-5 bg-white rounded-[2rem] border border-slate-100 shadow-premium group/item overflow-hidden relative"
              >
                <div className={cn(
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[11px] font-black text-slate-900 uppercase tracking-widest truncate">{f.file.name}</p>
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-tighter tabular-nums px-2 py-0.5 bg-slate-50 rounded-lg">
