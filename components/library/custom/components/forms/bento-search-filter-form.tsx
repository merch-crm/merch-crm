...
import { Search, Filter, SlidersHorizontal, Calendar, Tag, ChevronDown, X, Sparkles } from "lucide-react";
...
export function BentoSearchFilterForm({ className }: { className?: string }) {
  ...
  return (
    <div className={cn(
      "bg-white dark:bg-zinc-950 text-slate-950 dark:text-zinc-50 shadow-premium border border-slate-200 dark:border-zinc-800 p-6 sm:p-8 rounded-[3rem] flex flex-col w-full max-w-4xl mx-auto relative overflow-hidden",
      className
    )}>
      <div className="relative z-10 flex flex-col md:flex-row gap-3 items-center">
        <div className="relative flex-1 w-full group">
          ...
          <input 
            ...
            className="w-full pl-14 pr-6 py-5 bg-slate-50 dark:bg-zinc-900 border-none rounded-[2rem] text-sm font-bold focus:ring-2 focus:ring-primary-base/20 transition-all outline-none shadow-inner"
          />
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button 
            ...
            className={cn(
              "flex items-center justify-center gap-2 px-6 py-5 rounded-[2rem] border transition-all font-black text-xs  ",
              isFilterOpen 
                ? "bg-primary-base border-primary-base text-white shadow-lg shadow-primary-base/20" 
                : "bg-white dark:bg-zinc-900 border-slate-100 dark:border-zinc-800 text-slate-500 hover:border-slate-200"
            )}
          >
            <Filter size={16} /> Filters
          </button>
          
          <button type="button" className="flex items-center justify-center size-14 rounded-[2rem] bg-slate-950 dark:bg-white text-white dark:text-slate-950 hover:shadow-xl active:scale-95 transition-all">
             <Sparkles size={20} />
          </button>
        </div>
      </div>
      ...
      {isFilterOpen && (
        <div className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-3 mt-8 pt-8 border-t border-slate-100 dark:border-zinc-800/50 animate-in slide-in-from-top-4 duration-500">
           {[
             ...
             <div key={i} className="bg-slate-50 dark:bg-zinc-900/50 p-4 rounded-3xl border border-slate-100 dark:border-zinc-800 flex items-center justify-between group cursor-pointer hover:bg-white dark:hover:bg-zinc-900 transition-all shadow-sm hover:shadow-md">
