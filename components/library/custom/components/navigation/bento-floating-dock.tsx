...
import { Home, Search, MessageSquare, Database, Settings, Bell } from "lucide-react";
...
export function BentoFloatingDock({ items: propItems }: { items?: { icon: React.ReactNode; label: string; color: string; badge?: number }[] }) {
  ...
  return (
    <div className="w-full flex justify-center py-6 group/dock">
      <div className="flex items-end gap-3 px-6 py-4 bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-gray-100 shadow-crm-lg transition-all hover:shadow-2xl">
         {(items || []).map((item, i) => (
           <div key={i} className="relative flex flex-col items-center">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className={cn(
                  "size-12 rounded-2xl flex items-center justify-center text-white shadow-lg transition-all relative group/item",
                  item.color,
                  "hover:shadow-2xl"
                )}
              >
                 ...
                 <div className="absolute inset-0 bg-white/20 opacity-0 group-hover/item:opacity-100 rounded-2xl transition-opacity pointer-events-none" />
              </motion.button>
