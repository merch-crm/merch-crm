...
export function ThreeViewer({ 
    ...
    return (
        <div className="w-full h-[500px] rounded-2xl bg-slate-950 overflow-hidden relative border border-slate-800 shadow-2xl group/viewer isolate">
            ...
            <ThreeErrorBoundary>
                <Canvas key={resetKey} shadows dpr={[1, 2]} camera={{ position: [0, 0.5, 4], fov: 45 }} gl={{ preserveDrawingBuffer: true, toneMapping: THREE.ACESFilmicToneMapping }} style={{ touchAction: 'pan-y pinch-zoom', zIndex: 1 }}>
                    ...
                    <ReactBitsPhysicsWrapper zone={placementZone} autoRotate={autoRotate}>
                        <Stage environment="city" intensity={0.6} adjustCamera={false}>
                            <MerchModel url={modelUrl} color={itemColor} textureUrl={logoUrl} zone={placementZone} decalScale={decalScale} decalOffset={decalOffset} decalRotation={decalRotation} />
                        </Stage>
            ...
            <div className="absolute top-4 right-4 flex gap-2 z-10">
                <Button type="button" variant="ghost" size="sm" onClick={() => setResetKey(k => k + 1)}
