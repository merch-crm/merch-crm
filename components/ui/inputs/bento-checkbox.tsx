...
export function BentoCheckbox({ 
  ...
  if (!isMounted) {
    return (
      <div className="w-full h-20 rounded-card bg-slate-50 animate-pulse border border-slate-100" />
    );
  }
  ...
  return (
    <button
      ...
      className={cn(
        "flex items-start gap-4 p-5 rounded-card transition-all text-left outline-none focus-visible:ring-4 focus-visible:ring-primary-base/20 border group/checkbox",
        isChecked 
          ? "bg-white border-primary-base shadow-xl shadow-slate-200" 
          : "bg-[#F8FAFC] border-slate-100 hover:bg-white hover:border-slate-300 hover:shadow-sm",
        className
      )}
    >
      <div className={cn(
      ...
      <div className="flex flex-col gap-1.5">
        <span className={cn(
          "text-[11px] font-black tracking-[0.2em] transition-all duration-500",
          isChecked ? "text-slate-950" : "text-slate-400 group-hover/checkbox:text-slate-600"
        )}>
          {label}
        </span>
        {description && (
          <span className="text-[10px] font-black text-slate-300 tracking-widest leading-none line-clamp-1">
            {description}
          </span>
        )}
      </div>
