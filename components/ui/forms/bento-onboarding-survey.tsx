...
export function BentoOnboardingSurvey({ className }: { className?: string }) {
  ...
  return (
    <div className={cn(
      "bg-white dark:bg-zinc-950 text-slate-950 dark:text-zinc-50 shadow-premium border border-slate-200 dark:border-zinc-800 p-8 sm:p-12 rounded-card flex flex-col w-full max-w-2xl mx-auto relative overflow-hidden",
      className
    )}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 relative z-10">
        {roles.map((role) => (
          <button 
            ...
            className={cn(
              "group relative p-6 rounded-card border-2 transition-all duration-300 flex flex-col items-center text-center",
              "hover:shadow-xl hover:scale-[1.02] active:scale-[0.98]",
              selectedRole === role.id 
                ? "border-primary-base bg-[#F1F5F9] shadow-premium text-primary-base" 
                : "border-slate-100 dark:border-zinc-800 hover:border-primary-base bg-[#F8FAFC] dark:bg-zinc-900"
            )}
          >
            <div className={cn(
              "w-14 h-14 rounded-element mb-4 flex items-center justify-center transition-all duration-500",
              selectedRole === role.id 
                ? "bg-primary-base text-white shadow-lg rotate-6" 
      ...
      <div className="mt-10 pt-8 border-t border-slate-100 dark:border-zinc-800/50 flex flex-col sm:flex-row justify-between items-center gap-3 relative z-10">
        ...
        <button 
          ...
          className={cn(
            "w-full sm:w-auto px-10 py-3.5 rounded-element font-black text-xs   transition-all",
