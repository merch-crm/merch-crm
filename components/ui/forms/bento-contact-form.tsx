...
import { textAreaStyles, BentoInput } from "@/components/ui/bento-primitives";
...
export function BentoContactForm({ className }: { className?: string }) {
  ...
  return (
    <div className={cn(
      "bg-white dark:bg-zinc-950 text-slate-950 dark:text-zinc-50 shadow-premium border border-slate-200 dark:border-zinc-800 p-8 sm:p-10 rounded-card flex flex-col w-full max-w-lg mx-auto relative overflow-hidden",
      className
    )}>
      <div className="relative z-10 w-full">
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center justify-center py-12 animate-in slide-in-from-bottom-5 duration-700">
             <div className="w-20 h-20 rounded-card bg-primary-base/10 border border-primary-base/20 flex items-center justify-center mb-6 text-primary-base shadow-lg shadow-primary-base/10 rotate-3">
             ...
             <button 
               ...
               className="mt-8 px-8 py-3 bg-slate-950 dark:bg-white text-white dark:text-slate-950 rounded-element font-black text-xs   transition-all hover:shadow-xl active:scale-95"
             >
                Send another
             </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full mt-4 bg-slate-950 dark:bg-white text-white dark:text-slate-950 py-5 rounded-element font-black text-xs   flex items-center justify-center gap-2 transition-all hover:shadow-xl active:scale-95 disabled:opacity-50"
            >
              {loading ? (
