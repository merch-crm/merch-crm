...
import { Clock, Users, ArrowUpRight, Flame, ShieldCheck, MoreHorizontal } from 'lucide-react';
...
export function DealCard({ 
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col p-5 bg-white rounded-card border border-gray-100 shadow-sm transition-all duration-300 overflow-hidden",
        className
      )}
    >
      ...
      <div className="flex flex-col gap-1 pr-20">
         <div className="flex items-center gap-2">
            <Typo as="span" className={cn("px-2 py-0.5 rounded-lg text-[11px] font-black border", PRIORITY_MAP[priority].color)}>
               {PRIORITY_MAP[priority].label}
            </Typo>
            <Typo as="div" className="flex items-center gap-1 text-[11px] font-bold text-gray-400">
               <Clock className="size-3" />
               {daysAtStage}д в работе
            </Typo>
         </div>
         <Typo as="h4" className="text-base font-black text-gray-900 leading-tight mt-1 group-hover:text-primary-base transition-colors">
            {title}
         </Typo>
         <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <Typo as="span" className="text-xs font-bold text-gray-500 italic">{client}</Typo>
         </div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3">
         <div className="flex items-end justify-between">
            <div className="flex flex-col">
               <Typo as="span" className="text-[11px] font-black text-gray-400 leading-none mb-1">Сумма сделки</Typo>
               <Typo as="div" className="flex items-center text-2xl font-black text-gray-900 italic">
                  <Typo as="span" className="text-primary-base mr-1">₽</Typo>
                  {value.toLocaleString()}
               </Typo>
            </div>
            <div className="flex flex-col items-end">
               <Typo as="span" className="text-[11px] font-black text-gray-400 leading-none mb-1">Вероятность</Typo>
               <Typo as="span" className="text-sm font-black text-emerald-500 italic">{confidence}%</Typo>
            </div>
         </div>
      ...
      <div className="mt-6 pt-4 border-t border-gray-50 flex items-center justify-between">
         <div className="flex items-center gap-2">
            <div className="px-3 py-1.5 bg-gray-900 rounded-xl flex items-center gap-2 group-hover:bg-primary-dark transition-colors">
               ...
               <Typo as="span" className="text-[11px] font-black text-white">{stage}</Typo>
            </div>
         </div>
