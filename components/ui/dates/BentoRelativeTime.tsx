...
export function BentoRelativeTime({ items: propItems }: BentoRelativeTimeProps) {
  ...
  if (!isMounted) {
    return (
      <div className="w-full max-w-sm h-64 bg-white rounded-card border border-gray-100 shadow-sm animate-pulse p-6">
        <div className="flex items-center gap-2 mb-6">
           <div className="size-4 bg-gray-50 rounded-full" />
           <div className="h-4 w-32 bg-gray-50 rounded-full" />
        </div>
        <div className="space-y-4">
           {[1, 2, 3].map(i => <div key={i} className="h-12 bg-gray-50 rounded-element w-full" />)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-sm bg-white rounded-card border border-gray-100 shadow-sm p-6 relative group overflow-hidden hover:border-primary-base/30 transition-colors">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <Clock className="size-4 text-primary-base" />
          <h4 className="text-[11px] font-black text-slate-900 tracking-wide leading-none">Недавняя активность</h4>
        </div>
        ...
        <AnimatePresence initial={false}>
          {(items || []).map((item, i) => (
            <motion.div
              ...
              className="flex items-start gap-3 p-3 rounded-element hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 group/item"
            >
              <div className={cn(
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-[11px] font-black text-slate-900 tracking-tight">{item.text}</span>
                <span className="text-[11px] font-black text-slate-400 tracking-wider text-[10px]">
