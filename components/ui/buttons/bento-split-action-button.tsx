...
export function BentoSplitActionButton({ 
  ...
  if (!isMounted) {
    return (
      <div className={cn("inline-flex h-14 rounded-card bg-gray-100 animate-pulse w-40", className)} />
    );
  }

  return (
    <div className={cn("inline-flex h-14 rounded-card bg-primary shadow-crm-md relative", className)}>
      <motion.button
        type="button"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 mt-2 p-2 bg-white dark:bg-zinc-950 border border-gray-100 dark:border-zinc-800 shadow-2xl rounded-element z-50 overflow-hidden"
          >
            {options.map((opt, i) => (
              <button
                type="button"
