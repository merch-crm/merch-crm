...
export function BentoOrderQueue({ orders: propOrders }: { orders?: typeof DEFAULT_ORDERS }) {
  ...
  if (!isMounted) {
     return <div className="w-full max-w-sm h-64 bg-white rounded-card border border-gray-100 animate-pulse" />;
  }

  return (
    <div className="w-full max-w-sm bg-white rounded-card border border-gray-100 shadow-crm-md p-6 flex flex-col gap-3 group overflow-hidden relative">
      <div className="flex items-center justify-between px-1">
         ...
         <button type="button" className="text-xs font-black text-primary-base hover:underline flex items-center gap-1 group/btn">
      ...
      <div className="space-y-2 relative z-10">
         {(orders || []).map((order, i) => (
           <motion.div
             ...
             className="flex items-center justify-between p-3.5 rounded-xl bg-gray-50/50 border border-transparent hover:border-gray-100 hover:bg-white hover:shadow-sm transition-all duration-300 group/item"
           >
              <div className="flex items-center gap-3">
                 ...
                 <div className="flex flex-col">
                    ...
                    <span className="text-xs font-black text-gray-400 ">{order.items} изд. • {order.total}</span>
                 </div>
              </div>
              <div className="flex items-center gap-2">
                 <span className={cn(
      ...
      <div className="flex items-center gap-2 px-1 text-xs font-black text-gray-400">
