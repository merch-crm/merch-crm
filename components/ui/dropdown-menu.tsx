...
>(({ className, inset, children, ...props }, ref) => (
    <DropdownMenuPrimitive.SubTrigger
        ref={ref}
        className={cn(
            "flex cursor-default select-none items-center rounded-[var(--radius-inner)] px-3 py-2 text-sm font-bold outline-none focus:bg-slate-50 data-[state=open]:bg-slate-50 transition-colors",
            inset && "pl-9",
            className
        )}
        {...props}
...
>(({ className, inset, ...props }, ref) => (
    <DropdownMenuPrimitive.Label
        ref={ref}
        className={cn(
            "px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-400 leading-none",
            inset && "pl-9",
            className
        )}
        {...props}
...
const DropdownMenuShortcut = ({
    ...
    return (
        <span
            className={cn("ml-auto text-[10px] font-bold tracking-widest opacity-40 group-hover:opacity-60 transition-opacity", className)}
            {...props}
        />
    )
}
