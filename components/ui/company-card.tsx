  ...
  ({ name, industry, employees, website, location, logoUrl, revenue, className, ...rest }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          'group flex gap-3 rounded-element border border-stroke-soft-200 bg-bg-white-0 p-5 shadow-sm transition duration-200 hover:shadow-md hover:border-primary-base/30',
          className,
        )}
        {...rest}
      >
        ...
        <div className="flex flex-1 flex-col gap-1.5 min-w-0">
          <div className="flex items-center justify-between gap-3">
            ...
            {revenue && (
               <span className="shrink-0 text-[11px] font-black text-emerald-600 tabular-nums tracking-tighter">
                {revenue}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 pt-1 text-[11px] font-bold text-slate-400 tracking-wide">
            {employees && (
              <div className="flex items-center gap-1.5 title-case">
