  ...
  ({ title, value, change, changeLabel, trend = 'neutral', icon, sparkline, className, ...rest }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'relative flex flex-col gap-3 rounded-element border border-stroke-soft-200 bg-bg-white-0 p-5 shadow-sm transition duration-200 hover:shadow-md',
          className,
        )}
        {...rest}
      >
        <div className="flex items-center justify-between">
          <span className="text-[11px] font-black text-text-soft-400 tracking-wider">{title}</span>
          {icon && (
