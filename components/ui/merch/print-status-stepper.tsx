...
export function MerchStatusStepper({ currentStatus, className }: MerchStatusStepperProps) {
  ...
  return (
    <div className={cn("flex flex-col gap-0 w-full max-w-md", className)}>
        ...
        return (
          <div key={step.status} className="relative flex gap-3 group">
            ...
            <div className="flex flex-col py-3 pb-8">
              <Typo as="span" className={cn( "text-sm font-bold transition-colors", isActive ? "text-gray-900" : isCompleted ? "text-gray-600" : "text-gray-400" )}>
                {step.label}
              </Typo>
              <Typo as="span" className={cn( "text-xs transition-colors", isActive ? "text-gray-500" : "text-gray-400" )}>
                {step.description}
              </Typo>
              ...
              {isActive && (
                 ...
                 >
                   ...
                   <Typo as="span" className="text-[11px] font-bold text-primary-base ">Текущий статус</Typo>
                 </motion.div>
              )}
            </div>
          </div>
      ...
      {isError && (
        <div className="mt-4 p-4 bg-red-50 border border-red-100 rounded-element flex gap-3 items-center">
          <div className="size-10 rounded-xl bg-red-500 flex items-center justify-center text-white shrink-0">
             <AlertCircle className="size-6" />
          </div>
          <div className="flex flex-col">
            <Typo as="span" className="text-sm font-bold text-red-900">Ошибка выполнения</Typo>
            <Typo as="span" className="text-xs text-red-600">Процесс приостановлен. Пожалуйста, проверьте настройки оборудования.</Typo>
          </div>
        </div>
      )}
    </div>
