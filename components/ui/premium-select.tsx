...
export function PremiumSelect({
    ...
    return (
        <div className={cn("relative", variant === "default" && "space-y-1.5", className)}>
            {label && (
                <label className="block text-[10px] font-black text-slate-400 ml-1 uppercase tracking-widest leading-none mb-1.5">
                    {label}
                </label>
            )}
            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <button
                        ...
                        className={cn(
                            ...
                            variant === "default" ? [
                                "bg-white border border-slate-100 rounded-xl shadow-sm hover:shadow-md",
                                compact ? "h-10 px-3.5" : "h-12 px-4",
                                open && "ring-4 ring-primary/5 border-primary/20 shadow-md"
                            ] : [
                                "bg-white rounded-[16px] shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_15px_rgba(0,0,0,0.08)] px-3 py-2 border border-slate-50",
                                open && "shadow-[0_4px_20px_rgba(0,0,0,0.12)] border-primary/20"
                            ]
                        )}
                    >
                        <div className="flex items-center gap-2 overflow-hidden">
                            <span className={cn(
                                ...
                                compact || variant === "minimal" ? "text-[12px]" : "text-[13px]",
                                selectedOption ? "text-slate-900" : "text-slate-400"
                            )}>
                                {selectedOption ? selectedOption.title : placeholder}
                            </span>
                            {selectedOption?.badge && !compact && variant !== "minimal" && (
                                <span className="text-[9px] font-black bg-primary/10 text-primary px-1.5 py-0.5 rounded-full shrink-0">
                ...
                >
                    <div className={cn(
                        "bg-white rounded-[22px] p-2 shadow-[0_20px_60px_rgba(0,0,0,0.2)] ring-1 ring-slate-900/5 border border-slate-50 overflow-hidden flex flex-col",
                        variant === "minimal" ? "min-w-[140px]" : "min-w-[var(--radix-popover-trigger-width)] w-max",
                        "max-h-[300px]"
                    )}>
                        {effectiveShowSearch && (
                            <div className="p-1.5 border-b border-slate-50 mb-1">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-9 pl-8 pr-3 rounded-lg bg-slate-50 border-none outline-none text-xs font-bold placeholder:text-slate-300"
                                    />
                                </div>
                            </div>
                        )}
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    effectiveGridColumns ? [
                                                        "flex flex-col items-center justify-center p-3 rounded-xl gap-1",
                                                ...
                                                <div className={cn(
                                                    ...
                                                    <div className={cn(
                                                        ...
                                                        <div className="flex items-center gap-2 min-w-0">
                                                            ...
                                                            {option.badge && (
                                                                <span className={cn(
                                                                    "text-[8px] font-black px-1.5 py-0.5 rounded-full shrink-0",
                                                    ...
                                                    {/* Description hidden as per user request to avoid technical codes like 'Код: X' */}
                                                    {/* {option.description && !effectiveGridColumns && (
                                                        <p className={cn(
                                                            "text-[10px] font-medium leading-tight mt-1 whitespace-nowrap",
