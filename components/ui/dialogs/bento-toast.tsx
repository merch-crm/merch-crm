...
const colors = {
  success: "border-emerald-100 bg-emerald-50 shadow-md",
  error: "border-rose-100 bg-rose-50 shadow-md",
  info: "border-blue-100 bg-blue-50 shadow-md",
  warning: "border-amber-100 bg-amber-50 shadow-md"
};
      ...
      <AnimatePresence mode="popLayout">
        {toasts.map((toast) => (
          <motion.div
            ...
            className={cn(
              "p-4 rounded-card border shadow-2xl flex items-start gap-4 group/toast relative overflow-hidden",
              colors[toast.type]
            )}
          >
            ...
            <div className="flex-1 pr-6 flex flex-col gap-1">
              <h4 className="text-[11px] font-black text-slate-900 tracking-[0.2em] leading-none">
                {toast.title}
              </h4>
              {toast.message && (
                <p className="text-[11px] font-black text-slate-400 tracking-tight leading-tight line-clamp-2">
                  {toast.message}
                </p>
              )}
            </div>
            ...
            <motion.div
              ...
              className={cn(
                ...
                toast.type === "info" ? "bg-blue-200" : "bg-amber-200"
              )}
            />
          </motion.div>
        ))}
