...
function EventCard({ title, type, startTime, endTime, location, attendees, status = 'upcoming', className, ...rest }: EventCardProps) {
  ...
  return (
    <div
      className={cn(
        'group flex flex-col gap-3 rounded-element border border-stroke-soft-200 bg-bg-white-0 p-5 shadow-sm transition duration-200 hover:shadow-md hover:border-primary-base/20',
        className,
      )}
      {...rest}
    >
