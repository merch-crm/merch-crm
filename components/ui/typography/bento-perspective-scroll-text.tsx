...
export function BentoPerspectiveScrollText() {
  return (
    <div className="w-full max-w-sm rounded-card bg-white p-6 h-[250px] flex flex-col gap-3 overflow-hidden relative group border border-gray-100 shadow-crm-md">
      <div className="flex items-center justify-between mb-2 z-20">
         <h3 className="text-orange-600 text-xs font-black tracking-tight">Поток данных</h3>
      </div>
             ...
             {[1, 2, 3, 4, 5, 6].map((i) => (
               <div key={i} className="flex flex-col gap-2 p-5 bg-white border border-gray-200 rounded-xl shadow-crm-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-mono text-gray-500 ">Событие #{i + 142}</span>
