...
```mermaid
graph TD
    COLL[Коллекции Принтов] -->|1:N| DES[Дизайны Принтов]
    DES -->|1:N| VER[Версии Дизайнов]
    VER -->|1:N| FILES[Файлы Версий]
    DES -->|1:N| MOCK[Мокапы Дизайнов]
    
    ORDER[Заказы] -->|1:N| ODT[Задачи Дизайна]
    ODT -->|1:N| ODTF[Файлы Задач]
    ODT -->|1:N| ODTH[История Задач]
    DES -.->|Шаблон| ODT
```
