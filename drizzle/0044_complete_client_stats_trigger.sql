-- Migration: Complete Client Statistics Synchronization with RFM
-- This script replaces the existing trigger function with a more comprehensive one 
-- that includes RFM scoring and segment determination.

-- Cleanup old conflicting triggers if they exist
...
AFTER INSERT OR UPDATE OF total_amount, is_archived, client_id, created_at, status OR DELETE
ON orders
FOR EACH ROW
