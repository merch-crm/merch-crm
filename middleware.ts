...
export function middleware(request: NextRequest) {
 ...
 // Skip CSP and nonce for API routes to reduce overhead
 if (isApiRoute) {
 ...
 const isCalculatorsRoute = request.nextUrl.pathname.includes('/dashboard/production/calculators');
 const appUrl = process.env.NEXT_PUBLIC_APP_URL || '';
 const isHttps = appUrl.startsWith('https');

 // Relieve CSP for calculators (Fabric.js/Three.js might need eval)
 const scriptSrc = (isDev || isCalculatorsRoute)
